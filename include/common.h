...
/* missing constants */
	...
	#define countof(__x)   (sizeof(__x) / sizeof(__x[0]))
		...
		#define isnan _isnan
					...
					#undef isnan
				#endif 
template <typename Tp> inline int isnan( Tp f )
{
	return ( f != f );
}
...
namespace std
{
	template <typename comparable> inline const comparable &max( const comparable &a, const comparable &b )
	{
		return a<b ? b : a;
	}
}
...
namespace std
{
	template <typename comparable> inline const comparable &min( const comparable &a, const comparable &b )
	{
		return b<a ? b : a;
	}
}
