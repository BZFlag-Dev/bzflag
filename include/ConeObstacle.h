...
public:
	...
	enum
	{
		Edge, Bottom, StartFace, EndFace, MaterialCount
	};

	ConeObstacle();
	ConeObstacle( const MeshTransform &transform, const float *_pos, const float *_size, float _rotation, float _angle, const float _texsize[2], bool _useNormals, int _divisions, const BzMaterial *mats[MaterialCount], int physics, bool bounce, bool drive, bool shoot );
	~ConeObstacle();
