...
public:
	...
	unsigned char *readImage( const std::string &filename, int &width, int &height, int &depth )const;
	...
	// the returned memory.
	float *readSound( const std::string &filename, int &numFrames, int &rate )const;
	...
	virtual void audioSleep( bool checkLowWater, double maxTime =  - 1.0 ) = 0;
...
protected:
	...
	virtual unsigned char *doReadImage( const std::string &filename, int &width, int &height, int &depth )const;

	// return NULL on failure
	virtual float *doReadSound( const std::string &filename, int &numFrames, int &rate )const;

	// concatenate directory to filename
	virtual std::string makePath( const std::string &dir, const std::string &filename )const;

	// replace (or add) extension to pathname
	std::string replaceExtension( const std::string &pathname, const std::string &extension )const;
