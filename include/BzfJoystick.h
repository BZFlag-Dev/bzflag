...
public:
	...
	virtual void ffRumble( int count, float delay, float duration, float strong_motor, float weak_motor = 0.0f );
	...
	enum PeriodicType
	{
		FF_Sine = 0, FF_Square, FF_Triangle, FF_SawtoothUp, FF_SawtoothDown
	};
	enum ResistanceType
	{
		FF_Position = 0, FF_Velocity, FF_Acceleration
	};
	virtual bool ffHasDirectional()const;
	virtual void ffDirectionalConstant( int count, float delay, float duration, float x_direction, float y_direction, float strength );
	virtual void ffDirectionalPeriodic( int count, float delay, float duration, float x_direction, float y_direction, float amplitude, float period, PeriodicType type );
	virtual void ffDirectionalResistance( float time, float coefficient, float saturation, ResistanceType type );
};
