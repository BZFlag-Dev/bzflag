...
typedef enum
{
	...
	ResetColor = 9, FinalResetColor = 12, BrightColor = 13, DimColor = 14, PulsatingColor = 10, NonPulsatingColor = 15, UnderlineColor = 11, NonUnderlineColor = 16, 
...
inline std::string stripAnsiCodes( const std::string &text )
{
	...
	for( int i = 0; i < length; i++ )
	{
		if( text[i] == ESC_CHAR )
		{
			...
			{
				i++;
				while(( i < length ) && (( text[i] == ';' ) || (( text[i] >= '0' ) && ( text[i] <= '9' ))))
				{
					i++;
				}
			}
