...
public:
	...
	MeshFace( MeshObstacle *mesh, int vertexCount, float **vertices, float **normals, float **texcoords, const BzMaterial *material, int physics, bool noclusters, bool smoothBounce, bool drive, bool shoot );
	~MeshFace();
...
private:
	...
	enum
	{
		XPlane = ( 1 << 0 ), YPlane = ( 1 << 1 ), ZPlane = ( 1 << 2 ), UpPlane = ( 1 << 3 ), DownPlane = ( 1 << 4 ), WallPlane = ( 1 << 5 )
	} PlaneBits;
	...
	enum
	{
		LinkToFace = ( 1 << 0 ), LinkFromFace = ( 1 << 1 ), BaseFace = ( 1 << 2 ), IcyFace = ( 1 << 3 ), StickyFace = ( 1 << 5 ), DeathFace = ( 1 << 6 ), PortalFace = ( 1 << 7 )
	} SpecialBits;
