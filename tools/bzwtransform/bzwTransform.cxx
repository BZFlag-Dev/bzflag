...
public:
	...
	void postMultiply( Matrix3D &inMatrix )
	{
		...
		for( int y = 0; y < 4; y++ )
		{
			for( int x = 0; x < 4; x++ )
			{
				matrix[x][y] = src[0][y] *inMatrix.matrix[x][0] + src[1][y] *inMatrix.matrix[x][1] + src[2][y] *inMatrix.matrix[x][2] + src[3][y] *inMatrix.matrix[x][3];
			}
		}
	...
	void preMultiply( Matrix3D &inMatrix )
	{
		...
		for( int y = 0; y < 4; y++ )
		{
			for( int x = 0; x < 4; x++ )
			{
				matrix[x][y] = inMatrix.matrix[0][y] *src[x][0] + inMatrix.matrix[1][y] *src[x][1] + inMatrix.matrix[2][y] *src[x][2] + inMatrix.matrix[3][y] *src[x][3];
			}
		}
	}
private:
...
void parsebzwt( std::ifstream &bzwt, std::ofstream &bzw )
{
	enum
	{
		FREE, IN_TRANSFORM, IN_BOX, IN_PYRAMID, IN_MATRIX, IN_POSITION0, IN_POSITION1, IN_POSITION2, IN_SIZE0, IN_SIZE1, IN_SIZE2, IN_ROTATION, IN_MATRIX_POSITION, IN_MATRIX_SIZE, IN_MATRIX_ROTATION, IN_ANGLE, IN_SCALE0, IN_SCALE1, IN_SCALE2, IN_TRANSLATE0, IN_TRANSLATE1, IN_TRANSLATE2, IN_COUNT
	};
