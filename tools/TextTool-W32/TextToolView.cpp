...
ON_WM_ERASEBKGND()ON_COMMAND( ID_FONT_SETFONT, OnFontSetfont )ON_COMMAND( ID_FONT_SAVEFONTFILES, OnFontSavefontfiles )ON_COMMAND( ID_FILE_BATCHPROCESSING, OnBatchProcessing )
//}}AFX_MSG_MAP
...
BOOL CTextToolView::PreCreateWindow( CREATESTRUCT &cs )
{
	...
	m_pFont = new CFont;
	m_pFont->CreateFont( m_iFontPointSize, 0, 0, 0, FW_NORMAL, FALSE, FALSE, 0, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, "Arial" );
...
void CTextToolView::OnBatchProcessing()
{
	...
	if( oFile.DoModal() == IDOK )
	{
		...
		while( good )
		{
			...
			delete ( m_pFont );
			m_pFont = new CFont;
			m_pFont->CreateFont( nHeight, 0, 0, 0, bold, italic, FALSE, FALSE, ANSI_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, quality, DEFAULT_PITCH | FF_DONTCARE, item.font.c_str());
