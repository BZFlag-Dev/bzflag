...
function bzfquery1910($server, &$fp)
{
  ...
  while (in_array(false, $have) && $loop < 64)
  {
    ...
    switch ($packet['code'])
    {
      case MsgQueryGame:
        $server += unpack(
          "nstyle/nmaxPlayers/nmaxShots/nrogueSize/nredSize/ngreenSize/nblueSize/npurpleSize/nrogueMax/nredMax/ngreenMax/nblueMax/npurpleMax/nshakeWins/nshakeTimeout/nmaxPlayerScore/nmaxTeamScore/nmaxTime", $packet['data']);
        $have['QueryGame'] = true;
        break;
      ...
      case MsgTeamUpdate:
        ...
        {
          $server['team'][$team] = unpack("nnum/nsize/nwon/nlost",
            $packet['data']);
        ...
        break;
      case MsgAddPlayer:
        $player = unpack("Cid/ntype/nteam/nwon/nlost/ntks/a32sign/a128email",
          $packet['data']);
...
function bzfquery0026($server, &$fp)
{
  ...
  while (in_array(false, $have) && $loop < 64)
  {
    ...
    switch ($packet['code'])
    {
      case MsgQueryGame:
        $server += unpack(
          "nstyle/nmaxPlayers/nmaxShots/nrogueSize/nredSize/ngreenSize/nblueSize/npurpleSize/nobserverSize/nrogueMax/nredMax/ngreenMax/nblueMax/npurpleMax/nobserverMax/nshakeWins/nshakeTimeout/nmaxPlayerScore/nmaxTeamScore/nmaxTime/ntimeElapsed", $packet['data']);
        $have['QueryGame'] = true;
        break;
      case MsgQueryPlayers:
        $server += unpack("nnumTotalTeams/nnumPlayers", $packet['data']);
...
function bzfquery0047($server, &$fp)
{
  ...
  while (in_array(false, $have) && $loop < 64)
  {
    ...
    switch ($packet['code'])
    {
      case MsgQueryGame:
        $server += unpack(
          "ngameType/ngameOptions/nmaxPlayers/nmaxShots/nrogueSize/nredSize/ngreenSize/nblueSize/npurpleSize/nobserverSize/nrogueMax/nredMax/ngreenMax/nblueMax/npurpleMax/nobserverMax/nshakeWins/nshakeTimeout/nmaxPlayerScore/nmaxTeamScore/nmaxTime/ntimeElapsed", $packet['data']);
        $have['QueryGame'] = true;
        break;
      case MsgQueryPlayers:
        $server += unpack("nnumTotalTeams/nnumPlayers", $packet['data']);
  ...
  switch ($server['protocol'])
  {
    case '1910':
      ...
      if ($server['style'] & 0x0001)
        echo" CTF";
      if ($server['style'] & 0x0002)
        echo" flags";
      if ($server['style'] & 0x0008)
        echo" jumping";
      if ($server['style'] & 0x0010)
        echo" inertia";
      if ($server['style'] & 0x0020)
        echo" ricochet";
      if ($server['style'] & 0x0040)
        echo" shaking";
      if ($server['style'] & 0x0080)
        echo" antidote";
      if ($server['style'] & 0x0100)
        echo" handicap";
    ...
    case '0047':
      ...
      if ($server['gameType'] == 1)
        echo" Classic CTF";
      if ($server['gameType'] == 2)
        echo "Open FFA";
      if ($server['gameType'] == 3)
        echo "Rabbit Chase";
      echo"\n";

      echo"gameOptions:";
      if ($server['gameOptions'] & 0x0002)
        echo" Super Flag";
      if ($server['gameOptions'] & 0x0004)
        echo" No Team Kills";
      if ($server['gameOptions'] & 0x0008)
        echo" Jumping";
      if ($server['gameOptions'] & 0x0010)
        echo" Inertia";
      if ($server['gameOptions'] & 0x0020)
        echo" Ricochet";
      if ($server['gameOptions'] & 0x0040)
        echo" Shakable";
      if ($server['gameOptions'] & 0x0080)
        echo" Antidote";
      ...
      echo"\n";
      $playerType = array(0 => "tank player", 1 => "computer player", 2 => "chat player");
      while (list(, $player) = each($server['player']))
      {
        echo"player " . $player['sign'] . "
          (" . $teamName[$player['team']] . " team) is a
