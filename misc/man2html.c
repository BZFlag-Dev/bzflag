...
char *get_token( char *inp, char *buf )
{
	int quoted = 0;
	/* skip whitespace */
...
void twoggle( char *a, char *b, char *l )
{
	int first = 1;
	char *c;
	char buf[1000];
...
int process_line()
{
	...
	else if( strcmp( cmd, ".br" ) == 0 || strcmp( cmd, ".Sp" ) == 0 || strcmp( cmd, ".ti" ) == 0 )
	{
		if( need_undl )
		{
			need_undl = 0;
	...
	else if( strcmp( cmd, ".LP" ) == 0 || strcmp( cmd, ".PP" ) == 0 || strcmp( cmd, ".sp" ) == 0 || strcmp( cmd, ".P" ) == 0 )
	{
		if( need_undl )
		{
			need_undl = 0;
	...
	else if( strcmp( cmd, ".SH" ) == 0 || strcmp( cmd, ".SS" ) == 0 )
	{
		char *cp = buf;
		int got_token = 0;
		while( need_re )
	...
	else if( strcmp( cmd, ".TP" ) == 0 || strcmp( cmd, ".Tp" ) == 0 )
	{
		if( !need_undl )
		{
			printf( "<p><dl compact>" );
