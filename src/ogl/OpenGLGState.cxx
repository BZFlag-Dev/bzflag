...
OpenGLGStateState::Sorted::Sorted(): hasTexture( false ), hasTextureMatrix( false ), hasSphereMap( false ), hasMaterial( false ), texture(  - 1 ), textureMatrix( NULL ), textureEnvMode( GL_MODULATE ), material( OpenGLMaterial())
{
	// do nothing
} 
...
bool OpenGLGStateState::Sorted::operator == ( const Sorted &s )const
{
	if( hasTexture != s.hasTexture || texture != s.texture )
	{
		return false;
	}
	if(( hasTextureMatrix != s.hasTextureMatrix ) || ( textureMatrix != s.textureMatrix ))
	{
		return false;
	}
...
OpenGLGStateState::Unsorted::Unsorted(): needsSorting( false ), hasBlending( false ), hasStipple( false ), hasSmoothing( false ), hasCulling( true ), hasShading( false ), hasAlphaFunc( false ), blendSFactor( GL_ONE ), blendDFactor( GL_ZERO ), stippleIndex( 0 ), culling( GL_BACK ), alphaFunc( GL_ALWAYS ), alphaRef( 0.0f )
{
	// do nothing
}
...
{
	if( on )
		sorted.hasMaterial = sorted.material.isValid();
	else
		sorted.hasMaterial = false;
}
...
void OpenGLGStateState::setTexture( const int _texture )
{
...
void OpenGLGStateState::setTextureMatrix( const GLfloat *_textureMatrix )
{
...
void OpenGLGStateState::setTextureEnvMode( GLenum mode )
{
...
void OpenGLGStateState::setMaterial( const OpenGLMaterial &_material )
{
...
void OpenGLGStateState::setStipple( float alpha )
{
	...
	unsorted.hasStipple = ( unsorted.stippleIndex < OpenGLGState::getOpaqueStippleIndex());
}
...
void OpenGLGStateState::setOpenGLState( const OpenGLGStateState *oldState )const
{
	...
	if( oldState )
	{
		...
		if( unsorted.hasBlending )
		{
			...
			{
				if( unsorted.blendSFactor != oldState->unsorted.blendSFactor || unsorted.blendDFactor != oldState->unsorted.blendDFactor )
		...
		if( unsorted.hasAlphaFunc )
		{
			...
			{
				if( unsorted.alphaFunc != oldState->unsorted.alphaFunc || unsorted.alphaRef != oldState->unsorted.alphaRef )
...
protected:
	// only protected to shutup compiler;  would be private
...
SortedGState::SortedGState( const OpenGLGStateState::Sorted &_state ): prev( NULL ), next( NULL ), state( _state ), firstGState( NULL )
{
...
OpenGLGStateRep::OpenGLGStateRep( const OpenGLGStateState &_state ): refCount( 1 ), state( _state )
{
	SortedGState::add( this );
}
...
void OpenGLGStateRep::addRenderNode( RenderNode *node, const OpenGLGState *gstate )
{
	assert( bucket != NULL );
...
OpenGLGState::ContextInitializer *OpenGLGState::ContextInitializer::head = NULL;
OpenGLGState::ContextInitializer *OpenGLGState::ContextInitializer::tail = NULL;
bool OpenGLGState::executingFreeFuncs = false;
bool OpenGLGState::executingInitFuncs = false;

OpenGLGState::ContextInitializer::ContextInitializer( OpenGLContextFunction _freeCallback, OpenGLContextFunction _initCallback, void *_userData ): freeCallback( _freeCallback ), initCallback( _initCallback ), userData( _userData )
{
	prev = NULL;
	next = head;
	head = this;
...
OpenGLGState::ContextInitializer *OpenGLGState::ContextInitializer::find( OpenGLContextFunction _freeCallback, OpenGLContextFunction _initCallback, void *_userData )
{
	ContextInitializer *scan = head;
	while( scan )
	{
		if(( scan->freeCallback == _freeCallback ) && ( scan->initCallback == _initCallback ) && ( scan->userData == _userData ))
			return scan;
		scan = scan->next;
	}
	return NULL;
...
void OpenGLGState::initStipple( void* )
{
	...
	for( int i = 0; i < NumStipples; i++ )
	{
		...
		lineStipple = ( GLushort )stipplePattern[i][0] + ((( GLushort )stipplePattern[i][0] ) << 8 );
...
void OpenGLGState::registerContextInitializer( OpenGLContextFunction freeCallback, OpenGLContextFunction initCallback, void *userData )
{
...
void OpenGLGState::unregisterContextInitializer( OpenGLContextFunction freeCallback, OpenGLContextFunction initCallback, void *userData )
{
	ContextInitializer *ci = ContextInitializer::find( freeCallback, initCallback, userData );
	if( ci == NULL )
	{
...
void OpenGLGStateBuilder::setTexture( const int texture )
{
...
void OpenGLGStateBuilder::setTextureMatrix( const GLfloat *textureMatrix )
{
...
void OpenGLGStateBuilder::setTextureEnvMode( GLenum mode )
{
...
void OpenGLGStateBuilder::setMaterial( const OpenGLMaterial &material )
{
...
void OpenGLGStateBuilder::setBlending( GLenum sFactor, GLenum dFactor )
{
...
void OpenGLGStateBuilder::setAlphaFunc( GLenum func, GLclampf ref )
{
...
static const int maxMatrixDepth[3] = 
{
	32, 2, 2
}; // guaranteed
