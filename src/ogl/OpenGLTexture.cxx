...
{
	GL_NEAREST, GL_NEAREST, GL_LINEAR, GL_NEAREST_MIPMAP_NEAREST, GL_LINEAR_MIPMAP_NEAREST, GL_NEAREST_MIPMAP_LINEAR, GL_LINEAR_MIPMAP_LINEAR
};
const GLenum OpenGLTexture::magnifyFilter[] = 
{
	GL_NEAREST, GL_NEAREST, GL_LINEAR, GL_NEAREST, GL_LINEAR, GL_NEAREST, GL_LINEAR
};
const char *OpenGLTexture::configFilterNames[] = 
{
	"no", "nearest", "linear", "nearestmipmapnearest", "linearmipmapnearest", "nearestmipmaplinear", "linearmipmaplinear"
};
...
OpenGLTexture::OpenGLTexture( int _width, int _height, const GLvoid *pixels, Filter _filter, bool _repeat, int _internalFormat ): width( _width ), height( _height )
{
	alpha = false;
	repeat = _repeat;
	...
	OpenGLGState::registerContextInitializer( static_freeContext, static_initContext, ( void* )this );
}
...
{
	OpenGLGState::unregisterContextInitializer( static_freeContext, static_initContext, ( void* )this );
	delete []imageMemory;
...
void OpenGLTexture::initContext()
{
	...
	gluBuild2DMipmaps( GL_TEXTURE_2D, internalFormat, scaledWidth, scaledHeight, GL_RGBA, GL_UNSIGNED_BYTE, image );
	glBindTexture( GL_TEXTURE_2D, 0 );
...
bool OpenGLTexture::setupImage( const GLubyte *pixels )
{
	...
	if(( scaledWidth != width ) || ( scaledHeight != height ))
	{
		...
		gluScaleImage( GL_RGBA, width, height, GL_UNSIGNED_BYTE, aligned, scaledWidth, scaledHeight, GL_UNSIGNED_BYTE, alignedScaled );
...
int OpenGLTexture::getBestFormat( int _width, int _height, const GLvoid *pixels )
{
	...
	static bool noIntensity = (( glRenderer == NULL ) || ( strncmp( glRenderer, "IMPACT", 6 ) == 0 ) || ( strncmp( glRenderer, "3Dfx", 4 ) == 0 ));
	if( !noIntensity )
	{
	...
	return ( useLuminance ? ( useAlpha ? GL_LUMINANCE_ALPHA : GL_LUMINANCE ): ( useAlpha ? GL_RGBA : GL_RGB ));
}
