...
OpenGLTexture::Rep::Rep(int _maxFilter) :
				refCount(1), list(0),
				alpha(False), maxFilter(_maxFilter)
				...
				int internalFormat)
{
  ...
  while (scaledWidth < width) scaledWidth <<= 1;
  ...
  const int copyWidth = (scaledWidth > width) ? scaledWidth : width;
  const int copyHeight = (scaledHeight > height) ? scaledHeight : height;
...
#if defined(BZF_TEXTURE_OBJECT)
  if (hasTextureObject > 0)
    glBindTexture(GL_TEXTURE_2D, rep->list);
  else
#endif // BZF_TEXTURE_OBJECT
  glNewList(rep->list, GL_COMPILE);

  rep->setFilter(maxFilter);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S,
			repeat ? GL_REPEAT : GL_CLAMP);
  glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T,
			repeat ? GL_REPEAT : GL_CLAMP);
  do {
    boolean doScale = (scaledWidth != width || scaledHeight != height);

    // scale image to next mipmap level
    if (doScale)
      gluScaleImage(
		GL_RGBA, width, height, GL_UNSIGNED_BYTE, data,
    ...
    scaledHeight >>= 1;
    if (doScale)
      ::memcpy(data, scaledData, 4 * width * height);
  } while (mipmap && (scaledWidth > 0 || scaledHeight > 0));

  rep->setFilter(getFilter());
