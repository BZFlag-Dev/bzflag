...
int openBroadcast( int port, const char *service, struct sockaddr_in *addr )
{
	...
	/* set reuse port */
	if( setsockopt( fd, SOL_SOCKET, SO_REUSEPORT, ( SSOType ) &optOn, sizeof( optOn )) < 0 )
	{
	...
	/* set reuse address */
	if( setsockopt( fd, SOL_SOCKET, SO_REUSEADDR, ( SSOType ) &optOn, sizeof( optOn )) < 0 )
	{
	...
	/* make broadcast */
	if( setsockopt( fd, SOL_SOCKET, SO_BROADCAST, ( SSOType ) &optOn, sizeof( optOn )) < 0 )
	{
	...
	for( i = 0; i < numInterfaces; ++i )
	{
		...
		const sockaddr_in *ifbaddr = ( const sockaddr_in* ) &req[i].ifr_ifru.ifru_broadaddr;
...
int sendBroadcast( int fd, const void *buffer, int bufferLength, const struct sockaddr_in *addr )
{
	return sendto( fd, ( const char* )buffer, bufferLength, 0, ( const struct sockaddr* )addr, sizeof( *addr ));
} 
...
int recvBroadcast( int fd, void *buffer, int bufferLength, struct sockaddr_in *addr )
{
	struct sockaddr_in from;
	AddrLen fromLength = sizeof( from );

	int byteCount = recvfrom( fd, ( char* )buffer, bufferLength, 0, ( struct sockaddr* ) &from, ( socklen_t* ) &fromLength );
	if( byteCount < 0 )
	{
