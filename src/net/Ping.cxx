...
PingPacket::PingPacket(): gameStyle( TeamFFAGameStyle ), maxShots( 1 ), shakeWins( 0 ), shakeTimeout( 0 ), maxPlayerScore( 0 ), maxTeamScore( 0 ), maxTime( 0 ), maxPlayers( 1 ), rogueCount( 0 ), rogueMax( 1 ), redCount( 0 ), redMax( 1 ), greenCount( 0 ), greenMax( 1 ), blueCount( 0 ), blueMax( 1 ), purpleCount( 0 ), purpleMax( 1 ), observerCount( 0 ), observerMax( 1 )
{
	// do nothing
}
...
bool PingPacket::waitForReply( int fd, const Address &from, int millisecondsToBlock )
{
...
bool PingPacket::write( int fd, const struct sockaddr_in *addr )
const
{
...
bool PingPacket::isRequest( int fd, struct sockaddr_in *addr )
{
	if( fd < 0 )
		return false;
	char buffer[6];
...
bool PingPacket::sendRequest( int fd, const struct sockaddr_in *addr )
{
	if( fd < 0 || !addr )
		return false;
	char buffer[6];
