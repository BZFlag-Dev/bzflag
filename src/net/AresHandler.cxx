...
void AresHandler::queryHostname( struct sockaddr *clientAddr )
{
	if( aresFailed )
		return ;
	memcpy( &requestedAddress, &(( sockaddr_in* )clientAddr )->sin_addr, sizeof( requestedAddress ));
	...
	ares_gethostbyaddr( aresChannel, &requestedAddress, sizeof( in_addr ), AF_INET, staticCallback, ( void* )this );
} 
...
void AresHandler::queryHost( char *hostName )
{
	...
	// launch the asynchronous query to look up this hostname
	status = HbNPending;
	ares_gethostbyname( aresChannel, queryHostName, AF_INET, staticCallback, ( void* )this );
}
...
void AresHandler::staticCallback( void *arg, int callbackStatus, struct hostent *hostent )
{
...
void AresHandler::callback( int callbackStatus, struct hostent *hostent )
{
	...
	else if( status == HbAPending )
	{
		...
		status = HbASucceeded;
		DEBUG2( "Address %s resolved to %s\n", inet_ntoa( requestedAddress ), hostname );
	}
