...
const char *usageString = "[-admsg <text>] ""[-advertise <group,group...>]""[-autoTeam] ""[-b] ""[-badwords <filename>] ""[-ban ip{,ip}*] ""[-banfile <filename>] ""[-c] ""[-cache <url prefix>] ""[-cacheout <filename>] ""[-conf <filename>] ""[-cr] ""[-d] ""[-density <num>] ""[-disableBots] ""[+f {good|<id>}] ""[-f {bad|<id>}] ""[-fb] ""[-filterCallsigns] ""[-filterChat] ""[-filterSimple] ""[-g] ""[-gndtex <texture name>] ""[-groupdb <group file>] ""[-h] ""[-handicap] ""[-helpmsg <file> <name>] ""[-i interface] ""[-j] ""[-lagdrop <num>] ""[-lagwarn <time/ms>] ""[-jitterdrop <num>] ""[-jitterwarn <time/ms>] ""[-loadplugin <pluginname,commandline>] ""[-masterBanURL <URL>]""[-maxidle <time/s>] ""[-mp {<count>|[<count>][,<count>][,<count>][,<count>][,<count>][,<count>]}] ""[-mps <score>] ""[-ms <shots>] ""[-mts <score>] ""[-noMasterBanlist]""[-noradar]""[-offa] ""[-p <port>] ""[-passwd <password>] ""[-pidfile <filename>] ""[-poll <variable>=<value>]"
#ifdef PRINTSCORE
"[-printscore] "
#endif 
"[-public <server-description>] ""[-publicaddr <server-hostname>[:<server-port>]] ""[-publiclist <list-server-url>] ""[-q] ""[+r] ""[-rabbit [score|killer|random]] ""[-recbuf <Mbytes>] ""[-recbufonly] ""[-recdir <dirname>] ""[-replay] ""[-reportfile <filename>] ""[-reportpipe <filename>] ""[-requireudp] ""[+s <flag-count>] ""[-s <flag-count>] ""[-sa] ""[-sb] ""[-set <name> <value>] ""[-setforced <name> <value>] ""[-sl <id> <num>] ""[-spamtime <time>] ""[-spamwarn <warnAmt>] ""[-speedtol <tolerance>] ""[-srvmsg <text>] ""[-st <time>] ""[-sw <num>] ""[-synctime] ""[-synclocation] ""[-t] ""[-tftimeout <seconds>] ""[-time {<seconds>|endTime}] ""[-timemanual] ""[-tk] ""[-tkkr <percent>] ""[-ts [micros]] ""[-userdb <user permissions file>] ""[-vars <filename>] ""[-version] ""[-world <filename>] ""[-worldsize <world size>] ";
...
{
	std::cout << "BZFlag server " << getAppVersion() << " (protocol " << getProtocolVersion() << ") http://BZFlag.org/\n";
	...
	for( i = 1; i < argc; i++ )
	{
		...
		else if( strcmp( argv[i], "-ms" ) == 0 )
		{
			...
			else if( newMaxShots > MaxShots )
			{
				...
				options.maxShots = uint16_t( newMaxShots );
		}
		...
		else if( strcmp( argv[i], "-time" ) == 0 )
		{
			...
			if( strchr( argv[i], ':' ))
			{
				...
				unsigned int hour = now->tm_hour, min = now->tm_min, sec = now->tm_sec, cmdHour = atoi( endTime[0].c_str()), cmdMin = atoi( endTime[1].c_str()), cmdSec = atoi( endTime[2].c_str());
				unsigned long secsToday = ( hour *3600 ) + ( min *60 ) + sec, secsTill = ( cmdHour *3600 ) + ( cmdMin *60 ) + cmdSec;
				if( secsToday > secsTill )
				//if the requested time has already past
					options.timeLimit = ( float )(( 86400-secsToday ) + secsTill );
				//secs left today + till req. time
				else
...
static int getZoneTeamFlagCount( FlagType *flagType, EntryZones &entryZones, const std::set < FlagType * >  &forbidden )
{
...
static int addZoneTeamFlags( int startIndex, FlagType *flagType, EntryZones &entryZones, const std::set < FlagType * >  &forbidden )
{
	if( forbidden.find( flagType ) != forbidden.end())
	{
		return startIndex;
...
void finalizeParsing( int /*argc*/, char **argv, CmdLineOptions &options, EntryZones &entryZones )
{
	...
	if( options.gameStyle &int( RabbitChaseGameStyle ))
	{
		...
		{
			if( options.maxTeam[j] > 0 && options.maxTeam[RogueTeam] != maxRealPlayers )
				std::cout << "only rogues are allowed in Rabbit Style; zeroing out " << Team::getName(( TeamColor )j ) << std::endl;
			options.maxTeam[j] = 0;
		}
	}
	...
	{
		0,  // rogue
		getZoneTeamFlagCount( Flags::RedTeam, entryZones, forbidden ), getZoneTeamFlagCount( Flags::GreenTeam, entryZones, forbidden ), getZoneTeamFlagCount( Flags::BlueTeam, entryZones, forbidden ), getZoneTeamFlagCount( Flags::PurpleTeam, entryZones, forbidden )
	};
	...
	if( options.gameStyle &ClassicCTFGameStyle )
	{
		...
		{
			if(( options.maxTeam[col] > 0 ) && ( options.numTeamFlags[col] <= 0 ) && ( zoneTeamFlagCounts[col] <= 0 ))
			{
