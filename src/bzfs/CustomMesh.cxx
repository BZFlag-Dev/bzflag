...
bool CustomMesh::read( const char *cmd, std::istream &input )
{
	...
	else if( strcasecmp( cmd, "face" ) == 0 )
	{
		...
		face = new CustomMeshFace( material, phydrv, noclusters, smoothBounce, driveThrough, shootThrough );
	}
...
void CustomMesh::writeToGroupDef( GroupDefinition *groupdef )const
{
	...
	MeshObstacle *mesh = new MeshObstacle( xform, checkTypes, checkPoints, vertices, normals, texcoords, faces.size(), noclusters, smoothBounce, driveThrough || forcePassable, shootThrough || forcePassable );
