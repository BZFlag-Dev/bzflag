...
typedef enum
{
	NOT_IN_BUILDING, IN_BASE, IN_BOX_NOTDRIVETHROUGH, IN_BOX_DRIVETHROUGH, IN_MESH, IN_MESHFACE, IN_PYRAMID, IN_TETRA, IN_TELEPORTER
} InBuildingType;
...
public:
	...
	void addWeapon( const FlagType *type, const float *origin, float direction, float tilt, TeamColor teamColor, float initdelay, const std::vector < float >  &delay, TimeKeeper &sync );
	...
	void addBase( const float pos[3], float r, const float size[3], int color, bool drive = false, bool shoot = false );
...
public:
	 ...
	 */
	InBuildingType inCylinderNoOctree( Obstacle **location, float x, float y, float z, float r, float height )const;
	 ...
	 */
	InBuildingType cylinderInBuilding( const Obstacle **obstacle, float x, float y, float z, float radius, float height = 0.0f )const;
	 ...
	 * Checking is quite raw
	 */
	InBuildingType cylinderInBuilding( const Obstacle **obstacle, const float *pos, float radius, float height = 0.0f )const;

	/** check collision between world object and a Z-axis aligned box.
	 * return value is kind of collision.
	 * location will return a pointer to the world colliding object
	 */
	InBuildingType boxInBuilding( const Obstacle **obstacle, const float *pos, float angle, float width, float breadth, float height )const;
