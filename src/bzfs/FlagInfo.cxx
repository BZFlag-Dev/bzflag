...
void FlagInfo::dropFlag( float pos[3], float landingPos[3], bool vanish )
{
	...
	const float gravity = BZDBCache::gravity;
	const float flagAltitude = BZDB.eval( StateDatabase::BZDB_FLAGALTITUDE );
	const float thrownAltitude = ( flag.type == Flags::Shield ) ? BZDB.eval( StateDatabase::BZDB_SHIELDFLIGHT ) *flagAltitude: flagAltitude;
...
float FlagInfo::getNextDrop( TimeKeeper &tm )
{
	...
	if( numFlagsInAir > 0 )
	{
		for( int i = 0; i < numFlags; i++ )
		{
			...
			if( flag.flag.status != FlagNoExist && flag.flag.status != FlagOnTank && flag.flag.status != FlagOnGround && flag.dropDone - tm < waitTime )
...
{
	sprintf( message, "#%-3d i:%-3s p:%-3d r:%-2d g:%-2d s:%-2d ""p:{%.1f, %.1f, %.1f}", flagIndex, flag.type->flagAbbv, player, required ? 1 : 0, grabs, flag.status, flag.position[0], flag.position[1], flag.position[2] );
}
