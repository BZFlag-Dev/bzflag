...
void CustomArc::writeToGroupDef( GroupDefinition *groupdef )const
{
	...
	if( !boxStyle )
	{
		arc = new ArcObstacle( transform, pos, size, rotation, angle, ratio, texsize, useNormals, divisions, mats, phydrv, smoothBounce, driveThrough, shootThrough );
	}
	else
	{
		const float zAxis[3] = 
		...
		newSize[2] = size[2];
		arc = new ArcObstacle( xform, origin, newSize, ( float )( M_PI *0.25 ), angle, ratio, texsize, useNormals, divisions, mats, phydrv, smoothBounce, driveThrough, shootThrough );
	}
