...
void AccessControlList::idBan( std::string idpat, const char *bannedBy, int period, const char *reason, bool fromMaster )
{
...
void AccessControlList::sendBans( PlayerId id, const char *pattern )
{
	...
	{
		const BanInfo &bi =  *it;
		if( bi.fromMaster && ( glob_match( glob, getBanMaskString( bi.addr )) || glob_match( glob, TextUtils::toupper( bi.reason )) || glob_match( glob, TextUtils::toupper( bi.bannedBy ))))
		{
			sendBan( id,  *it );
		}
	}
...
void AccessControlList::sendHostBans( PlayerId id, const char *pattern )
{
	...
	for( hostBanList_t::iterator it = hostBanList.begin(); it != hostBanList.end(); ++it )
	{
		...
		if( !glob_match( glob, TextUtils::toupper( bi.hostpat )) && !glob_match( glob, TextUtils::toupper( bi.reason )) && !glob_match( glob, TextUtils::toupper( bi.bannedBy )))
		{
			continue;
		}
...
void AccessControlList::sendIdBans( PlayerId id, const char *pattern )
{
	...
	for( idBanList_t::iterator it = idBanList.begin(); it != idBanList.end(); ++it )
	{
		...
		if( !glob_match( glob, TextUtils::toupper( bi.idpat )) && !glob_match( glob, TextUtils::toupper( bi.reason )) && !glob_match( glob, TextUtils::toupper( bi.bannedBy )))
		{
			continue;
		}
...
bool AccessControlList::load()
{
	...
	while( !is.eof())
	{
		...
		if( ipAddress == "host:" )
		{
			hostBan( hostpat, ( bannedBy.size() ? bannedBy.c_str(): NULL ), banEnd, ( reason.size() > 0 ? reason.c_str(): NULL ));
		}
		else if( ipAddress == "bzid:" )
		{
			idBan( bzId, ( bannedBy.size() ? bannedBy.c_str(): NULL ), banEnd, ( reason.size() > 0 ? reason.c_str(): NULL ));
		}
		else
		{
			...
			if( !ban( ipAddress, ( bannedBy.size() ? bannedBy.c_str(): NULL ), banEnd, ( reason.size() > 0 ? reason.c_str(): NULL )))
			{
...
int AccessControlList::merge( const std::string &banData )
{
	...
	while( !is.eof())
	{
		...
		if( ipAddress == "host:" )
		{
			hostBan( hostpat, ( bannedBy.size() ? bannedBy.c_str(): NULL ), banEnd, ( reason.size() > 0 ? reason.c_str(): NULL ), true );
			bansAdded++;
		}
		else if( ipAddress == "bzid:" )
		{
		...
		else
		{
			...
			if( !ban( ipAddress, ( bannedBy.size() ? bannedBy.c_str(): NULL ), banEnd, ( reason.size() > 0 ? reason.c_str(): NULL ), true ))
			{
				DEBUG3( "Banfile: bad ban\n" );
				return bansAdded;
			}
...
void AccessControlList::save()
{
	...
	for( banList_t::const_iterator it = banList.begin(); it != banList.end(); ++it )
	{
		if( !it->fromMaster )
		{
			// don't save stuff from the master list
			// print address
			in_addr mask = it->addr;
			...
			if( it->banEnd.getSeconds() == TimeKeeper::getSunExplodeTime().getSeconds())
			{
				os << "end: 0" << '\n';
			}
			else
			{
				os << "end: " << ( long( it->banEnd.getSeconds() + time( NULL ) - TimeKeeper::getCurrent().getSeconds())) << '\n';
			}
	...
	for( hostBanList_t::const_iterator ith = hostBanList.begin(); ith != hostBanList.end(); ++ith )
	{
		...
		// print ban end, banner, and reason
		if( ith->banEnd.getSeconds() == TimeKeeper::getSunExplodeTime().getSeconds())
		{
			os << "end: 0" << '\n';
		}
		else
		{
			os << "end: " << ( long( ith->banEnd.getSeconds() + time( NULL ) - TimeKeeper::getCurrent().getSeconds())) << '\n';
		}
	...
	for( idBanList_t::const_iterator iti = idBanList.begin(); iti != idBanList.end(); ++iti )
	{
		...
		// print ban end, banner, and reason
		if( iti->banEnd.getSeconds() == TimeKeeper::getSunExplodeTime().getSeconds())
		{
			os << "end: 0" << '\n';
		}
		else
		{
			os << "end: " << ( long( iti->banEnd.getSeconds() + time( NULL ) - TimeKeeper::getCurrent().getSeconds())) << '\n';
		}
...
std::vector < std::pair < std::string, std::string >  > AccessControlList::listMasterBans( void )const
{
	...
	for( iItr = idBanList.begin(); iItr != idBanList.end(); iItr++ )
	{
		if( iItr->fromMaster )
		{
			explain = TextUtils::format( "%s (banned by %s)", iItr->reason.c_str(), iItr->bannedBy.c_str());
			const std::pair < std::string, std::string > baninfo = std::make_pair( iItr->idpat, explain );
			bans.push_back( baninfo );
		}
	}
...
bool AccessControlList::convert( char *ip, in_addr &mask )
{
	...
	mask.s_addr = htonl((( unsigned int )b[0] << 24 ) | (( unsigned int )b[1] << 16 ) | (( unsigned int )b[2] << 8 ) | ( unsigned int )b[3] );
	return true;
}
