	...
	if( oldMsg.length() > 0 && dt < clOptions->msgTimer )
	{
		...
		if( TextUtils::compare_nocase( newMsg, oldMsg ) == 0 )
		{
			...
			if( player.getSpamWarns() > clOptions->spamWarnMax || clOptions->spamWarnMax == 0 )
			{
				...
				DEBUG2( "Kicking player %s [%d] for spamming too much: ""2 messages sent within %fs after %d warnings", player.getCallSign(), t, dt, player.getSpamWarns());
...
bool checkChatGarbage( char *message, GameKeeper::Player *playerData, int t )
{
	...
	if( totalChars > tooLong )
	{
		...
		if( badChars > 5 )
		{
			...
			DEBUG2( "Kicking player %s [%d] for sending a garbage message: %d of %d non-printable chars", player.getCallSign(), t, badChars, totalChars );
