...
struct BanInfo
{
	...
	minutes from now. */
	BanInfo( in_addr &banAddr, const char *_bannedBy = NULL, int period = 0, bool isFromMaster = false )
	{
...
struct HostBanInfo
{
	...
	HostBanInfo( std::string _hostpat, const char *_bannedBy = NULL, int period = 0, bool isFromMaster = false )
	{
		hostpat = _hostpat;
		if( _bannedBy )
			bannedBy = _bannedBy;
...
struct IdBanInfo
{
	...
	@c period minutes from now. */
	IdBanInfo( std::string _idpat, const char *_bannedBy = NULL, int period = 0, bool isFromMaster = false )
	{
		idpat = _idpat;
		if( _bannedBy )
			bannedBy = _bannedBy;
...
public:
	...
	be replaced. */
	void idBan( std::string hostpat, const char *bannedBy, int period = 0, const char *reason = NULL, bool fromMaster = false );
