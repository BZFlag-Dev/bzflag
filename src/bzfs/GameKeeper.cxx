...
GameKeeper::Player *GameKeeper::Player::playerList[PlayerSlot] = 
{
	NULL
};
...
GameKeeper::Player::Player( int _playerIndex, NetHandler *_netHandler, tcpCallback _clientCallback ): player( _playerIndex ), netHandler( _netHandler ), lagInfo( &player ), playerIndex( _playerIndex ), closed( false ), clientCallback( _clientCallback ), needThisHostbanChecked( false ), idFlag(  - 1 )
{
	playerHandler = NULL;
...
GameKeeper::Player::Player( int _playerIndex, bz_ServerSidePlayerHandler *handler ): player( _playerIndex ), netHandler( NULL ), lagInfo( &player ), playerIndex( _playerIndex ), closed( false ), clientCallback( NULL ), needThisHostbanChecked( false ), idFlag(  - 1 )
{
...
int GameKeeper::Player::count()
{
	...
	for( int i = 0; i < PlayerSlot; i++ )
		if(( playerData = playerList[i] ) && !playerData->closed && playerData->player.isPlaying())
			count++;
	return count;
}
...
void GameKeeper::Player::dumpScore()
{
	...
	int p;
	for( p = 0; p < PlayerSlot; p++ )
	if(( playerData = playerList[p] ) && !playerData->closed && playerData->player.isPlaying())
	{
...
int GameKeeper::Player::anointRabbit( int oldRabbit )
{
	...
	if(( playerData = playerList[i] ) && !playerData->closed && playerData->player.canBeRabbit( true ))
	{
...
std::vector < int > GameKeeper::Player::allowed( PlayerAccessInfo::AccessPerm right, int targetPlayer )
{
	...
	if( targetPlayer !=  - 1 )
	{
		if(( playerData = playerList[targetPlayer] ) && !playerData->closed && playerData->accessInfo.hasPerm( right ))
			receivers.push_back( targetPlayer );
	}
	else
	{
		for( int i = 0; i < PlayerSlot; i++ )
			if(( playerData = playerList[i] ) && !playerData->closed && playerData->accessInfo.hasPerm( right ))
...
bool GameKeeper::Player::loadEnterData( uint16_t &rejectCode, char *rejectMsg )
{
...
int GameKeeper::Player::getPlayerIDByName( const std::string &name )
{
	...
	{
		for( int i = 0; i < PlayerSlot; i++ )
			if(( playerData = playerList[i] ) && !playerData->closed && ( TextUtils::compare_nocase( playerData->player.getCallSign(), name ) == 0 ))
				return i;
	}
	return  - 1;
}
...
bool GameKeeper::Player::addShot( int id, int salt, FiringInfo &firingInfo )
{
	...
	if( id < ( int )shotsInfo.size() && shotsInfo[id].present && now < shotsInfo[id].expireTime )
	{
