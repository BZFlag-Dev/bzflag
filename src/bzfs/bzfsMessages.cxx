...
void sendFlagTransferMessage (int toPlayer, int fromPlayer , FlagInfo &flag )
{
  ...
  flag.pack(msg);
  msg->packUByte(toData->effectiveShotType);
...
bool sendGrabFlagMessage (int playerIndex, FlagInfo &flag )
{
  ...
  for (int i = 0; i < curMaxPlayers; i++) {
    GameKeeper::Player* otherData = GameKeeper::Player::getPlayerByIndex(i);
    if (otherData && otherData->playerHandler)
      otherData->playerHandler->grabFlag(playerIndex,flag.getIndex(),flag.flag.type->flagAbbv,(bz_eShotType)playerData->effectiveShotType);
  }
...
void sendPlayerKilledMessage(int victimIndex, BlowedUpReason reason, int id )
{
  ...
  for (int i = 0; i < curMaxPlayers; i++)
  {
    GameKeeper::Player* otherData = GameKeeper::Player::getPlayerByIndex(i);
    if (otherData && otherData->playerHandler && otherData->player.isPlaying())
      otherData->playerHandler->playerKilledMessage(victimIndex,(bz_ePlayerDeathReason)reason,id);
  }
}
