...
{
    return glm::distance(center, shot.StartPosition) <= shot.LastUpdatePosition.x - radius;
}
...
bool ShockwaveLogic::CollideCylinder(Shot& shot, glm::vec3& center, float height, float radius)
{
    ...
    return glm::length(vecToPoint) <= shot.LastUpdatePosition.x - radius;
}

bool ShockwaveLogic::PointInSphere(glm::vec3& point, Shot& shot)
{
    return glm::distance(point, shot.StartPosition) <= shot.LastUpdatePosition.x;
}
}
