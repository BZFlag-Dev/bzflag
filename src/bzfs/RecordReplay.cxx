...
enum RecordType
{
	StraightToFile = 0, BufferedRecord = 1
};
...
static const unsigned int RRpacketHdrSize = PACKET_SIZE_STUFFING + ( 2 *sizeof( u16 )) + ( 3 *sizeof( u32 )) + sizeof( RRtime );
...
static const unsigned int ReplayHeaderSize = HEADER_SIZE_STUFFING + ( sizeof( u32 ) *6 ) + sizeof( RRtime ) + CallSignLen + EmailLen + 8+MessageLen + 64+4+WorldSettingsSize;
...
static RRbuffer ReplayBuf = 
{
	0, 0, NULL, NULL
}; // for replaying
static RRbuffer RecordBuf = 
{
	0, 0, NULL, NULL
}; // for recording
...
static void initPacket( u16 mode, u16 code, int len, const void *data, RRpacket *p ); // copy params into packet
...
bool Record::sendStats( int playerIndex )
{
	...
	if( RecordMode == BufferedRecord )
	{
		...
		snprintf( buffer, MessageLen, "Buffered:  %i bytes / %i packets / %.1f seconds", RecordBuf.byteCount, RecordBuf.packetCount, saveTime );
		sendMessage( ServerPlayer, playerIndex, buffer );
	}
	else
	{
		...
		saveTime = ( float )diff / 1000000.0f;
		snprintf( buffer, MessageLen, "Filename:  %s", RecordFilename.c_str());
		sendMessage( ServerPlayer, playerIndex, buffer );
		snprintf( buffer, MessageLen, "   saved:  %i bytes / %i packets / %.1f seconds", RecordFileBytes, RecordFilePackets, saveTime );
		sendMessage( ServerPlayer, playerIndex, buffer );
	}
...
bool Record::saveFile( int playerIndex, const char *filename )
{
	...
	if( !allowFileRecords )
	{
		sendMessage( ServerPlayer, playerIndex, "This server does not allow recording straight to a file" );
		return false;
	}

	if( badFilename( filename ))
	{
		sendMessage( ServerPlayer, playerIndex, "Files must be with the local directory" );
		return false;
	}
...
bool Record::saveBuffer( int playerIndex, const char *filename, int seconds )
{
	...
	if( badFilename( filename ))
	{
		sendMessage( ServerPlayer, playerIndex, "Files must be within the local directory" );
		return false;
	}
...
bool Replay::loadFile( int playerIndex, const char *filename )
{
	...
	sendMessage( ServerPlayer, playerIndex, buffer );
	snprintf( buffer, MessageLen, "  author:     %s (%s)", header.callSign, header.email );
	sendMessage( ServerPlayer, playerIndex, buffer );
...
static bool parseListOptions( const char *opts, int &sortMode, std::string &pattern )
{
	// defaults
	pattern = "*";
...
bool Replay::sendStats( int playerIndex )
{
	...
	char buffer[MessageLen];
	snprintf( buffer, MessageLen, "Replay File:  %s", ReplayFilename.c_str());
	sendMessage( ServerPlayer, playerIndex, buffer );
	...
	snprintf( buffer, MessageLen, "Replay Date:  %s [%.2f %%]  (%.1f secs / %.1f secs)", ctime( &replayTime ), percent, usedTime, fullTime );
	sendMessage( ServerPlayer, playerIndex, buffer );
...
bool Replay::skip( int playerIndex, int seconds )
{
	...
	char buffer[MessageLen];
	snprintf( buffer, MessageLen, REPLAY_LABEL "skipped %.1f seconds (asked %i)", ( float )diff / 1000000.0f, seconds );
...
bool Replay::sendPackets()
{
		...
		if( p->mode != HiddenPacket )
		{
			...
			for( i = MaxPlayers; i < curMaxPlayers; i++ )
			{
				...
				if( pi->isPlaying())
				{
					...
					// send the packets
					if((( p->mode == StatePacket ) && ( state == ReplayReceiving )) || (( p->mode == RealPacket ) && ( state == ReplayStateful )))
					{
	...
	if( sent && ( ReplayPos->prev != NULL ))
	{
		...
		{
			char buffer[MessageLen];
			sprintf( buffer, "No activity for the next %.1f seconds", ( float )diff / 1000000.0f );
...
static RRpacket *nextFilePacket()
{
	...
	if(( ReplayPos->nextFilePos == 0 ) || ( fseek( ReplayFile, ReplayPos->nextFilePos, SEEK_SET ) < 0 )){}
	else
	{
		p = loadPacket( ReplayFile );
...
static RRpacket *prevFilePacket()
{
	...
	// set the file position
	if(( ReplayPos->prevFilePos <= 0 ) || ( fseek( ReplayFile, ReplayPos->prevFilePos, SEEK_SET ) < 0 )){}
	else
	{
		p = loadPacket( ReplayFile );
...
static bool saveTeamsState()
{
	...
	routePacket( MsgTeamUpdate, ( char* )buf - ( char* )bufStart, bufStart, StatePacket );
	...
	for( flagIndex = 0; flagIndex < numFlags; flagIndex++ )
	{
		...
		if( flag.exist())
		{
			if(( length + sizeof( u16 ) + FlagPLen ) > MaxPacketLen - 2 * sizeof( u16 ))
			{
				...
				routePacket( MsgFlagUpdate, ( char* )buf - ( char* )bufStart, bufStart, StatePacket );
...
static bool saveRabbitState()
{
	if( clOptions->gameStyle &int( RabbitChaseGameStyle ))
	{
		...
		routePacket( MsgNewRabbit, ( char* )buf - ( char* )bufStart, bufStart, StatePacket );
	}
	return true;
}
...
static bool savePlayersState()
{
	...
	for( i = 0; i < curMaxPlayers; i++ )
	{
		...
		if( pi->isPlaying())
		{
			...
			buf = pi->packId( buf );
			routePacket( MsgAddPlayer, ( char* )buf - ( char* )bufStart, bufStart, StatePacket );
	...
	if( infoPtr != ( infoBuf + sizeof( unsigned char )))
	{
		...
		routePacket( MsgPlayerInfo, ( char* )infoPtr - ( char* )infoBuf, infoBuf, StatePacket );
	}
	if( adminPtr != ( adminBuf + sizeof( unsigned char )))
	{
		nboPackUByte( adminBuf, count );
		routePacket( MsgAdminInfo, ( char* )adminPtr - ( char* )adminBuf, adminBuf, HiddenPacket );
	...
	for( i = 0; i < curMaxPlayers; i++ )
	{
		...
		if( pi->isAlive())
		{
			...
			routePacket( MsgAlive, ( char* )buf - ( char* )bufStart, bufStart, StatePacket );
		}
	}
...
static bool makeDirExistMsg( const char *dirname, int playerIndex )
{
	...
	{
		char buffer[MessageLen];
		sendMessage( ServerPlayer, playerIndex, "Could not open or create record directory:" );
...
static bool loadHeader( ReplayHeader *h, FILE *f )
{
	...
	if( replaced )
	{
		sendMessage( ServerPlayer, AllPlayers, "An incompatible recording has been loaded" );
		sendMessage( ServerPlayer, AllPlayers, "Please rejoin or face the consequences (client crashes)" );
	}
...
static bool replaceFlagTypes( ReplayHeader *h )
{
	...
	for( it = FlagType::getFlagMap().begin(); it != FlagType::getFlagMap().end(); ++it )
	{
		...
		if(( type != Flags::Null ) && ( headerFlag[type] ) && !flagIsActive( type ))
		{
...
static bool replaceSettings( ReplayHeader *h )
{
	...
	const int maxPlayersOffset = sizeof( uint16_t ) +  // packet len
...
static bool replaceWorldDatabase( ReplayHeader *h )
{
	if(( h->worldSize != worldDatabaseSize ) || ( memcmp( h->world, worldDatabase, h->worldSize ) != 0 ))
	{
...
static bool flagIsActive( FlagType *type )
{
	...
	if(( clOptions->flagCount[type] > 0 ) || (( clOptions->numExtraFlags > 0 ) && !clOptions->flagDisallowed[type] ))
	{
		return true;
	}
	return false;
...
static RRtime getRRtime()
{
	...
	static RRtime offset = (( RRtime )time( NULL )*( RRtime )1000000 ) - (( RRtime )timeGetTime()*( RRtime )1000 );
