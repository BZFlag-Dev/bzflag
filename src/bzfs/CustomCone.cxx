...
void CustomCone::writeToGroupDef( GroupDefinition *groupdef )const
{
	...
	if( !pyramidStyle )
	{
		cone = new ConeObstacle( transform, pos, size, rotation, angle, texsize, useNormals, divisions, mats, phydrv, smoothBounce, driveThrough, shootThrough );
	}
	else
	{
		const float zAxis[3] = 
		...
		{
			const float flipScale[3] = 
			{
				1.0f, 1.0f,  - 1.0f
			};
			const float flipShift[3] = 
			{
				0.0f, 0.0f,  + size[2]
			};
		...
		cone = new ConeObstacle( xform, origin, newSize, ( float )( M_PI *0.25 ), angle, texsize, useNormals, divisions, mats, phydrv, smoothBounce, driveThrough, shootThrough );
	}
