...
bool CustomGroup::read( const char *cmd, std::istream &input )
{
	...
	else if( strcasecmp( cmd, "matref" ) == 0 )
	{
		...
		{
			std::cout << "couldn't find reference material: " << materialName << std::endl;
		}
		else
		{
	...
	else if( strcasecmp( cmd, "matswap" ) == 0 )
	{
		...
		if( srcMat == NULL )
		{
			std::cout << "couldn't find matswap source: " << srcName << std::endl;
		}
		else if( dstMat == NULL )
		{
			std::cout << "couldn't find matswap destination: " << dstName << std::endl;
		}
		else
		{
