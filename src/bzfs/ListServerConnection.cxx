...
ListServerLink::ListServerLink( std::string listServerURL, std::string publicizedAddress, std::string publicizedTitle, std::string _advertiseGroups )
{
			...
			else if( !strncmp( base, bzIdentifier, strlen( bzIdentifier )))
			{
				...
				else
				{
					...
					for( int i = 0; i < curMaxPlayers; i++ )
					{
						...
						if(( gkp != NULL ) && ( strcasecmp( gkp->player.getCallSign(), nick.c_str()) == 0 ))
						{
			...
			if( authReply )
			{
				...
				if( playerIndex < curMaxPlayers )
				{
					if( registered )
					{
						...
						if( !playerData->accessInfo.hasRealPassword() || playerData->accessInfo.getUserInfo( callsign ).hasGroup( "LOCAL.GLOBAL" ))
						{
...
void ListServerLink::addMe( PingPacket pingInfo, std::string publicizedAddress, std::string publicizedTitle, std::string _advertiseGroups )
{
	std::string msg;
	std::string hdr;
	...
	for( int i = 0; i < curMaxPlayers; i++ )
	{
		...
		if( !playerData )
			continue;
		if(( playerData->_LSAState != GameKeeper::Player::required ) && ( playerData->_LSAState != GameKeeper::Player::requesting ))
			continue;
