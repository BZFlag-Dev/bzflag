...
BZWReader::BZWReader( std::string filename ): cURLManager(), location( filename ), input( NULL )
{
	...
	if(( filename.substr( 0, httpProtocol.size()) == httpProtocol ) || ( filename.substr( 0, ftpProtocol.size()) == ftpProtocol ) || ( filename.substr( 0, fileProtocol.size()) == fileProtocol ))
	{
		setURL( location );
		performWait();
	...
	if(( filename.length() < 4 ) || ( strcasecmp( filename.substr( filename.length() - 4, 4 ).c_str(), ".bzw" ) != 0 ))
	{
...
bool BZWReader::readWorldStream( std::vector < WorldFileObject * >  &wlist, GroupDefinition *groupDef )
{
	...
	while( !input->eof() && !input->fail() && input->good())
	{
		...
		else if( strcasecmp( buffer, "define" ) == 0 )
		{
			...
			{
				errorHandler->warning( std::string( "group definitions can not be nested \"" ) + std::string( buffer ) + std::string( "\" - skipping" ), line );
			}
			else
			{
				readToken( buffer, sizeof( buffer ));
				if( strlen( buffer ) > 0 )
				{
					...
					{
						errorHandler->warning( std::string( "duplicate group definition \"" ) + std::string( buffer ) + std::string( "\" - using newest" ), line );
					}
					groupDef = new GroupDefinition( buffer );
				}
				else
				{
					errorHandler->warning( std::string( "missing group definition name" ), line );
				}
			}
		...
		else if( strcasecmp( buffer, "enddef" ) == 0 )
		{
			if( groupDef == worldDef )
			{
				errorHandler->warning( std::string( "enddef without define - skipping" ), line );
			}
			else
			{
		...
		else if( strcasecmp( buffer, "group" ) == 0 )
		{
			...
			if( strlen( buffer ) <= 0 )
			{
				errorHandler->warning( std::string( "missing group definition reference" ), line );
			}
		...
		else if( strcasecmp( buffer, "include" ) == 0 )
		{
			...
			if( object == NULL )
			{
				...
				else
				{
					...
					errorHandler->fatalError( TextUtils::format( "including \"%s\"", incName.c_str()), line );
					return false;
				}
			}
			else
			{
				errorHandler->warning( TextUtils::format( "including \"%s\" within an obstacle, skipping", incName.c_str()), line );
			}
...
WorldInfo *BZWReader::defineWorldFromFile()
{
	...
	for( i = 0; i < baseList.size(); i++ )
	{
		...
		bases[color].addBase( base->getPosition(), base->getSize(), base->getRotation());
	}
