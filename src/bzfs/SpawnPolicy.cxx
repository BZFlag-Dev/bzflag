...
void SpawnPolicy::getPosition( float pos[3], int playerId, bool onGroundOnly, bool notNearEdges )
{
	...
	if( !BZDB.isTrue( "freeCtfSpawns" ) && playerData->player.shouldRestartAtBase() && ( team >= RedTeam ) && ( team <= PurpleTeam ) && ( bases.find( team ) != bases.end()))
	{
...
bool SpawnPolicy::isFacing( const float *enemyPos, const float enemyAzimuth, const float deviation )const
{
	...
	for( int i = 0; i < curmax; i++ )
	{
		...
		if( playerData->player.isAlive())
		{
			...
			if( playerData->player.getFlag() >= 0 )
			{
				...
				// FIXME: any more?
				if( ftype == Flags::Laser )
				{
					// don't spawn in the line of sight of an L
					if( isFacing( enemyPos, enemyAngle, twentyDegrees ))
					{
						// he's looking within 20 degrees of spawn point
						return true; // eek, don't spawn here
					}
				}
				else if( ftype == Flags::ShockWave )
				{
					// don't spawn next to a SW
					if( distanceFrom( enemyPos ) < safeSWRadius )
					{
						// too close to SW
						return true; // eek, don't spawn here
					}
				}
				else if( ftype == Flags::Steamroller || ftype == Flags::Burrow )
				{
					// don't spawn if you'll squish or be squished
					if( distanceFrom( enemyPos ) < safeSRRadius )
					{
						// too close to SR or BU
						return true; // eek, don't spawn here
					}
				}
			}
			// don't spawn in the line of sight of a normal-shot tank within a certain distance
			if( distanceFrom( enemyPos ) < safeDistance )
			{
				// within danger zone?
				if( isFacing( enemyPos, enemyAngle, twentyDegrees ))
				{
...
float SpawnPolicy::enemyProximityCheck( float &enemyAngle )const
{
	...
	for( int i = 0; i < curmax; i++ )
	{
		...
		if( !playerData )
			continue;
		if( playerData->player.isAlive() && areFoes( playerData->player.getTeam(), team ))
		{
			float *enemyPos = playerData->lastState.pos;
