...
WorldInfo *defineRandomWorld()
{
	...
	for (i = 0; i < CitySize * CitySize; i++) {
		...
		world->addBox(atof(BZDB->get("worldSize").c_str()) * ((float)bzfrand() - 0.5f),
				atof(BZDB->get("worldSize").c_str()) * ((float)bzfrand() - 0.5f),
	...
	for (i = 0; i < CitySize * CitySize; i++) {
		...
		world->addPyramid(atof(BZDB->get("worldSize").c_str()) * ((float)bzfrand() - 0.5f),
				atof(BZDB->get("worldSize").c_str()) * ((float)bzfrand() - 0.5f),
				0.0f, 2.0f * M_PI * (float)bzfrand(),
				PyrBase, PyrBase, h);
	}
	...
	if (useTeleporters) {
		...
		for (i = 0; i < numTeleporters;) {
			const float x = (atof(BZDB->get("worldSize").c_str()) - 4.0f * TeleBreadth) * ((float)bzfrand() - 0.5f);
			const float y = (atof(BZDB->get("worldSize").c_str()) - 4.0f * TeleBreadth) * ((float)bzfrand() - 0.5f);
