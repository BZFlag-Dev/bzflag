...
KickCommand::KickCommand(): ServerCommand( "/kick", "<#slot|PlayerName|\"Player Name\"> <reason> - kick the player off the server" ){}
KillCommand::KillCommand(): ServerCommand( "/kill", "<#slot|PlayerName|\"Player Name\"> [reason] - kill a player" ){}
BanListCommand::BanListCommand(): ServerCommand( "/banlist", "[pattern] - List the IPs currently banned from this server" ){}
CheckIPCommand::CheckIPCommand(): ServerCommand( "/checkip", "<ip> - check if IP is banned and print corresponding ban info" ){}
BanCommand::BanCommand(): ServerCommand( "/ban", "<#slot|PlayerName|\"Player Name\"|ip> <duration> <reason>  - ban a player, ip or ip range off the server" ){}
UnbanCommand::UnbanCommand(): ServerCommand( "/unban", "<ip> - remove a ip pattern from the ban list" ){}
HostbanCommand::HostbanCommand(): ServerCommand( "/hostban", "<host pattern> [duration] [reason] - ban using host pattern off the server" ){}
HostUnbanCommand::HostUnbanCommand(): ServerCommand( "/hostunban", "<host pattern> - remove a host pattern from the host ban list" ){}
HostbanListCommand::HostbanListCommand(): ServerCommand( "/hostbanlist", "[pattern] - List the host patterns currently banned from this server" ){}
IdBanCommand::IdBanCommand(): ServerCommand( "/idban", "<#slot|+id|PlayerName|\"Player Name\"> <duration> <reason> - ban using BZID" ){}
IdUnbanCommand::IdUnbanCommand(): ServerCommand( "/idunban", "<id> - remove a BZID from the ban list" ){}
IdBanListCommand::IdBanListCommand(): ServerCommand( "/idbanlist", "[pattern] - List the BZIDs currently banned from this server" ){}
MuteCommand::MuteCommand(): ServerCommand( "/mute", "<#slot|PlayerName|\"Player Name\"> - remove the ability for a player to communicate with other players" ){}
UnmuteCommand::UnmuteCommand(): ServerCommand( "/unmute", "<#slot|PlayerName|\"Player Name\"> - restore the TALK permission to a previously muted player" ){}
MasterBanCommand::MasterBanCommand(): ServerCommand( "/masterban", "<flush|reload|list> - manage the masterban list" ){}

bool MuteCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
...
bool UnmuteCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::mute ))
	{
...
bool KickCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	...
	if( i >= 0 )
	{
		...
		snprintf( kickmessage, MessageLen, "You were kicked off the server by %s", playerData->player.getCallSign());
...
bool KillCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	...
	if( i >= 0 )
	{
		...
		snprintf( killmessage, MessageLen, "You were killed by %s", playerData->player.getCallSign());
...
bool CheckIPCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
...
bool BanListCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::banlist ))
	{
...
bool HostbanListCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::banlist ))
	{
...
bool IdBanListCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::banlist ))
	{
...
static bool doBanKick( GameKeeper::Player *victim, GameKeeper::Player *banner, const char *reason )
{
	...
	if( victim->accessInfo.isOperator() || !banner->accessInfo.isOperator())
	{
		// make sure this player isn't protected
		if( victim->accessInfo.hasPerm( PlayerAccessInfo::antiban ))
		{
			snprintf( buffer, MessageLen, "%s is protected from being banned (skipped).", victim->player.getCallSign());
	...
	snprintf( buffer, MessageLen, "You were banned from this server by %s", banner->player.getCallSign());
	...
	snprintf( buffer, MessageLen, "%s banned by %s, reason: %s", victim->player.getCallSign(), banner->player.getCallSign(), reason );
...
bool BanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::ban ) && !playerData->accessInfo.hasPerm( PlayerAccessInfo::shortBan ))
	{
	...
	if( argv.size() != 4 )
	{
		...
		sendMessage( ServerPlayer, t, "	<duration> can be 'short' or 'default' for the default"" ban time " );
		sendMessage( ServerPlayer, t, "	or 'forever' or 'max' for infinite bans " );
		sendMessage( ServerPlayer, t, "	or a time in the format <weeks>W<days>D<hours>""H<minutes>M " );
		sendMessage( ServerPlayer, t, "	or just a number of minutes " );
		sendMessage( ServerPlayer, t, "	Please keep in mind that reason is displayed to the ""user." );
		return true;
	}
	...
	if( victim >= 0 )
	{
		...
		if( victimPlayer )
		{
			...
			{
				char buffer[MessageLen];
				snprintf( buffer, MessageLen, "%s is protected from being banned (skipped).", victimPlayer->player.getCallSign());
	...
	if( !TextUtils::parseDuration( argv[2].c_str(), specifiedDuration ))
	{
		...
		sendMessage( ServerPlayer, t, "Duration examples:  30m 1h  1d  1w  and mixing: 1w2d4h ""1w2d1m" );
		return true;
	}
	if( specifiedDuration >= 0 )
	{
		...
		{
			sendMessage( ServerPlayer, t, "You only have SHORTBAN privileges,"" using default ban time" );
		}
		else
		{
	...
	if( victimPlayer )
	{
		...
		{
			clOptions->acl.load();
			clOptions->acl.idBan( bzid, playerData->player.getCallSign(), banEvent.duration, banEvent.reason.c_str());
			clOptions->acl.save();
	...
	if( clOptions->acl.ban( banEvent.ipAddress.c_str(), playerData->player.getCallSign(), banEvent.duration, banEvent.reason.c_str()))
	{
		clOptions->acl.save();
		...
		for( int i = 0; i < curMaxPlayers; i++ )
		{
			...
			if( tmpVictim && !clOptions->acl.validate( tmpVictim->netHandler->getIPAddress()))
			{
	...
	{
		char buffer[MessageLen];
		snprintf( buffer, MessageLen, "Malformed address or invalid Player/Slot: %s", argv[1].c_str());
		sendMessage( ServerPlayer, t, buffer );
	}
...
bool HostbanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::ban ) && !playerData->accessInfo.hasPerm( PlayerAccessInfo::shortBan ))
	{
	...
	clOptions->acl.hostBan( hostBanEvent.hostPattern.c_str(), playerData->player.getCallSign(), hostBanEvent.duration, hostBanEvent.reason.c_str());
	clOptions->acl.save();
...
bool IdBanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::ban ) && !playerData->accessInfo.hasPerm( PlayerAccessInfo::shortBan ))
	{
	...
	if( argv.size() != 4 )
	{
		sendMessage( ServerPlayer, t, "Syntax: /idban <#slot|+id|PlayerName|\"Player Name\"> <duration> <reason>" );
		sendMessage( ServerPlayer, t, "  Please keep in mind that reason is displayed to the user." );
		return true;
	}
	...
	clOptions->acl.idBan( idpat, playerData->player.getCallSign(), durationInt, reason.c_str());
	clOptions->acl.save();
...
bool UnbanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
...
bool HostUnbanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::unban ))
	{
		sendMessage( ServerPlayer, t, "You do not have permission to run the /hostunban command" );
		return true;
	}
...
bool IdUnbanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::unban ))
	{
		sendMessage( ServerPlayer, t, "You do not have permission to run the /idunban command" );
		return true;
	}
 ...
 */
bool MasterBanCommand::operator()( const char *message, GameKeeper::Player *playerData )
{
	int t = playerData->getIndex();
	...
	size_t start = 0;
	while(( start < argument.size()) && ( isspace( argument[start] )))
	{
		start++;
	}
	...
	if( cmd == "reload" )
	{
		if( !playerData->accessInfo.hasPerm( PlayerAccessInfo::ban ) || !playerData->accessInfo.hasPerm( PlayerAccessInfo::unban ))
		{
