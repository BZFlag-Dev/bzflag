...
static Real getSafeFlagRadius(FlagId id)
{
	...
	// of any building.
	Real r = BZDB->eval(StateDatabase::BZDB_TANKRADIUS);
	if (id == ObesityFlag)
		r *= R_(2.0) * BZDB->eval(StateDatabase::BZDB_OBESEFACTOR);
	return r;
}
...
static bool addFlag(int flagIndex)
{
	...
	// compute drop time
	const float flightTime = 2.0f * sqrtf(-2.0f * BZDB->eval(std::string("_flagAltitude")) / BZDB->eval(StateDatabase::BZDB_GRAVITY));
...
static void dropFlag(PlayerId playerId, FlagDropReason reason, float pos[3])
{
	...
	pos[2] += BZDB->eval(StateDatabase::BZDB_TANKHEIGHT);
	...
	pFlagInfo->flag.launchPosition[2] = pos[2] + BZDB->eval(StateDatabase::BZDB_TANKHEIGHT);
	...
	const float thrownAltitude = (pFlagInfo->flag.id == ShieldFlag) ?
								BZDB->eval(std::string("_shieldFlight")) * BZDB->eval(std::string("_flagAltitude")) : BZDB->eval(std::string("_flagAltitude"));
	...
	const float downTime = sqrtf(-2.0f * (maxAltitude -
								pFlagInfo->flag.landingPosition[2]) / BZDB->eval(StateDatabase::BZDB_GRAVITY));
