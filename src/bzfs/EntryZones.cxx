...
bool EntryZones::getClosePoint( const std::string &qual, const float pos[3], float *pt )const
{
	QualifierMap::const_iterator mit = qmap.find( qual );
	if( mit == qmap.end())
		return false;
...
bool EntryZones::getSafetyPoint( const std::string &qualifier, const float *pos, float *pt )const
{
...
void EntryZones::makeSplitLists( int zone, std::vector < FlagType * >  &flags, std::vector < TeamColor >  &teams, std::vector < TeamColor >  &safety )const
{
	flags.clear();
	teams.clear();
	safety.clear();
