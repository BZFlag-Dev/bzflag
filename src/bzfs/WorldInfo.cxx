...
WorldInfo::WorldInfo(): maxHeight( 0.0f ), database( NULL )
{
	size[0] = 400.0f;
	size[1] = 400.0f;
	gravity =  - 9.81f;
...
void WorldInfo::addWeapon( const FlagType *type, const float *origin, float direction, float tilt, TeamColor teamColor, float initdelay, const std::vector < float >  &delay, TimeKeeper &sync )
{
	worldWeapons.add( type, origin, direction, tilt, teamColor, initdelay, delay, sync );
}
...
void WorldInfo::addBase( const float pos[3], float r, const float _size[3], int color, bool /* drive */, bool /* shoot */ )
{
...
bool WorldInfo::inRect( const float *p1, float angle, const float *_size, float x, float y, float r )const
{
	// translate origin
	float pa[2];
	pa[0] = x - p1[0];
...
InBuildingType WorldInfo::inCylinderNoOctree( Obstacle **location, float x, float y, float z, float radius, float height )const
{
	if( height < Epsilon )
	{
		height = Epsilon;
...
InBuildingType WorldInfo::cylinderInBuilding( const Obstacle **location, const float *pos, float radius, float height )const
{
	if( height < Epsilon )
	{
		height = Epsilon;
...
InBuildingType WorldInfo::cylinderInBuilding( const Obstacle **location, float x, float y, float z, float radius, float height )const
{
	const float pos[3] = 
	{
		x, y, z
...
InBuildingType WorldInfo::boxInBuilding( const Obstacle **location, const float *pos, float angle, float width, float breadth, float height )const
{
	if( height < Epsilon )
	{
		height = Epsilon;
	...
	*location = NULL;
	...
	// check everything but walls
	const ObsList *olist = COLLISIONMGR.boxTest( pos, angle, width, breadth, height );
	for( int i = 0; i < olist->count; i++ )
	{
		const Obstacle *obs = olist->list[i];
...
bool WorldInfo::getFlagDropPoint( const FlagInfo *fi, const float *pos, float *pt )const
{
	...
	if( teamFlag )
	{
		const std::string &safetyQual = CustomZone::getFlagSafetyQualifier( team );
	...
	{
		const std::string &idQual = CustomZone::getFlagIdQualifier( fi->getIndex());
		if( entryZones.getClosePoint( idQual, pos, pt ))
		{
			return true;
		}
		const std::string &typeQual = CustomZone::getFlagTypeQualifier( flagType );
...
{
	const std::string &teamQual = CustomZone::getPlayerTeamQualifier(( int )pi->getTeam());
...
int WorldInfo::packDatabase()
{
	...
	databaseSize = DYNCOLORMGR.packSize() + TEXMATRIXMGR.packSize() + MATERIALMGR.packSize() + PHYDRVMGR.packSize() + TRANSFORMMGR.packSize() + OBSTACLEMGR.packSize() + links.packSize() + worldWeapons.packSize() + entryZones.packSize();
