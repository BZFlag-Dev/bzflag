...
void Authentication::init( const char *address, int port, const char *password )
{
	...
	if( !retval && ( retval = krb5_get_init_creds_password( context, &my_creds, client, intPassword, krb5_prompter_posix, NULL, 0, NULL, NULL )))
...
void Authentication::verifyCredential( char *buf, int len )
{
	...
	if(( retval = krb5_get_credentials( context, KRB5_GC_USER_USER, cc, &creds,  &new_creds )))
