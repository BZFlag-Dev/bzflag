...
CursesUI::CursesUI( BZAdminClient &c ): BZAdminUI( c ), menuState( NoMenu ), menu( c ), client( c ), players( c.getPlayers()), me( c.getMyId()), maxHistory( 20 ), currentHistory( 0 ), maxBufferSize( 300 ), scrollOffset( 0 )
{
...
bool CursesUI::checkCommand( std::string &str )
{
	...
	switch( c )
	{
		...
		case 13:
			...
			// scroll main window
		case KEY_NPAGE:
			scrollOffset = ( scrollOffset < unsigned( LINES - 2 ) / 2 ? 0: scrollOffset - ( LINES - 2 ) / 2 );
		...
		case KEY_DOWN:
			...
			// kick target
			case KEY_F( 5 ): if( targetIter != players.end() && targetIter->first != me && targetIter->first <= LastRealPlayer )
			{
				if( targetIter->second.isAdmin )
				{
					outputMessage( "Warning: Kicking Administrator (" + targetIter->second.name + ")!", Red );
				}
				cmd = "/kick \"";
			...
			// ban target
			case KEY_F( 6 ): if( targetIter != players.end() && targetIter->first != me && targetIter->first <= LastRealPlayer )
			{
				if( targetIter->second.ip != "" )
				{
					if( targetIter->second.isAdmin )
					{
						outputMessage( "Warning: Banning Administrator (" + targetIter->second.name + ")!", Red );
					}
					cmd = "/ban ";
...
void CursesUI::updateTargetWin()
{
	...
	tmp = tmp + ( targetIter == players.end() || targetIter->first == me ? "all": targetIter->second.name ) + ":";
...
void CursesUI::initMainMenu( CursesMenu &menu )
{
	...
	menu.clear();
	menu.addItem( new SubmenuCMItem( "Show players",  &CursesUI::initPlayerMenu ));
	//menu.addItem(new SubmenuCMItem("Edit banlist",
	//				  &CursesUI::initBanMenu));
	menu.addItem( new SubmenuCMItem( "Edit server variables",  &CursesUI::initServerVarMenu ));
	menu.addItem( new SubmenuCMItem( "Edit message filter",  &CursesUI::initFilterMenu ));
}
...
void CursesUI::initPlayerMenu( CursesMenu &menu )
{
	...
	menu.addItem( new SubmenuCMItem( "Back to main menu",  &CursesUI::initMainMenu ));
...
void CursesUI::initBanMenu( CursesMenu &menu )
{
	...
	menu.clear();
	menu.addItem( new SubmenuCMItem( "Not implemented - go back",  &CursesUI::initMainMenu ));
...
void CursesUI::initFilterMenu( CursesMenu &menu )
{
	...
	for( iter = menu.client.getMessageTypeMap().begin(); iter != menu.client.getMessageTypeMap().end(); ++iter )
