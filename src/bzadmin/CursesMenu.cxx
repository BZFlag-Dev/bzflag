...
void CursesMenuItem::showItem( WINDOW *menuWin, int line, int col, int width, bool selected )
{
...
SubmenuCMItem::SubmenuCMItem( const std::string &str, MenuCallback callback ): CursesMenuItem( str ), cb( callback ){
...
CallbackCMItem::CallbackCMItem( const std::string &str, MenuCallback callback ): CursesMenuItem( str ), cb( callback ){
...
CommandCMItem::CommandCMItem( const std::string &str, const std::string &cmd, bool update ): CursesMenuItem( str ), command( cmd ), forceUpdate( update ){
...
BoolCMItem::BoolCMItem( std::string name, bool &variable, std::string trueText, std::string falseText ): CursesMenuItem( name ), varRef( variable ), trueTxt( trueText ), falseTxt( falseText ){

}


void BoolCMItem::showItem( WINDOW *menuWin, int line, int col, int width, bool selected )
{
...
FilterCMItem::FilterCMItem( const std::string &msgType, BZAdminClient &c ): messageType( msgType ), client( c ), numMsgType( c.getMessageTypeMap().find( msgType )->second ){

}


void FilterCMItem::showItem( WINDOW *menuWin, int line, int col, int width, bool selected )
{
...
BZDBCMItem::BZDBCMItem( const std::string &variable ): CursesMenuItem( variable ), editing( false ){

}


void BZDBCMItem::showItem( WINDOW *menuWin, int line, int col, int width, bool selected )
{
	/* print the name of the variable to the left of the center and the
...
bool BZDBCMItem::handleKey( int c, std::string &str, CursesMenu & )
{
	...
	switch( c )
	{
		...
		case KEY_DC:
		case 127:
			editString = ( editString.size() > 0 ? editString.substr( 0, editString.size() - 1 ): editString );
			break;
...
PlayerCMItem::PlayerCMItem( const PlayerIdMap &players, PlayerId playerId ): CursesMenuItem(), playerMap( players ), id( playerId ){

}


void PlayerCMItem::showItem( WINDOW *menuWin, int line, int col, int width, bool selected )
{
	...
	if( iter != playerMap.end())
	{
		...
		if( selected )
			wattron( menuWin, A_REVERSE );
		wmove( menuWin, line, col );
		waddstr( menuWin, ( oss.str().substr( 0, width - ip.size() - 1 ) + " " ).c_str());
...
CursesMenu::CursesMenu( BZAdminClient &c ): client( c ), players( c.getPlayers()), dirty( true ){
...
void CursesMenu::showMenu()
{
	...
	start = ( start + ( h - 3 ) > ( signed )items.size() ? items.size() - ( h - 3 ): start );
