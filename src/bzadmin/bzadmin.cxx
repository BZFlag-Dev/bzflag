...
int main( int argc, char **argv )
{
	...
	// startup winsock
	{
		...
		{
			std::cerr << "Invalid WinSock version (got " << ( int )LOBYTE( wsaData.wVersion ) << '.' << ( int )HIBYTE( wsaData.wVersion ) << ", expected" << major << '.' << minor << ')';
			WSACleanup();
			return 1;
		}
	}
	...
	OptionParser op( std::string( "bzadmin " ) + getAppVersion(), "CALLSIGN[:password]@HOST[:PORT] [COMMAND] [COMMAND] ..." );
	...
	op.registerVector( "show", visibleMsgs, "[-show msgtype{,msgtype}*]", "tell bzadmin to show these message types" );
	op.registerVector( "hide", invisibleMsgs, "[-hide msgtype{,msgtype}*]", "tell bzadmin not to show these message types" );
	...
	// check that we have callsign and host in the right format and extract them
	{
		...
		if( !( op.getParameters().size() > 0 && ( atPos = op.getParameters()[0].find( '@' )) > 0 ))
		{
		...
		{
			// callsign:password@host:port on command line
	...
	std::cerr << "Connecting to " << startupInfo.callsign << "@" << startupInfo.serverName << ":" << startupInfo.serverPort;
