...
BZAdminClient::BZAdminClient( BZAdminUI *bzInterface ): myTeam( ObserverTeam ), sLink( Address( startupInfo.serverName ), startupInfo.serverPort ), valid( false ), ui( bzInterface )
{
	...
	if( sLink.getState() != ServerLink::Okay )
	{
		switch( sLink.getState())
		{
			case ServerLink::BadVersion: 
			{
				...
				strncpy( versionError + strlen( versionError ) - 8, sLink.getVersion(), 8 );
			...
			case ServerLink::Rejected: std::cout << "Game is full or over.  Try again later.";
			break;
			case ServerLink::SocketError: std::cout << "Error connecting to server.";
			break;
			case ServerLink::CrippledVersion: std::cout << "Cannot connect to full version server.";
			break;
			default:
	...
	sLink.sendEnter( sLink.getId(), ChatPlayer, myTeam, startupInfo.callsign, "bzadmin", startupInfo.token );
	sLink.flush();
	if( sLink.getState() != ServerLink::Okay )
	{
...
BZAdminClient::ServerCode BZAdminClient::checkMessage()
{
	...
	if( sLink.read( code, len, inbuf, 100 ) == 1 )
	{
		...
		Address a;
		if((( *( messageMask.find( code ))).second ) || ( code == MsgSetVar || code == MsgRemovePlayer || code == MsgAddPlayer || code == MsgAdminInfo || code == MsgPlayerInfo ))
		switch( code )
		{

			case MsgNewRabbit:
				vbuf = nboUnpackUByte( vbuf, p );
				lastMessage.first = std::string( "*** '" ) + players[p].name + "' is now the rabbit.";
				break;
			...
			case MsgPause:
				...
				lastMessage.first = std::string( "*** '" ) + players[p].name + "': " + ( paused ? "paused" : "resumed" ) + ".";
				break;

			case MsgAlive:
				vbuf = nboUnpackUByte( vbuf, p );
				lastMessage.first = std::string( "*** '" ) + players[p].name + "' has respawned.";
				break;
			...
			case MsgSetVar:
				...
				{
					lastMessage.first = std::string( "*** Received BZDB update, " ) + TextUtils::format( "%d", numVars ) + " variable" + ( numVars == 1 ? "" : "s" ) + " updated.";
				}
				break;
			...
			case MsgAddPlayer:
				...
				{
					Team temp;
					std::string joinMsg = std::string( "*** \'" ) + callsign + "\' joined the game as " + temp.getName( players[p].team ) + ".";
			...
			case MsgRemovePlayer:
				...
				{
					lastMessage.first = std::string( "*** '" ) + players[p].name + "' left the game.";
				}
				players.erase( p );
				break;
			...
			case MsgAdminInfo:
				...
				if( numIPs > 1 )
				{
					for( i = 0; i < numIPs; ++i )
					{
						...
						{
							ui->outputMessage( "*** IPINFO: " + players[p].name + " from " + players[p].ip, Default );
						}
					}
				}
				...
				else if( numIPs == 1 )
				{
					...
					{
						std::string joinMsg = std::string( "*** \'" ) + players[p].name + "\' joined the game as " + temp.getName( players[p].team ) + " from " + players[p].ip + ".";
						lastMessage.first = joinMsg;
					}
				}
				break;
			...
			case MsgTimeUpdate:
				...
				if( timeLeft == 0 )
					...
					lastMessage.first = std::string( "*** " ) + TextUtils::format( "%u", timeLeft ) + " seconds remaining.";
				break;
			...
			case MsgKilled:
				...
				{
					lastMessage.first = lastMessage.first + "destroyed by '" + killerName + "'.";
				}
				break;
			...
			case MsgScore:
				...
				{
					lastMessage.first = std::string( "*** Received score update, score for " ) + TextUtils::format( "%d", numScores ) + " player" + ( numScores == 1 ? "s" : "" ) + " updated.";
				}
				break;
			...
			case MsgMessage:
				...
				TeamColor dstTeam = ( dst >= 244 && dst <= 250 ? TeamColor( 250-dst ): NoTeam );
				lastMessage.first = formatMessage(( char* )vbuf, src, dst, dstTeam, me );
				PlayerIdMap::const_iterator iterator = players.find( src );
				lastMessage.second = ( iterator == players.end() ? colorMap[NoTeam]: colorMap[iterator->second.team] );
				break;
		}
		if( ui != NULL )
...
void BZAdminClient::outputServerList()const
{
	...
	if( ui )
	{
		...
		for( std::vector < ServerItem > ::const_iterator server = servers.begin(); server != servers.end(); server++ )
		{
	...
	while( true )
	{
		...
		if( ui != NULL && ui->checkCommand( cmd ))
		{
			...
			else if( cmd.substr( 0, 6 ) == "/show " )
			{
				...
				{
					ui->outputMessage( std::string( "--- ERROR: " ) + cmd.substr( 6 ) + " is an unknown message type", Red );
				}
				else
				{
					showMessageType( cmd.substr( 6 ));
					ui->outputMessage( std::string( "--- Will now show messages of the " ) + "type " + cmd.substr( 6 ), Yellow );
				}
			}
			else if( cmd.substr( 0, 6 ) == "/hide " )
			{
				...
				else
				{
					...
					ui->outputMessage( std::string( "--- Will now hide messages of the " ) + "type " + cmd.substr( 6 ), Yellow );
				}
			}
			else if( cmd == "/list" )
			{
...
void BZAdminClient::sendMessage( const std::string &msg, PlayerId target )
{
	// local commands:
...
std::string BZAdminClient::formatMessage( const std::string &msg, PlayerId src, PlayerId dst, TeamColor dstTeam, PlayerId me )
{
	std::string formatted = "    ";

	// get sender and receiver
	const std::string srcName = ( src == ServerPlayer ? "SERVER" : ( players.count( src ) ? players[src].name: "(UNKNOWN)" ));
	const std::string dstName = ( players.count( dst ) ? players[dst].name: "(UNKNOWN)" );
