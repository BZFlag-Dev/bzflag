...
void GroupInstance::addMaterialSwap( const BzMaterial *srcMat, const BzMaterial *dstMat )
{
...
void *GroupInstance::pack( void *buf )
{
	...
	else
	{
		...
		const unsigned int fakeSize = ( 1+sizeof( int32_t ) + ( matMap.size() *2 * sizeof( int32_t )));
...
void GroupInstance::print( std::ostream &out, const std::string &indent )const
{
	...
	if( modifyColor )
	{
		out << indent << "  tint " << tint[0] << " " << tint[1] << " " << tint[2] << " " << tint[3] << " " << std::endl;
	}
	if( modifyPhysicsDriver )
	{
...
static bool isContainer( int type )
{
	switch( type )
	{
		case GroupDefinition::arcType: case GroupDefinition::coneType: case GroupDefinition::sphereType: case GroupDefinition::tetraType: return true;
		default:
			return false;
	}
}
...
void GroupDefinition::makeGroups( const MeshTransform &xform, const ObstacleModifier &obsMod )const
{
	if( active )
	{
	...
	char groupDefBit = isWorld ? Obstacle::WorldSource: Obstacle::GroupDefSource;
			...
			if( obs->isValid())
			{
				if( !isWorld )
				{
					...
					if( type == meshType )
					{
						...
						if(( diSource != NULL ) && ( !diSource->isServerSide()) && diSource->isValid())
						{
							MaterialSet matSet;
							MaterialMap matMap;
	...
	for( unsigned int i = 0; i < groups.size(); i++ )
	{
		...
		const GroupDefinition *groupDef = OBSTACLEMGR.findGroupDef( group->getGroupDef());
		if( groupDef != NULL )
		{
...
void GroupDefinition::replaceBasesWithBoxes()
{
	...
	for( unsigned int i = 0; i < list.size(); i++ )
	{
		...
		BoxBuilding *box = new BoxBuilding( base->getPosition(), base->getRotation(), baseSize[0], baseSize[1], baseSize[2], base->isDriveThrough(), base->isShootThrough(), false );
		delete base;
		list.remove( i );
		i--;
		addObstacle( box );
...
void GroupDefinition::printGrouped( std::ostream &out, const std::string &indent )const
{
	const bool isWorld = ( this == OBSTACLEMGR.getWorld());
...
void GroupDefinition::printFlatFile( std::ostream &out, const std::string &indent )const
{
...
void GroupDefinitionMgr::print( std::ostream &out, const std::string &indent )const
{
	const bool saveAsOBJ = BZDB.isTrue( "saveAsOBJ" );
