...
ConeObstacle::ConeObstacle( const MeshTransform &xform, const float *_pos, const float *_size, float _rotation, float _sweepAngle, const float _texsize[2], bool _useNormals, int _divisions, const BzMaterial *mats[MaterialCount], int physics, bool bounce, bool drive, bool shoot )
{
...
Obstacle *ConeObstacle::copyWithTransform( const MeshTransform &xform )const
{
	...
	ConeObstacle *copy = new ConeObstacle( tmpXform, pos, size, angle, sweepAngle, texsize, useNormals, divisions, ( const BzMaterial ** )materials, phydrv, smoothBounce, driveThrough, shootThrough );
	return copy;
}
...
MeshObstacle *ConeObstacle::makeMesh()
{
	...
	// validity checking
	if(( sz[0] < minSize ) || ( sz[1] < minSize ) || ( sz[2] < minSize ) || ( fabs( texsize[0] ) < minSize ) || ( fabs( texsize[1] ) < minSize ))
	{
		return NULL;
	}
...
#define TI(x) (divisions - T(x)) // edge, backwards
		...
		if( useNormals )
			push3Ints( nlist, NC( 0 ), NE( 0 ), NE( 1 ));
...
bool ConeObstacle::getHitNormal( const float *, float, const float *, float, float, float, float, float* )const
{
	assert( false );
	return false;
}
...
bool ConeObstacle::inMovingBox( const float *, float, const float *, float, float, float, float )const
{
	assert( false );
	return false;
}
...
bool ConeObstacle::isCrossing( const float * /*p*/, float /*angle*/, float /*dx*/, float /*dy*/, float /*height*/, float * /*_plane*/ )const
{
	assert( false );
	return false;
}
