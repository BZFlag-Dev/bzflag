...
PyramidBuilding::PyramidBuilding( const float *p, float a, float w, float b, float h, bool drive, bool shoot ): Obstacle( p, a, w, b, h, drive, shoot )
{
	finalize();
	return ;
}
...
Obstacle *PyramidBuilding::copyWithTransform( const MeshTransform &xform )const
{
	...
	PyramidBuilding *copy = new PyramidBuilding( newPos, newAngle, newSize[0], newSize[1], newSize[2], driveThrough, shootThrough );
...
{
	return timeRayHitsPyramids( r, getPosition(), getRotation(), getWidth(), getBreadth(), getHeight(), getZFlip());
}
...
void PyramidBuilding::getNormal( const float *p, float *n )const
{
	// get normal in z = const plane
	const float s = shrinkFactor( p[2] );
	getNormalRect( p, getPosition(), getRotation(), s *getWidth(), s *getBreadth(), n );
...
void PyramidBuilding::get3DNormal( const float *p, float *n )const
{
...
bool PyramidBuilding::inCylinder( const float *p, float radius, float height )const
{
	// really rough -- doesn't decrease size with height
	return ( p[2] < ( getPosition()[2] + getHeight())) && (( p[2] + height ) >= getPosition()[2] ) && testRectCircle( getPosition(), getRotation(), getWidth(), getBreadth(), p, radius );
}
...
bool PyramidBuilding::inBox( const float *p, float a, float dx, float dy, float height )const
{
	...
	return testRectRect( getPosition(), getRotation(), s *getWidth(), s *getBreadth(), p, a, dx, dy );
}
...
bool PyramidBuilding::inMovingBox( const float *, float, const float *p, float _angle, float dx, float dy, float dz )const
{
...
bool PyramidBuilding::isCrossing( const float *p, float a, float dx, float dy, float height, float *plane )const
{
	// if not inside or contained then not crossing
	if( !inBox( p, a, dx, dy, height ) || testRectInRect( getPosition(), getRotation(), getWidth(), getBreadth(), p, a, dx, dy ))
		return false;
	if( !plane )
		return true;
...
bool PyramidBuilding::getHitNormal( const float *pos1, float, const float *pos2, float, float, float, float height, float *normal )const
{
	...
	getNormalRect( pos1, getPosition(), getRotation(), s *getWidth(), s *getBreadth(), normal );
...
void PyramidBuilding::getCorner( int index, float *_pos )const
{
...
void PyramidBuilding::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "  size " << getWidth() << " " << getBreadth() << " " << getHeight() << std::endl;
...
void PyramidBuilding::printOBJ( std::ostream &out, const std::string & /*indent*/ )const
{
	...
	{
		{
			0.0f,  - sqrt1_2,  + sqrt1_2
		}

		, 
		{
			 + sqrt1_2, 0.0f,  + sqrt1_2
		}
	...
	float txcds[7][2] = 
	{
		...
		, 
		{
			0.5f *k * s[1], k *sqrtf( s[1] *s[1] + s[2] *s[2] )
		}
	};
	MeshTransform xform;
