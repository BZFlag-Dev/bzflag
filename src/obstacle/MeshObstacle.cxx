...
static void cfvec3ListToArray( const std::vector < cfvec3 >  &list, int &count, fvec3 * &array )
{
	count = list.size();
...
static void arrayToCfvec3List( const fvec3 *array, int count, std::vector < cfvec3 >  &list )
{
	list.clear();
	for( int i = 0; i < count; i++ )
	{
...
MeshObstacle::MeshObstacle( const MeshTransform &transform, const std::vector < char >  &checkTypesL, const std::vector < cfvec3 >  &checkList, const std::vector < cfvec3 >  &verticeList, const std::vector < cfvec3 >  &normalList, const std::vector < cfvec2 >  &texcoordList, int _faceCount, bool _noclusters, bool bounce, bool drive, bool shoot )
{
	unsigned int i;
...
bool MeshObstacle::addFace( const std::vector < int >  &_vertices, const std::vector < int >  &_normals, const std::vector < int >  &_texcoords, const BzMaterial *_material, int phydrv, bool _noclusters, bool bounce, bool drive, bool shoot, bool triangulate )
{
	...
	if(( count < 3 ) || (( _normals.size() > 0 ) && ( _normals.size() != count )) || (( _texcoords.size() > 0 ) && ( _texcoords.size() != count )))
	{
		return false;
	}
	...
	if( triangulate )
	{
		...
		debugLevel = 0;
		face = new MeshFace( this, count, v, n, t, _material, phydrv, _noclusters, bounce, drive, shoot );
	...
	else if( triangulate )
	{
		...
		else
		{
			...
			for( i = 0; i < triSize; i++ )
			{
				...
				face = new MeshFace( this, 3, v, n, t, _material, phydrv, _noclusters, bounce, drive, shoot );
				if( face->isValid())
				{
					faces[faceCount] = face;
					faceCount++;
...
void MeshObstacle::makeFacePointers( const std::vector < int >  &_vertices, const std::vector < int >  &_normals, const std::vector < int >  &_texcoords, float ** &v, float ** &n, float ** &t )
{
...
Obstacle *MeshObstacle::copyWithTransform( const MeshTransform &xform )const
{
	...
	if(( drawInfo != NULL ) && (( faceCount <= 0 ) || ( driveThrough && shootThrough )))
	{
		// load blanks for pure visual meshes
		copy = new MeshObstacle( xform, ctlist, clist, vlist, nlist, tlist, 0, noclusters, smoothBounce, driveThrough, shootThrough );
	}
	else
	{
		...
		copy = new MeshObstacle( xform, ctlist, clist, vlist, nlist, tlist, faceCount, noclusters, smoothBounce, driveThrough, shootThrough );
...
void MeshObstacle::copyFace( int f, MeshObstacle *mesh )const
{
	...
	mesh->addFace( vlist, nlist, tlist, face->getMaterial(), face->getPhysicsDriver(), face->noClusters(), face->isSmoothBounce(), face->isDriveThrough(), face->isShootThrough(), false );
	return ;
}
...
bool MeshObstacle::getHitNormal( const float * /*oldPos*/, float /*oldAngle*/, const float *p, float /*angle*/, float, float, float /*height*/, float *n )const
{
	if( n != NULL )
	{
		getNormal( p, n );
...
bool MeshObstacle::inCylinder( const float *p, float /*radius*/, float height )const
{
	const float mid[3] = 
	{
		p[0], p[1], p[2] + ( 0.5f *height )
	};
...
bool MeshObstacle::inBox( const float *p, float /*angle*/, float /*dx*/, float /*dy*/, float height )const
{
	const float mid[3] = 
	{
		p[0], p[1], p[2] + ( 0.5f *height )
...
bool MeshObstacle::inMovingBox( const float *, float, const float *p, float /*angle*/, float /*dx*/, float /*dy*/, float height )const
{
	const float mid[3] = 
	{
		p[0], p[1], p[2] + ( 0.5f *height )
...
bool MeshObstacle::isCrossing( const float * /*p*/, float /*angle*/, float /*dx*/, float /*dy*/, float /*height*/, float * /*plane*/ )const
{
...
int MeshObstacle::packSize()const
{
	...
	if(( drawInfo != NULL ) && !drawInfo->isCopy())
	{
		...
		if( debugLevel >= 4 )
		{
			DEBUG0( "DrawInfo packSize = %i, align = %i, full = %i\n", drawInfoPackSize, align *(( drawInfoPackSize + ( align - 1 )) / align ), align *(( drawInfoPackSize + ( align - 1 )) / align ) + sizeof( fvec2 ));
		}
	}
	for( int f = 0; f < faceCount; f++ )
	{
			...
			if( rewindLen <= ( int )( texcoordCount *sizeof( fvec2 )))
			{
				...
				for( i = 0; i < faceCount; i++ )
				{
					...
					if( face->useTexcoords())
					{
						...
						{
							face->texcoords[v] = ( float* )(( char* )face->texcoords[v] + ptrDiff );
						}
					}
				}
...
void MeshObstacle::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "# mins = " << extents.mins[0] << " " << extents.mins[1] << " " << extents.mins[2] << std::endl;
	out << indent << "# maxs = " << extents.maxs[0] << " " << extents.maxs[1] << " " << extents.maxs[2] << std::endl;
