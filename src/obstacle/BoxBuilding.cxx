...
BoxBuilding::BoxBuilding( const float *p, float a, float w, float b, float h, bool drive, bool shoot, bool invisible ): Obstacle( p, a, w, b, h, drive, shoot ), noNodes( invisible )
{
	finalize();
	return ;
}
...
Obstacle *BoxBuilding::copyWithTransform( const MeshTransform &xform )const
{
	...
	BoxBuilding *copy = new BoxBuilding( newPos, newAngle, newSize[0], newSize[1], newSize[2], driveThrough, shootThrough, noNodes );
	return copy;
}
...
bool BoxBuilding::inBox( const float *p, float a, float dx, float dy, float height )const
{
	return ( p[2] < ( getPosition()[2] + getHeight())) && (( p[2] + height ) >= getPosition()[2] ) && testRectRect( getPosition(), getRotation(), getWidth(), getBreadth(), p, a, dx, dy );
}
...
bool BoxBuilding::inMovingBox( const float *oldP, float, const float *p, float a, float dx, float dy, float height )const
{
	float higherZ;
	float lowerZ;
	if( oldP[2] > p[2] )
	...
	if(( higherZ + height ) < getPosition()[2] )
		return false;
	return testRectRect( getPosition(), getRotation(), getWidth(), getBreadth(), p, a, dx, dy );
}
...
bool BoxBuilding::isCrossing( const float *p, float a, float dx, float dy, float height, float *plane )const
{
...
bool BoxBuilding::getHitNormal( const float *pos1, float azimuth1, const float *pos2, float azimuth2, float width, float breadth, float, float *normal )const
{
...
void BoxBuilding::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "  position " << myPos[0] << " " << myPos[1] << " " << myPos[2] << std::endl;
