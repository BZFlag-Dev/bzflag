...
Teleporter::Teleporter( const float *p, float a, float w, float b, float h, float _border, bool _horizontal, bool drive, bool shoot ): Obstacle( p, a, w, b, h, drive, shoot ), border( _border ), horizontal( _horizontal )
{
	finalize();
	return ;
}
...
Obstacle *Teleporter::copyWithTransform( const MeshTransform &xform )const
{
	...
	Teleporter *copy = new Teleporter( newPos, newAngle, newSize[0], newSize[1], newSize[2], border, horizontal, driveThrough, shootThrough );
...
void Teleporter::makeLinks()
{
	...
	if( !horizontal )
	{
		...
		backLink = new MeshFace( NULL, 4, bvrts, NULL, btxcds, NULL,  - 1, false, false, true, true );
		...
		frontLink = new MeshFace( NULL, 4, fvrts, NULL, ftxcds, NULL,  - 1, false, false, true, true );
	}
	else
	{
		float xlen = w - br;
...
float Teleporter::intersect( const Ray &r )const
{
	...
	return timeRayHitsBlock( r, getPosition(), getRotation(), getWidth(), getBreadth(), getHeight());
}
...
{
	return ( p[2] + height ) >= getPosition()[2] && p[2] <= getPosition()[2] + getHeight() && testRectCircle( getPosition(), getRotation(), getWidth(), getBreadth(), p, radius );
}
...
bool Teleporter::inBox( const float *p, float a, float dx, float dy, float dz )const
{
	...
	{
		// test crossbar
		if( testRectRect( p, a, dx, dy, getPosition(), getRotation(), getWidth(), getBreadth()))
		{
			return true;
		}
	}
...
bool Teleporter::inMovingBox( const float *oldP, float /*oldAngle */, const float *p, float a, float dx, float dy, float dz )const
{
	float minPos[3];
	minPos[0] = p[0];
	minPos[1] = p[1];
...
bool Teleporter::isCrossing( const float *p, float a, float dx, float dy, float /* dz */, float *plane )const
{
	// if not inside or contained then not crossing
	const float *p2 = getPosition();
	if( !testRectRect( p, a, dx, dy, p2, getRotation(), getWidth(), getBreadth() - getBorder()) || p[2] < p2[2] || p[2] > p2[2] + getHeight() - getBorder())
		return false;
	if( !plane )
		return true;
...
float Teleporter::isTeleported( const Ray &r, int &face )const
{
	...
	const float t = timeRayHitsBlock( r, getPosition(), getRotation(), getWidth(), getBreadth() - getBorder(), getHeight() - getBorder());
	...
	const float x = cosf(  - getRotation())*( p[0] - getPosition()[0] ) - sinf(  - getRotation())*( p[1] - getPosition()[1] );
	face = ( x > 0.0f ) ? 0 : 1;
	return t;
}
...
float Teleporter::getProximity( const float *p, float radius )const
{
	...
	if( !testRectCircle( getPosition(), getRotation(), getWidth(), getBreadth() - getBorder(), p, 1.2f *radius ))
	{
		return 0.0f;
	}
	...
	if( pa[2] <  - 1.2f * radius || pa[2] > getHeight() - getBorder() + 1.2f * radius )
		return 0.0f;
...
bool Teleporter::hasCrossed( const float *p1, const float *p2, int &f )const
{
	...
	const float *p = getPosition();
	if(( p1[2] < p[2] && p2[2] < p[2] ) || ( p1[2] > p[2] + getHeight() - getBorder() && p2[2] > p[2] + getHeight() - getBorder()))
		return false;
...
void Teleporter::getPointWRT( const Teleporter &t2, int face1, int face2, const float *pIn, const float *dIn, float aIn, float *pOut, float *dOut, float *aOut )const
{
	const float x1 = pIn[0] - getPosition()[0];
	const float y1 = pIn[1] - getPosition()[1];
	const float a = t2.getRotation() - getRotation() + ( face1 == face2 ? ( float )M_PI: 0.0f );
	...
	pOut[2] = t2.getPosition()[2] + (( pIn[2] - getPosition()[2] )*( t2.getHeight() - t2.getBorder())) / getHeight();
...
bool Teleporter::getHitNormal( const float *pos1, float azimuth1, const float *pos2, float azimuth2, float width, float breadth, float, float *normal )const
{
	return Obstacle::getHitNormal( pos1, azimuth1, pos2, azimuth2, width, breadth, getPosition(), getRotation(), getWidth(), getBreadth(), getHeight(), normal ) >= 0.0f;
}
...
void Teleporter::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "  position " << _pos[0] << " " << _pos[1] << " " << _pos[2] << std::endl;
	out << indent << "  size " << origSize[0] << " " << origSize[1] << " " << origSize[2] << std::endl;
	out << indent << "  rotation " << (( getRotation() *180.0 ) / M_PI ) << std::endl;
