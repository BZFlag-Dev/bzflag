...
MeshDrawInfo::MeshDrawInfo( const MeshDrawInfo *di, const MeshTransform &xform, const std::map < const BzMaterial *, const BzMaterial * >  &_matMap )
{
...
bool MeshDrawInfo::validate( const MeshObstacle *mesh )const
{
	...
	for( int i = 0; i < cornerCount; i++ )
	{
		...
		if(( v < 0 ) || ( v >= vCount ) || ( n < 0 ) || ( n >= nCount ) || ( t < 0 ) || ( t >= tCount ))
		{
...
bool MeshDrawInfo::serverSetup( const MeshObstacle *mesh )
{
	...
	Extents tmpExts;
	const bool calcCenter = ( sphere[0] ==  + MAXFLOAT ) && ( sphere[1] ==  + MAXFLOAT ) && ( sphere[2] ==  + MAXFLOAT );
	const bool calcRadius = ( sphere[3] ==  + MAXFLOAT );
	const bool calcExtents = ( extents.mins[0] == tmpExts.mins[0] ) && ( extents.mins[1] == tmpExts.mins[1] ) && ( extents.mins[2] == tmpExts.mins[2] ) && ( extents.maxs[0] == tmpExts.maxs[0] ) && ( extents.maxs[1] == tmpExts.maxs[1] ) && ( extents.maxs[2] == tmpExts.maxs[2] );
	...
	for( int lod = 0; lod < lodCount; lod++ )
	{
		...
		{
			DrawSet &drawSet = drawLod.sets[set];
			const bool calcSetCenter = ( drawSet.sphere[0] ==  + MAXFLOAT ) && ( drawSet.sphere[1] ==  + MAXFLOAT ) && ( drawSet.sphere[2] ==  + MAXFLOAT );
		...
		for( int set = 0; set < drawLod.count; set++ )
		{
			...
			for( int cmd = 0; cmd < drawSet.count; cmd++ )
			{
				...
				switch( drawCmd.drawMode )
				{
					...
					case DrawCmd::DrawPoints: tris += points;
					break;
					case DrawCmd::DrawLines: tris += points / 2;
					break;
					case DrawCmd::DrawLineLoop: tris += points;
					break;
					case DrawCmd::DrawLineStrip: tris += points - 1;
					break;
					case DrawCmd::DrawTriangles: tris += points / 3;
					break;
					case DrawCmd::DrawTriangleStrip: tris += points - 2;
					break;
					case DrawCmd::DrawTriangleFan: tris += points - 2;
					break;
					case DrawCmd::DrawQuads: tris += points / 2;
					break;
					case DrawCmd::DrawQuadStrip: tris += points - 2;
					break;
					case DrawCmd::DrawPolygon: tris += points - 2;
					break;
					default:
						break;
				}
...
{
	bool success = true;
	if( !( input >> corner.vertex ) || !( input >> corner.normal ) || !( input >> corner.texcoord ))
	{
		success = false;
		DEBUG0( "Bad corner\n" );
	}
...
static bool parseDrawCmd( std::istream &input, const std::string &mode, DrawCmd &cmd )
{
	// parse the draw mode
...
static bool parseDrawSet( std::istream &input, DrawSet &set )
{
	...
	while( std::getline( input, line ))
	{
		...
		{
			if( !( parms >> set.sphere[0] ) || !( parms >> set.sphere[1] ) || !( parms >> set.sphere[2] ))
			{
				success = false;
				DEBUG0( "Bad center\n" );
			}
		...
		{
			if( !( parms >> set.sphere[0] ) || !( parms >> set.sphere[1] ) || !( parms >> set.sphere[2] ) || !( parms >> set.sphere[3] ))
			{
				success = false;
				DEBUG0( "Bad sphere\n" );
			}
...
static bool parseDrawLod( std::istream &input, DrawLod &lod )
{
	...
	while( std::getline( input, line ))
	{
		...
		else if(( strcasecmp( cmd.c_str(), "length" ) == 0 ) || ( strcasecmp( cmd.c_str(), "lengthPerPixel" ) == 0 ))
		{
...
bool MeshDrawInfo::parse( std::istream &input )
{
	...
	while( std::getline( input, line ))
	{
		...
		{
			if( !( parms >> extents.mins[0] ) || !( parms >> extents.mins[1] ) || !( parms >> extents.mins[2] ) || !( parms >> extents.maxs[0] ) || !( parms >> extents.maxs[1] ) || !( parms >> extents.maxs[2] ))
			{
				success = false;
				DEBUG0( "Bad extents\n" );
			}
		...
		{
			if( !( parms >> sphere[0] ) || !( parms >> sphere[1] ) || !( parms >> sphere[2] ))
			{
				success = false;
				DEBUG0( "Bad center\n" );
			}
		...
		{
			if( !( parms >> sphere[0] ) || !( parms >> sphere[1] ) || !( parms >> sphere[2] ) || !( parms >> sphere[3] ))
			{
				success = false;
				DEBUG0( "Bad sphere\n" );
			}
...
void MeshDrawInfo::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "  extents " << extents.mins[0] << " " << extents.mins[1] << " " << extents.mins[2] << " " << extents.maxs[0] << " " << extents.maxs[1] << " " << extents.maxs[2] << std::endl;
	out << indent << "  sphere " << sphere[0] << " " << sphere[1] << " " << sphere[2] << " " << sphere[3] << std::endl;
	...
	for( i = 0; i < rawVertCount; i++ )
	{
		...
		out << indent << "  vertex " << v[0] << " " << v[1] << " " << v[2] << std::endl;
	}
	// raw normals
	for( i = 0; i < rawNormCount; i++ )
	{
		const fvec3 &n = rawNorms[i];
		out << indent << "  normal " << n[0] << " " << n[1] << " " << n[2] << std::endl;
	}
	// raw texcoords
	for( i = 0; i < rawTxcdCount; i++ )
	{
	...
	for( i = 0; i < cornerCount; i++ )
	{
		...
		out << indent << "  corner " << corner.vertex << " " << corner.normal << " " << corner.texcoord << std::endl;
	}
	...
	for( i = 0; i < lodCount; i++ )
	{
		...
		for( int j = 0; j < lod.count; j++ )
		{
			...
			out << indent << "      sphere " << set.sphere[0] << " " << set.sphere[1] << " " << set.sphere[2] << " " << set.sphere[3] << std::endl;
...
{
	if(( vertex > MaxUShort ) || ( vertex < 0 ) || ( normal > MaxUShort ) || ( normal < 0 ) || ( texcoord > MaxUShort ) || ( texcoord < 0 ))
	{
