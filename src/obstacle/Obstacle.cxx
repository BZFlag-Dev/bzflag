...
Obstacle::Obstacle( const float *_pos, float _angle, float _width, float _breadth, float _height, bool drive, bool shoot )
{
	pos[0] = _pos[0];
	pos[1] = _pos[1];
	pos[2] = _pos[2];
...
bool Obstacle::isCrossing( const float *, float, float, float, float, float* )const
{
...
float Obstacle::getHitNormal( const float *pos1, float azimuth1, const float *pos2, float azimuth2, float width, float breadth, const float *oPos, float oAzimuth, float oWidth, float oBreadth, float oHeight, float *normal )const
{
	...
	for( i = 0; i < 4; i++ )
	{
		...
		d[2] = 0;
		int side;
		const float t = timeAndSideRayHitsRect( Ray( p, d ), oPos, oAzimuth, oWidth, oBreadth, side );
		if( side >= 0 && t <= minTime )
		{
			minTime = t;
			bestSide = side;
	...
	{
		const float _angle = ( float )( 0.5 *M_PI *( float )bestSide + minTime *( azimuth2 - azimuth1 ) + azimuth1 );
		normal[0] =  - cosf( _angle );
		normal[1] =  - sinf( _angle );
		normal[2] = 0.0f;
	}
