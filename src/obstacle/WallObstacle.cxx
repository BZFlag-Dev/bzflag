...
WallObstacle::WallObstacle( const float *p, float a, float b, float h ): Obstacle( p, a, 0.0, b, h )
{
	finalize();
}
...
float WallObstacle::intersect( const Ray &r )const
{
	...
	if( dot == 0.0f )
		return  - 1.0f;
	float t = ( o[0] *plane[0] + o[1] *plane[1] + o[2] *plane[2] + plane[3] ) / dot;
	return t;
}
...
bool WallObstacle::inBox( const float *p, float _angle, float halfWidth, float halfBreadth, float /* height */ )const
{
	...
	if( inCylinder( corner, 0.0f, 0.0f ))
		return true;
	corner[0] = p[0] + xWidth * halfWidth - xBreadth * halfBreadth;
	corner[1] = p[1] + yWidth * halfWidth - yBreadth * halfBreadth;
	if( inCylinder( corner, 0.0f, 0.0f ))
...
bool WallObstacle::inMovingBox( const float * /* oldP */, float /* oldAngle */, const float *p, float _angle, float halfWidth, float halfBreadth, float height )const 
...
bool WallObstacle::getHitNormal( const float *, float, const float *, float, float, float, float, float *normal )const
{
...
void WallObstacle::print( std::ostream & /*out*/, const std::string & /*indent*/ )const
{
	return ;
}
