...
SphereObstacle::SphereObstacle( const MeshTransform &xform, const float *_pos, const float *_size, float _rotation, const float _texsize[2], bool _useNormals, bool _hemisphere, int _divisions, const BzMaterial *mats[MaterialCount], int physics, bool bounce, bool drive, bool shoot )
{
...
Obstacle *SphereObstacle::copyWithTransform( const MeshTransform &xform )const
{
	...
	SphereObstacle *copy = new SphereObstacle( tmpXform, pos, size, angle, texsize, useNormals, hemisphere, divisions, ( const BzMaterial ** )materials, phydrv, smoothBounce, driveThrough, shootThrough );
	return copy;
}
...
MeshObstacle *SphereObstacle::makeMesh()
{
	...
	// validity checking
	if(( divisions < 1 ) || ( texsz[0] < minSize ) || ( texsz[1] < minSize ) || ( sz[0] < minSize ) || ( sz[1] < minSize ) || ( sz[2] < minSize ))
	{
		return NULL;
	}
	...
	for( i = 0; i < divisions; i++ )
	{
		for( j = 0; j < ( 4 *( i + 1 )); j++ )
		{
			...
			float v_angle = ( float )(( M_PI / 2.0 ) *( divisions - i - 1 ) / ( divisions ));
			float unit[3];
	...
	mesh = new MeshObstacle( transform, checkTypes, checkPoints, vertices, normals, texcoords, faceCount, false, smoothBounce, driveThrough, shootThrough );
...
bool SphereObstacle::getHitNormal( const float *, float, const float *, float, float, float, float, float* )const
{
	assert( false );
	return false;
}
...
bool SphereObstacle::inMovingBox( const float *, float, const float *, float, float, float, float )const
{
	assert( false );
	return false;
}
...
bool SphereObstacle::isCrossing( const float * /*p*/, float /*angle*/, float /*dx*/, float /*dy*/, float /*height*/, float * /*_plane*/ )const
{
	assert( false );
	return false;
}
...
void SphereObstacle::print( std::ostream &out, const std::string &indent )const
{
	...
	out << indent << "  texsize " << texsize[0] << " " << texsize[1] << std::endl;
