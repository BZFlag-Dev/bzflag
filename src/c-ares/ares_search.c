...
static void search_callback( void *arg, int status, unsigned char *abuf, int alen )
{
	...
	else
	{
		...
		{
			/* Try the next domain. */
			status = cat_domain( squery->name, channel->domains[squery->next_domain], &s );
			if( status != ARES_SUCCESS )
			...
			else
			{
				squery->trying_as_is = 0;
				squery->next_domain++;
				ares_query( channel, s, squery->dnsclass, squery->type, search_callback, squery );
				free( s );
			} 
		}
		else if( squery->status_as_is ==  - 1 )
		{
			...
			squery->trying_as_is = 1;
			ares_query( channel, squery->name, squery->dnsclass, squery->type, search_callback, squery );
		}
		else
	...
	if( !( channel->flags &ARES_FLAG_NOALIASES ) && !strchr( name, '.' ))
	{
		...
		if( hostaliases )
		{
			...
			if( fp )
			{
				...
				{
					if( strncasecmp( line, name, ( int )len ) != 0 || !isspace(( unsigned char )line[len] ))
						continue;
					p = line + len;
