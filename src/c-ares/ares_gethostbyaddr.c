...
static void addr_callback( void *arg, int status, unsigned char *abuf, int alen )
{
	...
	{
		status = ares_parse_ptr_reply( abuf, alen, &aquery->addr, sizeof( struct in_addr ), AF_INET, &host );
		end_aquery( aquery, status, host );
	} 
...
static int file_lookup( struct in_addr *addr, struct hostent **host )
{
	...
	if( IsNT )
	{
		...
		if( RegOpenKeyEx( HKEY_LOCAL_MACHINE, WIN_NS_NT_KEY, 0, KEY_READ, &hkeyHosts ) == ERROR_SUCCESS )
		{
			DWORD dwLength = MAX_PATH;
			RegQueryValueEx( hkeyHosts, DATABASEPATH, NULL, NULL, tmp,  &dwLength );
