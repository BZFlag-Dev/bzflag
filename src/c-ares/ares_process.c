...
static struct query *end_query( ares_channel channel, struct query *query, int status, unsigned char *abuf, int alen );
...
static void write_tcp_data( ares_channel channel, fd_set *write_fds, time_t now )
{
	...
	for( i = 0; i < channel->nservers; i++ )
	{
		...
		if( !server->qhead || server->tcp_socket == ARES_SOCKET_BAD || !FD_ISSET( server->tcp_socket, write_fds ))
			continue;
...
static void read_tcp_data( ares_channel channel, fd_set *read_fds, time_t now )
{
	...
	for( i = 0; i < channel->nservers; i++ )
	{
		...
		server = &channel->servers[i];
		if( server->tcp_socket == ARES_SOCKET_BAD || !FD_ISSET( server->tcp_socket, read_fds ))
			continue;
		...
		if( server->tcp_lenbuf_pos != 2 )
		{
			 ...
			 */
			count = recv( server->tcp_socket, server->tcp_lenbuf + server->tcp_buffer_pos, 2-server->tcp_buffer_pos, 0 );
			if( count <= 0 )
			{
				handle_error( channel, i, now );
				continue;
		...
		else
		{
			...
			count = recv( server->tcp_socket, server->tcp_buffer + server->tcp_buffer_pos, server->tcp_length - server->tcp_buffer_pos, 0 );
			if( count <= 0 )
			{
				handle_error( channel, i, now );
				continue;
			...
			if( server->tcp_buffer_pos == server->tcp_length )
			{
				 ...
				 */
				process_answer( channel, server->tcp_buffer, server->tcp_length, i, 1, now );
				if( server->tcp_buffer )
...
static void read_udp_packets( ares_channel channel, fd_set *read_fds, time_t now )
{
	...
	for( i = 0; i < channel->nservers; i++ )
	{
		...
		if( server->udp_socket == ARES_SOCKET_BAD || !FD_ISSET( server->udp_socket, read_fds ))
			continue;
...
void ares__send_query( ares_channel channel, struct query *query, time_t now )
{
	...
	else
	{
		...
		query->timeout = now + (( query->atry == 0 ) ? channel->timeout: channel->timeout << query->atry / channel->nservers );
	}
}
...
static int same_questions( const unsigned char *qbuf, int qlen, const unsigned char *abuf, int alen )
{
	...
	for( i = 0; i < q.qdcount; i++ )
	{
		...
		if( ares_expand_name( q.p, qbuf, qlen, &q.name, &q.namelen ) != ARES_SUCCESS )
			return 0;
		q.p += q.namelen;
