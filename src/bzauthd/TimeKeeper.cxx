...
const TimeKeeper&	TimeKeeper::getCurrent(void)
{
  ...
  if (qpcFrequency != 0) {
    ...
    if (!sane) {
      sLog.outLog("Sanity check failure in TimeKeeper::getCurrent()\n");
    }
    sane = false;
    ...
    if (QueryPerformanceFrequency(&freq)) {
      ...
      sLog.outLog("QueryPerformanceFrequency failed with error %d\n", GetLastError());
...
/*const char *TimeKeeper::timestamp(void) // const
{
  ...
  strncpy (buffer, TextUtils_format("%04d-%02d-%02d %02d:%02d:%02d",
...
void TimeKeeper::setProcessorAffinity(int processor)
{
  ...
  if (dwMask < dwProcs) {
    sLog.outLog("Unable to set process affinity mask (specified processor does not exist).\n");
    return;
  }
