...
struct LuaHashString {
	public:
		...
		inline lua_Hash GetHash() const { return hash; }
	...
	public:
		...
		inline void PushInt(lua_State* L, int value) const {
			Push(L);
...
#define HSTR_PUSH_INT(L, name, val) \
	{ static const LuaHashString hsPriv(name); hsPriv.PushInt(L, val); }
