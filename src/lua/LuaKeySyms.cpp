...
bool LuaKeySyms::PushEntries(lua_State* L) {
  ...
  lua_newtable(L); {
    ...
    LuaSetDualPair(L, "CTRL",   EventHandler::CtrlKeyModBit);
    LuaSetDualPair(L, "SHIFT",  EventHandler::ShiftKeyModBit);
    LuaSetDualPair(L, "REPEAT", EventHandler::RepeatKeyModBit);
  }
  lua_rawset(L, -3);

#define PUSH_KEYSYM(X)   LuaSetDualPair(L, #X, -BzfKeyEvent::X)
#define PUSH_BZKEYSYM(X) LuaSetDualPair(L, #X, -BzfKeyEvent::BZ_ ## X)
