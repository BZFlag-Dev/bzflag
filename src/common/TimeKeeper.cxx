...
const TimeKeeper &TimeKeeper::getCurrent( void )
{
	...
	if( lastTime.tv_sec != 0 )
	{
		...
		currentTime += double( now.tv_sec - lastTime.tv_sec ) + 1.0e-6 * double( now.tv_usec - lastTime.tv_usec );
		lastTime = now;
	}
	else
	{
...
const char *TimeKeeper::timestamp( void ) // const
{
	...
	strncpy( buffer, TextUtils::format( "%04d-%02d-%02d %02d:%02d:%02d", now->tm_year, now->tm_mon, now->tm_mday, now->tm_hour, now->tm_min, now->tm_sec ).c_str(), 256 );
	buffer[255] = '\0'; // safety
