...
template <> StateDatabase *Singleton<StateDatabase>::_instance = ( StateDatabase* )0;
...
StateDatabase::Item::Item(): value(), defValue(), isSet( false ), isTrue( false ), save( true ),  // FIXME -- false by default?
...
void StateDatabase::set( const std::string &name, const std::string &value, Permission access )
{
	...
	if( access >= index->second.permission )
	{
		...
		index->second.isTrue = ( index->second.value != "0" && index->second.value != "False" && index->second.value != "false" && index->second.value != "FALSE" && index->second.value != "no" && index->second.value != "No" && index->second.value != "NO" && index->second.value != "disable" );
...
void StateDatabase::setInt( const std::string &name, const int &value, Permission access )
{
...
void StateDatabase::setBool( const std::string &name, const bool &value, Permission access )
{
...
void StateDatabase::setFloat( const std::string &name, const float &value, Permission access )
{
...
void StateDatabase::setPointer( const std::string &name, const void *value, Permission access )
{
	char address[32];
	memset( address, 0, 32 );
...
void StateDatabase::unset( const std::string &name, Permission access )
{
	Map::iterator index = lookup( name );
	if( access >= index->second.permission )
	{
...
void StateDatabase::touch( const std::string &name, Permission access )
{
	Map::iterator index = lookup( name );
	if( access >= index->second.permission )
		notify( index );
...
void StateDatabase::setDefault( const std::string &name, const std::string &value )
{
	Map::iterator index = lookup( name );
	index->second.defValue = value;
}
...
void StateDatabase::setPermission( const std::string &name, Permission permission )
{
	Map::iterator index = lookup( name );
...
void StateDatabase::addCallback( const std::string &name, Callback callback, void *userData )
{
	Map::iterator index = lookup( name );
...
void StateDatabase::removeCallback( const std::string &name, Callback callback, void *userData )
{
	Map::iterator index = lookup( name );
...
bool StateDatabase::isEmpty( const std::string &name )const
{
	debugLookups( name );
	Map::const_iterator index = items.find( name );
	return ( index == items.end() || !index->second.isSet || index->second.value.empty());
}
...
StateDatabase::Permission StateDatabase::getPermission( const std::string &name )const
{
	debugLookups( name );
	Map::const_iterator index = items.find( name );
	if( index != items.end())
...
bool StateDatabase::onCallback( Callback callback, void *userData, void *iterateData )
{
...
std::ostream &operator << ( std::ostream &dst, const StateDatabase::ExpressionToken &src )
{
	...
	{
		case StateDatabase::ExpressionToken::number: dst << src.getNumber();
		break;
		case StateDatabase::ExpressionToken::oper: switch( src.getOperator())
		{
			case StateDatabase::ExpressionToken::add: dst << '+';
			break;
			case StateDatabase::ExpressionToken::subtract: dst << '-';
			break;
			case StateDatabase::ExpressionToken::multiply: dst << '*';
			break;
			case StateDatabase::ExpressionToken::divide: dst << '/';
			break;
			case StateDatabase::ExpressionToken::power: dst << '^';
			break;
			case StateDatabase::ExpressionToken::lparen: dst << '(';
			break;
			case StateDatabase::ExpressionToken::rparen: dst << ')';
			break;
			case StateDatabase::ExpressionToken::none: break;
		}
		break;
		case StateDatabase::ExpressionToken::variable: dst << src.getVariable();
		break;
	}
	return dst;
}
...
float StateDatabase::evaluate( Expression e )const
{
	...
	for( Expression::reverse_iterator i = e.rbegin(); i != e.rend(); i++ )
	{
		...
		{
			case ExpressionToken::number: evaluationStack.push( *i );
			break;
			...
			break;
			case ExpressionToken::oper: if( evaluationStack.size() == 0 )
			{
				// syntax error
			}
			...
			{
				case ExpressionToken::add: tok.setNumber( lvalue.getNumber() + rvalue.getNumber());
				evaluationStack.push( tok );
				break;
				case ExpressionToken::subtract: if( unary )
					tok.setNumber(  - rvalue.getNumber());
				else
					tok.setNumber( lvalue.getNumber() - rvalue.getNumber());
				evaluationStack.push( tok );
				break;
				case ExpressionToken::multiply: tok.setNumber( lvalue.getNumber() *rvalue.getNumber());
				evaluationStack.push( tok );
				break;
				case ExpressionToken::divide: tok.setNumber( lvalue.getNumber() / rvalue.getNumber());
				evaluationStack.push( tok );
				break;
				case ExpressionToken::power: tok.setNumber( pow( lvalue.getNumber(), rvalue.getNumber()));
				evaluationStack.push( tok );
				break;
				default:
