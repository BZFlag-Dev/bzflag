...
void AutoCompleter::unregisterWord( const std::string &str )
{
	...
	while( true )
	{
		std::vector < WordRecord > ::iterator iter = std::lower_bound( words.begin(), words.end(), rec );
...
std::string AutoCompleter::complete( const std::string &str, std::string *matches )
{
	...
	if(( first == words.end()) || ( first->word.substr( 0, tail.size()) != tail ))
	{
		return str; // no match
	}
	std::string tmp = tail;
	...
	const int minLen = first->word.size() < last->word.size() ? first->word.size(): last->word.size();
	int i;
	for( i = 0; i < minLen; ++i )
	{
		if(( !noQuotes && isspace( first->word[i] )) || ( first->word[i] != last->word[i] ))
		{
			break;
		}
	}
