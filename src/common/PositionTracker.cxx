...
PositionTracker::PositionTracker( const PositionTracker &tracker ): _trackedItem( tracker._trackedItem ), _waypointDistance( tracker._waypointDistance )
{
	return ;
}
...
{
	TrackedItemVector &trackSet = _trackedItem[group];
	if(( token >= trackSet.size()) || ( trackSet[token]->id != STR_ID ) || ( trackSet[token]->strID != id ) || ( trackSet[token]->forgotten ))
	{
		return false;
	}
...
{
	TrackedItemVector &trackSet = _trackedItem[group];
	if(( token >= trackSet.size()) || ( trackSet[token]->id != INT_ID ) || ( trackSet[token]->intID != id ) || ( trackSet[token]->forgotten ))
	{
		return false;
	}
...
bool PositionTracker::addWaypoint( const double from[3], const double to[3], double distance )
{
	...
	{
		if(( waypointSet[i]->position[0] = from[0] ) && ( waypointSet[i]->position[1] = from[1] ) && ( waypointSet[i]->position[2] = from[2] ))
		{
			foundPoint = true;
	...
	for( i = 0; i != waypointSet.size(); i++ )
	{
		if(( waypointSet[i]->position[0] = to[0] ) && ( waypointSet[i]->position[1] = to[1] ) && ( waypointSet[i]->position[2] = to[2] ))
		{
			foundPoint = true;
...
double PositionTracker::distanceBetween( unsigned short int fromToken, unsigned short int toToken, std::string fromGroup, std::string toGroup )const
{
	...
	if(( fromToken > fromSet.size()) || ( toToken > toSet.size()) || ( fromToken == toToken ))
	{
		return 0.0;
	}

	// basic Cartesian 3-space formula for distance between two points
	double distanceSquared = ((( fromSet[fromToken]->position[0] - toSet[toToken]->position[0] ) *( fromSet[fromToken]->position[0] - toSet[toToken]->position[0] )) + (( fromSet[fromToken]->position[1] - toSet[toToken]->position[1] ) *( fromSet[fromToken]->position[1] - toSet[toToken]->position[1] )) + (( fromSet[fromToken]->position[2] - toSet[toToken]->position[2] ) *( fromSet[fromToken]->position[2] - toSet[toToken]->position[2] )));
