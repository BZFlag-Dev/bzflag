...
template <> CommandManager *Singleton<CommandManager>::_instance = ( CommandManager* )0;
...
void CommandManager::add( const std::string &name, CommandFunction func, const std::string &help )
{
	commands.erase( name );
	CmdInfo info;
	info.func = func;
...
std::string CommandManager::run( const std::string &name, const ArgList &args, bool *ret )const
{
	// look up command
	Commands::const_iterator index = commands.find( name );
	if( index == commands.end())
...
void CommandManager::iterate( Callback callback, void *userData )const
{
	...
	for( Commands::const_iterator index = commands.begin(); index != commands.end(); ++index )
...
const char *CommandManager::readValue( const char *string, std::string *value )
{
	if( *string == '\"' )
...
const char *CommandManager::readUnquoted( const char *string, std::string *value )
{
...
const char *CommandManager::readQuoted( const char *string, std::string *value )
{
	*value = "";
	bool escaped = false;
	for( ;  *string != '\0'; ++string )
