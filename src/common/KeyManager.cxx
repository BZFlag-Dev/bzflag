...
template <> KeyManager *Singleton<KeyManager>::_instance = ( KeyManager* )0;

const char *KeyManager::buttonNames[] = 
{
	"???", "Pause", "Home", "End", "Left Arrow", "Right Arrow", "Up Arrow", "Down Arrow", "Page Up", "Page Down", "Insert", "Backspace", "Delete", "Kp0", "Kp1", "Kp2", "Kp3", "Kp4", "Kp5", "Kp6", "Kp7", "Kp8", "Kp9", "Kp_Period", "Kp_Divide", "Kp_Multiply", "Kp_Minus", "Kp_Plus", "Kp_Enter", "Kp_Equals", "F1", "F2", "F3", "F4", "F5", "F6", "F7", "F8", "F9", "F10", "F11", "F12", "Help", "Print", "Sysreq", "Break", "Menu", "Power", "Euro", "Undo", "Left Mouse", "Middle Mouse", "Right Mouse", "Wheel Up", "Wheel Down", "Mouse Button 6", "Mouse Button 7", "Mouse Button 8", "Mouse Button 9", "Mouse Button 10", "Joystick Button 1", "Joystick Button 2", "Joystick Button 3", "Joystick Button 4", "Joystick Button 5", "Joystick Button 6", "Joystick Button 7", "Joystick Button 8", "Joystick Button 9", "Joystick Button 10", "Joystick Button 11", "Joystick Button 12", "LastButton" // should always be last item listed
};
...
void KeyManager::unbind( const BzfKeyEvent &key, bool press )
{
	if( press )
		pressEventToCommand.erase( key );
	else
...
std::string KeyManager::get( const BzfKeyEvent &key, bool press )const
{
...
std::string KeyManager::keyEventToString( const BzfKeyEvent &key )const
{
	std::string name;
...
bool KeyManager::stringToKeyEvent( const std::string &name, BzfKeyEvent &key )const
{
	// find last + in name
	...
	// apply shift state
	if( strstr( shiftPart.c_str(), "+Shift+" ) != NULL || strstr( shiftPart.c_str(), "+shift+" ) != NULL )
		key.shift |= BzfKeyEvent::ShiftKey;
	if( strstr( shiftPart.c_str(), "+Ctrl+" ) != NULL || strstr( shiftPart.c_str(), "+ctrl+" ) != NULL )
		key.shift |= BzfKeyEvent::ControlKey;
	if( strstr( shiftPart.c_str(), "+Alt+" ) != NULL || strstr( shiftPart.c_str(), "+alt+" ) != NULL )
...
void KeyManager::iterate( IterateCallback callback, void *userData )
{
...
void KeyManager::addCallback( ChangeCallback callback, void *userData )
{
...
void KeyManager::removeCallback( ChangeCallback callback, void *userData )
{
...
void KeyManager::notify( const BzfKeyEvent &key, bool press, const std::string &cmd )
{
	CallbackInfo info;
...
bool KeyManager::onCallback( ChangeCallback callback, void *userData, void *vinfo )
{
...
bool KeyManager::KeyEventLess::operator()( const BzfKeyEvent &a, const BzfKeyEvent &b )const
{
