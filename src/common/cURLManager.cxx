...
cURLManager::cURLManager()
{
	...
	result = curl_easy_setopt( easyHandle, CURLOPT_WRITEFUNCTION, cURLManager::writeFunction );
	if( result != CURLE_OK )
...
size_t cURLManager::writeFunction( void *ptr, size_t size, size_t nmemb, void *stream )
{
	int len = size * nmemb;
...
void cURLManager::setTimeCondition( timeCondition condition, time_t &t )
{
	...
	{
		case None:
			result = curl_easy_setopt( easyHandle, CURLOPT_TIMECONDITION, CURL_TIMECOND_NONE );
			if( result != CURLE_OK )
			{
			...
			break;
		case ModifiedSince:
			result = curl_easy_setopt( easyHandle, CURLOPT_TIMECONDITION, CURL_TIMECOND_IFMODSINCE );
			if( result != CURLE_OK )
			{
				DEBUG1( "CURLOPT_TIMECONDITION error %d : %s\n", result, errorBuffer );
			}
			result = curl_easy_setopt( easyHandle, CURLOPT_TIMEVALUE, ( long )t );
			if( result != CURLE_OK )
			{
...
void cURLManager::setInterface( const std::string _interfaceIP )
{
	...
	result = curl_easy_setopt( easyHandle, CURLOPT_INTERFACE, interfaceIP.c_str());
	if( result != CURLE_OK )
...
void cURLManager::setUserAgent( const std::string _userAgent )
{
	...
	result = curl_easy_setopt( easyHandle, CURLOPT_USERAGENT, userAgent.c_str());
	if( result != CURLE_OK )
...
{
	CURLFORMcode result;
	result = curl_formadd( &formPost, &formLast, CURLFORM_COPYNAME, key, CURLFORM_COPYCONTENTS, value, CURLFORM_CONTENTSLENGTH, strlen( value ), CURLFORM_END );
...
void cURLManager::setDNSCachingTime( int time )
{
	...
	result = curl_easy_setopt( easyHandle, CURLOPT_DNS_CACHE_TIMEOUT, ( long )time );
	if( result != CURLE_OK )
