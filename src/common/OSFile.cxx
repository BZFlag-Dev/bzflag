...
std::string OSFile::getFileName()
{
	...
	info->title = lastslash == std::string::npos ? info->stdName: info->stdName.substr( lastslash + 1 );
...
{
	std::string::size_type lastslash = info->stdName.rfind( '/' );
	info->osPath = lastslash == std::string::npos ? info->stdName: info->stdName.substr( 0, lastslash );
