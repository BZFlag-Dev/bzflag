...
std::string BzPNG::create(const std::vector<Chunk>& extraChunks,
  ...
  char IHDR[13];
  void* p = IHDR;
  p = nboPackUInt32(p, sx);
  p = nboPackUInt32(p, sy);
  p = nboPackUInt8(p,  8); // bit depth
  p = nboPackUInt8(p,  colorType);
  p = nboPackUInt8(p,  0); // compression
  p = nboPackUInt8(p,  0); // filter
  p = nboPackUInt8(p,  0); // interlace
  ...
  for (uint32_t y = 0; y < sy; y++) {
    d = nboPackUInt8(d, 0); // prepend the scanline filter type, 0 = none
