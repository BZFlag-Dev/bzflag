...
bool WordFilter::aggressiveFilter( char *input )const
{
	...
	for( unsigned int j = 0; j < wordIndices.size(); j++ )
	{
		...
		for( ExpCompareSet::const_iterator i = filters[firstchar].begin(); i != filters[firstchar].end(); ++i )
		{
			...
			while( matched )
			{
				...
				if( regCode == 0 )
				{
					...
					if(( startOffset > 1 ) && ( isalpha( sInput[startOffset - 1] )))
					{
						...
						bool foundit = false;
						for( ExpCompareSet::const_iterator k = prefixes.begin(); k != prefixes.end(); ++k )
						{
					...
					if(( endOffset < inputLength - 1 ) && ( isalpha( sInput[endOffset] )))
					{
						...
						bool foundit = false;
						for( ExpCompareSet::const_iterator k = suffixes.begin(); k != suffixes.end(); ++k )
						{
							...
							if( regexec( k->compiled, sInput.c_str() + endOffset, 1, match, 0 ) == 0 )
							{
								...
								if(( match[0].rm_eo < inputLength - endOffset ) && ( isalpha( sInput[endOffset + match[0].rm_eo] )))
								{
									/* we matched, but we are still in the middle of a word */
									continue;
								}
...
WordFilter::WordFilter( const WordFilter &_filter ): alphabet( _filter.alphabet ), filterChars( _filter.filterChars ), suffixes( _filter.suffixes ), prefixes( _filter.prefixes )
{
...
WordFilter::~WordFilter( void )
{
	...
	{
		for( i = filters[j].begin(); i != filters[j].end(); i++ )
		{
			if( i->compiled )
			{
				regfree( i->compiled );
