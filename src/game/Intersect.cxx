...
void getNormalRect( const float *p1, const float *p2, float angle, float dx, float dy, float *n )
{
	// translate origin
	float pa[2];
	pa[0] = p1[0] - p2[0];
...
bool testRectCircle( const float *p1, float angle, float dx, float dy, const float *p2, float r )
{
	// translate origin
	float pa[2];
	pa[0] = p2[0] - p1[0];
...
static float timeRayHitsOrigBox( const float *p, const float *v, float dx, float dy, float dz )
{
	float tx, ty, tz;
	...
	if( tx < 0.0 || fabsf( p[1] + tx * v[1] ) > dy || p[2] + tx * v[2] < 0.0 || p[2] + tx * v[2] > dz )
		tx =  - 1.0f;
	if( ty < 0.0 || fabsf( p[0] + ty * v[0] ) > dx || p[2] + ty * v[2] < 0.0 || p[2] + ty * v[2] > dz )
		ty =  - 1.0f;
	if( tz < 0.0 || fabsf( p[0] + tz * v[0] ) > dx || fabsf( p[1] + tz * v[1] ) > dy )
		tz =  - 1.0f;
...
float timeRayHitsBlock( const Ray &r, const float *p1, float angle, float dx, float dy, float dz )
{
...
float timeRayHitsPyramids( const Ray &r, const float *p1, float angle, float dx, float dy, float dz, bool flipZ )
{
...
float timeAndSideRayHitsOrigRect( const float *p, const float *v, float dx, float dy, int &side )
{
	// check if inside
...
float timeAndSideRayHitsRect( const Ray &r, const float *p1, float angle, float dx, float dy, int &side )
{
	// get names for ray info
	const float *p2 = r.getOrigin();
	const float *d = r.getDirection();
...
static bool testOrigRectRect( const float *p, float angle, float dx1, float dy1, float dx2, float dy2 )
{
	static const float box[4][2] = 
	{
		{
			1.0, 1.0
		}
	...
	for( i = 0; i < 4; i++ )
	{
		...
		if(( e[1]*( corner2[0] - corner1[i][0] ) - e[0]*( corner2[1] - corner1[i][1] )) *( e[1]*( corner2[0] + corner1[i][0] ) - e[0]*( corner2[1] + corner1[i][1] )) > 0.0 )
			return true;
	}
	return false;
}
...
bool testRectRect( const float *p1, float angle1, float dx1, float dy1, const float *p2, float angle2, float dx2, float dy2 )
{
	// translate origin
	float pa[2];
	pa[0] = p2[0] - p1[0];
...
bool testRectInRect( const float *p1, float angle1, float dx1, float dy1, const float *p2, float angle2, float dx2, float dy2 )
{
	static const float box[4][2] = 
	{
		{
...
static inline void projectAxisBox( const float *dir, const Extents &extents, float *minDist, float *maxDist )
{
	static float i[3];
	static float o[3];
...
static inline void projectPolygon( const float *dir, int count, const float( *points )[3], float *minDist, float *maxDist )
{
	float mind = MAXFLOAT;
	float maxd =  - MAXFLOAT;
...
bool testPolygonInAxisBox( int pointCount, const float( *points )[3], const float *plane, const Extents &extents )
{
	int t;
	...
	const float icross = ( plane[0] *i[0] ) + ( plane[1] *i[1] ) + ( plane[2] *i[2] ) + plane[3];
	const float ocross = ( plane[0] *o[0] ) + ( plane[1] *o[1] ) + ( plane[2] *o[2] ) + plane[3];
	...
	for( t = 0; t < pointCount; t++ )
	{
		...
		for( int a = 0; a < 3; a++ )
		{
			...
			const float length = ( cross[0] *cross[0] ) + ( cross[1] *cross[1] ) + ( cross[2] *cross[2] );
			if( length < 0.001f )
			{
				continue;
			}
...
IntersectLevel testAxisBoxInFrustum( const Extents &extents, const Frustum *frustum )
{
...
IntersectLevel testAxisBoxOcclusion( const Extents &extents, const float( *planes )[4], int planeCount )
{
	static int s, t;
	static float i[3]; // inside point  (assuming partial)
	static float o[3]; // outside point (assuming partial)
...
bool testRayHitsAxisBox( const Ray *ray, const Extents &exts, float *inTime )
{
	int a;
...
bool testRayHitsAxisBox( const Ray *ray, const Extents &extents, float *inTime, float *outTime )
{
