...
void TextureMatrixManager::print( std::ostream &out, const std::string &indent )const
{
	std::vector < TextureMatrix * > ::const_iterator it;
	for( it = matrices.begin(); it != matrices.end(); it++ )
	{
...
void TextureMatrix::finalize()
{
	...
	if(( rotation != 0.0f ) || ( uFixedShift != 0.0f ) || ( vFixedShift != 0.0f ) || ( uFixedScale != 1.0f ) || ( vFixedScale != 1.0f ))
	{
		useStatic = true;
	}

	if(( spinFreq != 0.0f ) || ( uShiftFreq != 0.0f ) || ( vShiftFreq != 0.0f ) || ( uScaleFreq != 0.0f ) || ( vScaleFreq != 0.0f ))
	{
		useDynamic = true;
	}
	...
	if( useStatic )
	{
		...
		shift( staticMatrix,  - ( uFixedShift + uFixedCenter ),  - ( vFixedShift + vFixedCenter ));
...
void TextureMatrix::setDynamicScale( float uFreq, float vFreq, float _uScale, float _vScale )
{
	uScaleFreq = uFreq;
	vScaleFreq = vFreq;
	if( _uScale >= 1.0f )
...
void TextureMatrix::print( std::ostream &out, const std::string &indent )const
{
	...
	if( useDynamic )
	{
		...
		if(( uScaleFreq != 0.0f ) || ( vScaleFreq != 0.0f ) || ( uScale != 1.0f ) || ( vScale != 1.0f ))
		{
			out << indent << "  scale " << uScaleFreq << " " << vScaleFreq << " " << uScale << " " << vScale << std::endl;
		}
