...
void DynamicColor::finalize()
{
	...
	else
	{
		...
		if(( seq.count > 0 ) && ( noAlphaSeqMid && ( p.minValue == 0.0f ) && ( p.maxValue == 1.0f )))
		{
		...
		else
		{
			...
			if(( p.sinusoids.size() == 0 ) && ( p.clampUps.size() == 0 ) && ( p.clampDowns.size() == 0 ) && ( p.sequence.count == 0 ))
			{
...
void DynamicColor::setSequence( int channel, float period, float offset, std::vector < char >  &list )
{
...
void DynamicColor::update( double t )
{
	for( int c = 0; c < 4; c++ )
	{
		...
		color[c] = ( channel.minValue *( 1.0f - factor )) + ( channel.maxValue *factor );
	}
...
void DynamicColor::print( std::ostream &out, const std::string &indent )const
{
	...
	for( int c = 0; c < 4; c++ )
	{
		...
		{
			out << indent << "  " << colorStr << " limits " << p.minValue << " " << p.maxValue << std::endl;
		}
		unsigned int i;
		if( p.sequence.count > 0 )
		{
			out << indent << "  " << colorStr << " sequence " << p.sequence.period << " " << p.sequence.offset;
		...
		for( i = 0; i < p.sinusoids.size(); i++ )
		{
			...
			out << indent << "  " << colorStr << " sinusoid " << f.period << " " << f.offset << " " << f.weight << std::endl;
		}
		for( i = 0; i < p.clampUps.size(); i++ )
		{
			const clampParams &f = p.clampUps[i];
			out << indent << "  " << colorStr << " clampup " << f.period << " " << f.offset << " " << f.width << std::endl;
		}
		for( i = 0; i < p.clampDowns.size(); i++ )
		{
			const clampParams &f = p.clampDowns[i];
			out << indent << "  " << colorStr << " clampdown " << f.period << " " << f.offset << " " << f.width << std::endl;
		}
	}
