...
float getShotVel (ShotType type) {
  ...
  {
  case LaserShot:
    return (BZDB.eval(StateDatabase::BZDB_SHOTSPEED) * BZDB.eval(StateDatabase::BZDB_LASERADVEL));
  case ThiefShot:
    return (BZDB.eval(StateDatabase::BZDB_SHOTSPEED) * BZDB.eval(StateDatabase::BZDB_THIEFVELAD));
  case ShockWaveShot:
  case MachineGunShot:
    return (BZDB.eval(StateDatabase::BZDB_SHOTSPEED) * BZDB.eval(StateDatabase::BZDB_MGUNADVEL));
  case RapidFireShot:
    return (BZDB.eval(StateDatabase::BZDB_SHOTSPEED) * BZDB.eval(StateDatabase::BZDB_RFIREADVEL));
  default:
