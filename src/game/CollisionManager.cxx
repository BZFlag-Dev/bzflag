...
static ObsList EmptyList = 
{
	0, NULL
};
static ColDetNodeList EmptyNodeList = 
{
	0, NULL
};
...
bool CollisionManager::needReload()const
{
	...
	if(( newDepth != maxDepth ) || ( newElements != minElements ) || ( newWorldSize != worldSize ))
	{
		return true;
	}
	else
...
const ObsList *CollisionManager::cylinderTest( const float *pos, float radius, float height )const
{
	if( root == NULL )
	{
		return  &EmptyList;
...
const ObsList *CollisionManager::boxTest( const float *pos, float /*angle*/, float dx, float dy, float dz )const
{
...
const ObsList *CollisionManager::movingBoxTest( const float *oldPos, float /*oldAngle*/, const float *pos, float /*angle*/, float dx, float dy, float dz )const
{
	float newpos[3];
...
const ColDetNodeList *CollisionManager::rayTestNodes( const Ray *ray, float timeLeft )const
{
	if( root == NULL )
	{
...
void CollisionManager::load()
{
	...
	{
		if( !boxes[i]->isPassable())
			fullCount++;
	}
	for( i = 0; i < pyrCount; i++ )
	{
		if( !pyrs[i]->isPassable())
			fullCount++;
	}
	for( i = 0; i < baseCount; i++ )
	{
		if( !bases[i]->isPassable())
			fullCount++;
	}
	for( i = 0; i < teleCount; i++ )
	{
		if( !teles[i]->isPassable())
			fullCount++;
	}
	...
	for( i = 0; i < 3; i++ )
	{
		DEBUG2( "  grid extent[%i] = %f, %f\n", i, gridExtents.mins[i], gridExtents.maxs[i] );
	}
	for( i = 0; i < 3; i++ )
	{
		DEBUG2( "  world extent[%i] = %f, %f\n", i, worldExtents.mins[i], worldExtents.maxs[i] );
	}
...
ColDetNode::ColDetNode( unsigned char _depth, const Extents &exts, ObsList *_list )
{
	int i;
	...
	fullList.list = ( Obstacle ** )realloc( fullList.list, fullList.count *sizeof( Obstacle* ));
...
void ColDetNode::makeChildren()
{
	...
	childCount = 0;
	const float *extentSet[3] = 
	{
		extents.mins, center, extents.maxs
	};
...
void ColDetNode::boxTest( const float *pos, float angle, float dx, float dy, float dz )const
{
	int i;
...
{
	if( !testRayHitsAxisBox( ray, extents, &inTime ) || ( inTime > timeLeft ))
	{
		return ;
	}
...
{
	if( !testRayHitsAxisBox( ray, extents, &inTime, &outTime ) || ( inTime > timeLeft ))
	{
		return ;
	}
