...
void PhysicsDriverManager::print( std::ostream &out, const std::string &indent )const
{
	std::vector < PhysicsDriver * > ::const_iterator it;
	for( it = drivers.begin(); it != drivers.end(); it++ )
	{
...
void PhysicsDriver::print( std::ostream &out, const std::string &indent )const
{
	...
	{
		out << indent << "  linear " << v[0] << " " << v[1] << " " << v[2] << std::endl;
	}

	if( angularVel != 0.0f )
	{
		const float *ap = angularPos;
		out << indent << "  angular " << ( angularVel / ( M_PI *2.0f )) << " " << ap[0] << " " << ap[1] << std::endl;
	}

	if( radialVel != 0.0f )
	{
		const float *rp = radialPos;
		out << indent << "  radial " << radialVel << " " << rp[0] << " " << rp[1] << std::endl;
	}
