...
{
	PACKET_LIST_ENTRY( MsgNull ), PACKET_LIST_ENTRY( MsgAccept ), PACKET_LIST_ENTRY( MsgAlive ), PACKET_LIST_ENTRY( MsgAdminInfo ), PACKET_LIST_ENTRY( MsgAddPlayer ), PACKET_LIST_ENTRY( MsgCaptureFlag ), PACKET_LIST_ENTRY( MsgDropFlag ), PACKET_LIST_ENTRY( MsgEnter ), PACKET_LIST_ENTRY( MsgExit ), PACKET_LIST_ENTRY( MsgFlagUpdate ), PACKET_LIST_ENTRY( MsgGameTime ), PACKET_LIST_ENTRY( MsgGrabFlag ), PACKET_LIST_ENTRY( MsgGMUpdate ), PACKET_LIST_ENTRY( MsgGetWorld ), PACKET_LIST_ENTRY( MsgKilled ), PACKET_LIST_ENTRY( MsgMessage ), PACKET_LIST_ENTRY( MsgNewRabbit ), PACKET_LIST_ENTRY( MsgNegotiateFlags ), PACKET_LIST_ENTRY( MsgPause ), PACKET_LIST_ENTRY( MsgPlayerInfo ), PACKET_LIST_ENTRY( MsgPlayerUpdate ), PACKET_LIST_ENTRY( MsgPlayerUpdateSmall ), PACKET_LIST_ENTRY( MsgQueryGame ), PACKET_LIST_ENTRY( MsgQueryPlayers ), PACKET_LIST_ENTRY( MsgReject ), PACKET_LIST_ENTRY( MsgReplayReset ), PACKET_LIST_ENTRY( MsgRemovePlayer ), PACKET_LIST_ENTRY( MsgShotBegin ), PACKET_LIST_ENTRY( MsgScore ), PACKET_LIST_ENTRY( MsgScoreOver ), PACKET_LIST_ENTRY( MsgShotEnd ), PACKET_LIST_ENTRY( MsgSuperKill ), PACKET_LIST_ENTRY( MsgSetVar ), PACKET_LIST_ENTRY( MsgTimeUpdate ), PACKET_LIST_ENTRY( MsgTeleport ), PACKET_LIST_ENTRY( MsgTransferFlag ), PACKET_LIST_ENTRY( MsgTeamUpdate ), PACKET_LIST_ENTRY( MsgWantWHash ), PACKET_LIST_ENTRY( MsgUDPLinkRequest ), PACKET_LIST_ENTRY( MsgUDPLinkEstablished ), PACKET_LIST_ENTRY( MsgServerControl ), PACKET_LIST_ENTRY( MsgLagPing )
};
...
static std::string strVector( float *vector )
{
	std::string str = TextUtils::format( "(%8.3f, %8.3f, %8.3f)", vector[0], vector[1], vector[2] );
	return str;
}
...
static MsgStringList handleMsgAlive( PacketInfo *pi )
{
	...
	listPush( list, 2, "angle:  %-8.3f = %8.3f deg", azimuth, azimuth *( 180.0f / M_PI ));
...
static MsgStringList handleMsgPlayerUpdate( PacketInfo *pi )
{
	...
	listPush( list, 3, "angle:  %-8.3f = %8.3f deg", state.azimuth, state.azimuth *( 180.0f / M_PI ));
	listPush( list, 3, "angvel: %-8.3f = %8.3f deg/sec", state.angVel, state.angVel *( 180.0f / M_PI ));
