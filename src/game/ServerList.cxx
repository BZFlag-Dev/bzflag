...
ServerList::ServerList(): phase(  - 1 ), serverCache( ServerListCache::get()), pingBcastSocket(  - 1 ){}
...
{
	{
		...
		while( base < endS )
		{
			...
			*scan++ = '\0';
			...
			{
				strncpy( startupInfo->token, ( char* )( base + strlen( tokenIdentifier )), TokenLen );
#ifdef DEBUG
			...
			else if( !strncmp( base, noTokenIdentifier, strlen( noTokenIdentifier )))
			{
			...
			while( *version && isspace( *version ))
				 *version++ = 0;
			infoServer = version;
			while( *infoServer && !isspace( *infoServer ))
				infoServer++;
			while( *infoServer && isspace( *infoServer ))
				 *infoServer++ = 0;
			address = infoServer;
			...
			// check info
			if( strcmp( version, getServerVersion()) == 0 && ( int )strlen( infoServer ) == PingPacketHexPackedSize && port >= 1 && port <= 65535 )
			{
				// store info
				...
				{
					if( dot[0] >= 0 && dot[0] <= 255 && dot[1] >= 0 && dot[1] <= 255 && dot[2] >= 0 && dot[2] <= 255 && dot[3] >= 0 && dot[3] <= 255 )
					{
						InAddr addr;
...
void ServerList::addToList( ServerItem &info, bool doCache )
{
	...
	for( i = 0; i < ( int )servers.size(); i++ )
	{
		...
		if( server.ping.serverId.serverHost.s_addr == info.ping.serverId.serverHost.s_addr && server.ping.serverId.port == info.ping.serverId.port )
		{
	...
	if( insertPoint ==  - 1 )
	{
		// no spot to insert it into -- goes on back
		servers.push_back( info );
	}
	else
	{
		// found a spot to insert it into
...
void ServerList::checkEchos( StartupInfo *info )
{
	...
	while( 1 )
	{
		...
		const int nfound = select( fdMax + 1, ( fd_set* ) &read_set, ( fd_set* ) &write_set, 0, &timeout );
		if( nfound <= 0 )
			break;
...
int ServerList::updateFromCache()
{
	...
	for( SRV_STR_MAP::const_iterator iter = serverCache->begin(); iter != serverCache->end(); iter++ )
	{
