...
int NetHandler::udpReceive( char *buffer, struct sockaddr_in *uaddr, NetHandler **netHandler )
{
	...
	*netHandler = NULL;
	...
	{
		udpRead = 0;
		udpLen = recvfrom( udpSocket, udpmsg, MaxPacketLen, 0, ( struct sockaddr* ) &lastUDPRxaddr, &recvlen );
		// Error receiving data (or no data)
		if( udpLen < 0 )
			return  - 1;
		DEBUG4( "uread() len %d from %s:%d on %i\n", udpLen, inet_ntoa( lastUDPRxaddr.sin_addr ), ntohs( lastUDPRxaddr.sin_port ), udpSocket );
	} if(( udpLen - udpRead ) < 4 )
	{
		// No space for header :-( 
		udpLen = 0;
	...
	if( ! *netHandler )
	{
		...
		if( !( *it )->closed )
		{
			DEBUG3( "(%d-%d) %s:%d", ( *it )->udpin, ( *it )->udpout, inet_ntoa(( *it )->uaddr.sin_addr ), ntohs(( *it )->uaddr.sin_port ));
		}
		DEBUG2( "\n" );
		return  - 1;
	}
...
NetHandler::NetHandler( const struct sockaddr_in &clientAddr, int _fd ): fd( _fd ), clientType( clientNone ), tcplen( 0 ), closed( false ), outmsgOffset( 0 ), outmsgSize( 0 ), outmsgCapacity( 0 ), outmsg( NULL ), udpOutputLen( 0 ), udpin( false ), udpout( false ), toBeKicked( false )
{
...
void NetHandler::flushUDP()
{
	if( udpOutputLen )
	{
		sendto( udpSocket, udpOutputBuffer, udpOutputLen, 0, ( struct sockaddr* ) &uaddr, sizeof( uaddr ));
		udpOutputLen = 0;
	}
}
...
void NetHandler::dumpMessageStats()
{
	...
	for( direction = 0; direction <= 1; direction++ )
	{
		...
		for( MessageCountMap::iterator i = msg[direction].begin(); i != msg[direction].end(); i++ )
		{
			DEBUG1( " %c%c:%u(%u)", i->first >> 8, i->first &0xff, i->second.count, i->second.maxSize );
			total += i->second.count;
		}

		DEBUG1( " total:%u(%u) ", total, msgBytes[direction] );
		DEBUG1( "max msgs/bytes per second: %u/%u\n", perSecondMaxMsg[direction], perSecondMaxBytes[direction] );
	}
	fflush( stdout );
}
...
void NetHandler::udpSend( const void *b, size_t l )
{
	...
	{
		sendto( udpSocket, ( const char* )b, ( int )l, 0, ( struct sockaddr* ) &uaddr, sizeof( uaddr ));
	}
	else
	{
		// Buffer new data
...
bool NetHandler::isMyUdpAddrPort( struct sockaddr_in _uaddr, bool checkPort )
{
	if( closed )
		return false;
...
{
	sprintf( list, "%s%s%s%s%s%s", peer.getDotNotation().c_str(), getHostname() ? " (" : "", getHostname() ? getHostname(): "", getHostname() ? ")" : "", udpin ? " udp" : "", udpout ? "+" : "" );
}
...
NetHandler *NetHandler::whoIsAtIP( const std::string &IP )
{
	...
	if(( *it )->closed && !strcmp(( *it )->peer.getDotNotation().c_str(), IP.c_str()))
	{
		player =  *it;
		break;
	}
...
bool NetHandler::reverseDNSDone()
{
	...
	return ( status == AresHandler::Failed ) || ( status == AresHandler::HbASucceeded );
}
