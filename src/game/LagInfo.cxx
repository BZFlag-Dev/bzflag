...
LagInfo::LagInfo( PlayerInfo *_info ): info( _info ), lagavg( 0 ), jitteravg( 0 ), lostavg( 0 ), lagalpha( 1 ), jitteralpha( 1 ), lostalpha( 1 ), lagcount( 0 ), laglastwarn( 0 ), lagwarncount( 0 ), jittercount( 0 ), jitterlastwarn( 0 ), jitterwarncount( 0 ), pingpending( false ), pingseqno( 0 ), pingssent( 0 ), lasttimestamp( 0.0f ){}
...
void LagInfo::getLagStats( char *msg, bool isAdmin )const
{
	...
	int numchars;
	if( isAdmin )
		numchars = sprintf( msg, "[%3d] %-24.24s: %3d", info->getPlayerIndex(), TextUtils::str_trunc_continued( info->getCallSign(), 22 ).c_str(), lag );
	else
		numchars = sprintf( msg, "%-24.24s: %3d", TextUtils::str_trunc_continued( info->getCallSign(), 22 ).c_str(), lag );
...
void LagInfo::updatePingLag( void *buf, bool &warn, bool &kick, bool &jittwarn, bool &jittkick )
{
	uint16_t _pingseqno;
	...
	if( pingseqno == _pingseqno )
	{
		...
		if( !info->isObserver() && ( threshold > 0 ) && lagavg > threshold && lagcount - laglastwarn > 2 *lagwarncount )
		{
		...
		if( !info->isObserver() && ( jitterthreshold > 0 ) && jitteravg > jitterthreshold && jittercount - jitterlastwarn > 2 *jitterwarncount )
		{
