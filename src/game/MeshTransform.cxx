...
static void multiply( float m[4][4], const float n[4][4] )
{
	...
	for( int i = 0; i < 4; i++ )
	{
		for( int j = 0; j < 4; j++ )
		{
			t[i][j] = ( m[0][j] *n[i][0] ) + ( m[1][j] *n[i][1] ) + ( m[2][j] *n[i][2] ) + ( m[3][j] *n[i][3] );
		}
	}
...
{
	// normalize
	const float len = ( normal[0] *normal[0] ) + ( normal[1] *normal[1] ) + ( normal[2] *normal[2] );
	if( len <= 0.0f )
	{
		return ;
	}
...
MeshTransform::Tool::Tool( const MeshTransform &xform )
{
	...
	// setup the polarity
	const float determinant = ( vm[0][0]*(( vm[1][1] *vm[2][2] ) - ( vm[1][2] *vm[2][1] ))) + ( vm[0][1]*(( vm[1][2] *vm[2][0] ) - ( vm[1][0] *vm[2][2] ))) + ( vm[0][2]*(( vm[1][0] *vm[2][1] ) - ( vm[1][1] *vm[2][0] )));
	if( determinant < 0.0f )
	{
		inverted = true;
	}
...
void MeshTransform::Tool::processTransforms( const std::vector < TransformData >  &transforms )
{
...
void MeshTransform::Tool::modifyOldStyle( float pos[3], float size[3], float &angle, bool &flipz )const
{
	if( empty )
	{
		flipz = false;
...
void MeshTransform::printTransforms( std::ostream &out, const std::string &indent )const
{
	for( unsigned int i = 0; i < transforms.size(); i++ )
	{
		const TransformData &transform = transforms[i];
		...
		switch( transform.type )
		{
			case ShiftTransform:
				{
					out << indent << "  shift " << d[0] << " " << d[1] << " " << d[2] << std::endl;
					break;
				}
			case ScaleTransform:
				{
					out << indent << "  scale " << d[0] << " " << d[1] << " " << d[2] << std::endl;
					break;
				}
			case ShearTransform:
				{
					out << indent << "  shear " << d[0] << " " << d[1] << " " << d[2] << std::endl;
					break;
				}
			case SpinTransform:
				{
					const float degrees = ( float )( d[3]*( 180.0 / M_PI ));
					out << indent << "  spin " << degrees << " " << d[0] << " " << d[1] << " " << d[2] << std::endl;
					break;
				}
			case IndexTransform:
				{
