...
static bool checkExt( const std::string &base, const char *ext, std::string &result )
{
	#ifndef _WIN32
...
unsigned char *MediaFile::readImage( std::string filename, int *w, int *h )
{
	...
	std::string name;
	if( !checkExt( filename, "", name ) && !checkExt( filename, ".png", name ) && !checkExt( filename, ".gif", name ) && !checkExt( filename, ".bmp", name ) && !checkExt( filename, ".pcx", name ) && !checkExt( filename, ".tga", name ) && !checkExt( filename, ".xpm", name ) && !checkExt( filename, ".pbm", name ) && !checkExt( filename, ".pgm", name ) && !checkExt( filename, ".ppm", name ) && !checkExt( filename, ".jpg", name ) && !checkExt( filename, ".jpeg", name ) && !checkExt( filename, ".tif", name ) && !checkExt( filename, ".tiff", name ))
	{
...
{
	unsigned char *b = reinterpret_cast < unsigned char * > ( d );
	*d = static_cast < uint32_t > ( b[0] ) + ( static_cast < uint32_t > ( b[1] ) << 8 ) + ( static_cast < uint32_t > ( b[2] ) << 16 ) + ( static_cast < uint32_t > ( b[3] ) << 24 );
	return  *d;
}
...
{
	unsigned char *b = reinterpret_cast < unsigned char * > ( d );
	*d = static_cast < uint32_t > ( b[3] ) + ( static_cast < uint32_t > ( b[2] ) << 8 ) + ( static_cast < uint32_t > ( b[1] ) << 16 ) + ( static_cast < uint32_t > ( b[0] ) << 24 );
	return  *d;
}
