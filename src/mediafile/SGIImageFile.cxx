...
SGIImageFile::SGIImageFile( std::istream *input ): ImageFile( input )
{
	...
	if(( header[2] != 0 && header[2] != 1 ) || ( header[3] < 1 || header[3] > 1 ) || ( dimensions < 1 || dimensions > 3 ) || data32 != 0 )
	{
		// unsupported format
		return ;
	}
	...
	init( static_cast < int > ( depth ), static_cast < int > ( myWidth ), static_cast < int > ( myHeight ));
}
