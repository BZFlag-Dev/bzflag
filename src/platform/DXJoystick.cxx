...
void DXJoystick::initJoystick( const char *joystickName )
{
	...
	HRESULT success = directInput->CreateDeviceEx( thisDevice, IID_IDirectInputDevice7, ( void ** ) &device, NULL );
	...
	success = device->SetCooperativeLevel( WinWindow::getHandle(), DISCL_BACKGROUND | DISCL_EXCLUSIVE );
...
void DXJoystick::getJoy( int &x, int &y )
{
	...
	if( xAxis == "X" )
		...
		x = state.lRz;
	else if( xAxis == "Slider 1" )
		x = state.rglSlider[0];
	else if( xAxis == "Slider 2" )
		x = state.rglSlider[1];
	...
	if( yAxis == "X" )
		...
		y = state.lRz;
	else if( yAxis == "Slider 1" )
		y = state.rglSlider[0];
	else if( yAxis == "Slider 2" )
		y = state.rglSlider[1];
...
void DXJoystick::ffRumble( int count, float delay, float duration, float strong_motor, float weak_motor )
{
	if( !ffHasRumble())
		return ;
...
void DXJoystick::ffDirectionalConstant( int count, float delay, float duration, float x_direction, float y_direction, float strength )
{
	if( !ffHasDirectional())
		return ;
	...
	if( itr == effectDatabase.end())
	{
		...
		LONG dir[2] = 
		{
			( int )( 1000.0f *x_direction ), ( int )( 1000.0f *y_direction )
		};
...
void DXJoystick::ffDirectionalPeriodic( int count, float delay, float duration, float x_direction, float y_direction, float amplitude, float period, PeriodicType type )
{
	if( !ffHasDirectional())
		return ;
	...
	if( itr == effectDatabase.end())
	{
		...
		switch( type )
		{
			...
			break;
			default:
				DXError( "Unknown directional periodic effect type", type );
				return ;
		}
...
void DXJoystick::ffDirectionalResistance( float time, float coefficient, float saturation, ResistanceType type )
{
	if( !ffHasDirectional())
		return ;
	...
	if( itr == effectDatabase.end())
	{
		...
		switch( type )
		{
			case BzfJoystick::FF_Position: guid = GUID_Spring;
			break;
			case BzfJoystick::FF_Velocity: guid = GUID_Damper;
			break;
			case BzfJoystick::FF_Acceleration: guid = GUID_Inertia;
			break;
			default:
				DXError( "Unknown directional resistance effect type", type );
				return ;
		}
		success = device->CreateEffect( guid, &effect, &createdEffect, NULL );
...
void DXJoystick::enumerateDevices()
{
	...
	HRESULT success = directInput->EnumDevices( DIDEVTYPE_JOYSTICK,  &deviceEnumCallback, NULL, DIEDFL_ATTACHEDONLY );
