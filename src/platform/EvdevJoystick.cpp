...
void       EvdevJoystick::scanForJoysticks(std::map < std::string,
  ...
  while ((dent = readdir(inputdir))) {
    ...
    if (!fd) {
      ...
      if (fd) { debugf(4, "Opened event device %s as read-only.\n", info.filename.c_str()); }
    }
    else {
      debugf(4, "Opened event device %s as read-write.\n", info.filename.c_str());
    }
    /* no good, can't open it */
    if (!fd) {
      debugf(4, "Can't open event device %s.  Check permissions.\n", info.filename.c_str());
      continue;
    }

    /* Does it look like a joystick? */
    if (!(collectJoystickBits(fd, info) && isJoystick(info))) {
      debugf(4, "Device %s doesn't seem to be a joystick.  Skipping.\n", info.filename.c_str());
      close(fd);
      continue;
    }
