...
unsigned char *BzfMedia::readImage( const std::string &filename, int &width, int &height, int &depth )const
{
...
float *BzfMedia::readSound( const std::string &filename, int &numFrames, int &rate )const
{
	// try mediaDir/filename
	std::string name = makePath( mediaDir, filename );
...
std::string BzfMedia::makePath( const std::string &dir, const std::string &filename )const
{
	if(( dir.length() == 0 ) || filename[0] == '/' )
		return filename;
	std::string path = dir;
	path += "/";
	path += filename;
...
std::string BzfMedia::replaceExtension( const std::string &pathname, const std::string &extension )const
{
	std::string newName;
...
unsigned char *BzfMedia::doReadImage( const std::string &filename, int &dx, int &dy, int & )const
{
...
bool BzfMedia::doReadVerbatim( FILE *file, int dx, int dy, int dz, unsigned char *image )
{
	// zero image data
...
bool BzfMedia::doReadRLE( FILE *file, int dx, int dy, int dz, unsigned char *image )
{
	// zero image data
	memset( image, 0, 4 *dx * dy );
	...
	for( int k = 0; k < dz; ++k )
	{
		...
		for( int j = 0; j < dy; ++j )
		{
			...
			if( fseek( file, startTable[j + k * dy], SEEK_SET ) < 0 || fread( row, length, 1, file ) != 1 )
			{
				delete []startTable;
				delete []lengthTable;
				return false;
...
#else 
float *BzfMedia::doReadSound( const std::string &filename, int &numFrames, int &rate )const
{
