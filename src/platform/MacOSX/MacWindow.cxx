...
EXTERN_C_BEGIN extern WindowRef GetWindowRefFromNativeWindow( void *nativeWindow );
EXTERN_C_END 
...
struct Settings
{
	...
	Settings()
	{
		...
		Switch_Display = true;
		Window_Size = CGSizeMake( CGDisplayPixelsWide( kCGDirectMainDisplay ), CGDisplayPixelsHigh( kCGDirectMainDisplay ));
		Display_Hz = 60;
...
class GLContext
{
	const CGLPixelFormatAttribute *GetPixelFormat( u_int32_t display_id = 0x01, int color = settings.depth, int depth = 16, int stencil = 0 )
	{
		...
		*attrib++ = kCGLPFADisplayMask;
		 *attrib++ = ( CGLPixelFormatAttribute )display_id;
		*attrib++ = kCGLPFAColorSize;
		 *attrib++ = ( CGLPixelFormatAttribute )color;
		*attrib++ = kCGLPFADepthSize;
		 *attrib++ = ( CGLPixelFormatAttribute )depth;
		*attrib++ = kCGLPFAStencilSize;
		 *attrib++ = ( CGLPixelFormatAttribute )stencil;
...
MacWindow::MacWindow( const MacDisplay *display, MacVisual *visual ): BzfWindow( display )
{
	//  GLboolean ok;
	//  int argc = 0;
	//  char **argv = {NULL};
	...
	CGGetDisplayTransferByFormula( kCGDirectMainDisplay,  &settings.redMin, &settings.redMax, &settings.redGamma,  &settings.greenMin, &settings.greenMax, &settings.greenGamma,  &settings.blueMin, &settings.blueMax, &settings.blueGamma );
...
void MacWindow::getPosition( int &x, int &y )
{
	x = 0, y = 0;
}
...
#endif 
	err = CGSetDisplayTransferByFormula( kCGDirectMainDisplay, settings.redMin, settings.redMax, 1.0 / value,  //red
...
float MacWindow::getGamma()const
{
...
bool MacWindow::hasGammaControl()const
{
	return true;
}
