...
MacDisplay::MacDisplay()
{
	...
	setPassthroughSize( CGDisplayPixelsWide( kCGDirectMainDisplay ), CGDisplayPixelsHigh( kCGDirectMainDisplay ));
}
...
#pragma unused (name, videoFormat)
	...
	resInfo[0] = new ResInfo( "default", CGDisplayPixelsWide( kCGDirectMainDisplay ), CGDisplayPixelsHigh( kCGDirectMainDisplay ), 0 );
	//numModes = 1;
	//currentMode = 0;
...
bool MacDisplay::getEvent( BzfEvent &bzf_event )const
{
	...
	status = GetEventParameter( eventRef, ::kEventParamKeyModifiers, ::typeUInt32, NULL, sizeof( ::UInt32 ), NULL,  &eventModifiers );
	...
	if( eventModifiers &shiftKey )
		bzf_event.keyDown.shift = BzfKeyEvent::ShiftKey;
	if( eventModifiers &optionKey )
		bzf_event.keyDown.shift = BzfKeyEvent::AltKey;
	if( eventModifiers &controlKey )
		bzf_event.keyDown.shift = BzfKeyEvent::ControlKey;

	switch( eventClass )
	{
		case ::kEventClassMouse: status = GetEventParameter( eventRef, ::kEventParamWindowMouseLocation, ::typeHIPoint, NULL, sizeof( ::HIPoint ), NULL,  &eventLocation );

		status = GetEventParameter( eventRef, ::kEventParamMouseButton, ::typeMouseButton, NULL, sizeof( ::EventMouseButton ), NULL,  &eventButtons );

		status = GetEventParameter( eventRef, ::kEventParamMouseDelta, ::typeHIPoint, NULL, sizeof( ::HIPoint ), NULL,  &eventDelta );

		// handle the main event type
		switch( eventKind )
		{
			case ::kEventMouseDown: case ::kEventMouseUp: if( eventKind == ::kEventMouseDown )
			{
			...
			switch( eventButtons )
			{
				case ::kEventMouseButtonSecondary: bzf_event.keyDown.button = BzfKeyEvent::RightMouse;
				break;
				case ::kEventMouseButtonTertiary: bzf_event.keyDown.button = BzfKeyEvent::MiddleMouse;
				break;
				default:
				...
				case ::kEventMouseMoved: bzf_event.type = BzfEvent::MouseMove;
				bzf_event.mouseMove.x = static_cast < int > ( eventDelta.x );
				bzf_event.mouseMove.y = static_cast < int > ( eventDelta.y );
				break;

				case ::kEventMouseWheelMoved: break;
			}
			break;

			case ::kEventClassKeyboard: status = GetEventParameter( eventRef, ::kEventParamKeyMacCharCodes, ::typeChar, NULL, sizeof( char ), NULL,  &eventChar );
			status = GetEventParameter( eventRef, ::kEventParamKeyCode, ::typeUInt32, NULL, sizeof( eventKeyCode ), NULL,  &eventKeyCode );
			switch( eventKind )
			{
				case ::kEventRawKeyDown: case ::kEventRawKeyRepeat: bzf_event.type = BzfEvent::KeyDown;
				getKey( bzf_event.keyDown, eventChar, eventKeyCode );
				break;

				case ::kEventRawKeyUp: bzf_event.type = BzfEvent::KeyUp;
				getKey( bzf_event.keyUp, eventChar, eventKeyCode );
				break;
			}
			break;

			case ::kEventClassApplication: switch( eventKind )
			{
				case ::kEventAppQuit: bzf_event.type = BzfEvent::Quit;
				break;
			}
			break;

			case ::kEventClassWindow: status = GetEventParameter( eventRef, ::kEventParamDirectObject, ::typeWindowRef, NULL, sizeof( ::WindowRef ), NULL,  &eventWindow );
			switch( eventKind )
			{
				case ::kEventWindowUpdate: case ::kEventWindowDrawContent: bzf_event.type = BzfEvent::Redraw;
			...
			case ::kEventClassCommand: switch( eventKind ){}
			break;
	}
...
void MacDisplay::getKey( BzfKeyEvent &bzf_key, char char_code, ::UInt32 keycode )const
{
	enum
	{
		...
		kF5KeyCode = 0x60, kF6KeyCode = 0x61, kF7KeyCode = 0x62, kF8KeyCode = 0x64, kF9KeyCode = 0x65, kF10KeyCode = 0x6D, kF11KeyCode = 0x67, kF12KeyCode = 0x6F, kF13KeyCode = 0x69,  // Print Screen
	...
	switch( char_code )
	{
		case kUpArrowCharCode:
			bzf_key.button = BzfKeyEvent::Up;
			break;
		case kDownArrowCharCode:
			bzf_key.button = BzfKeyEvent::Down;
			break;
		case kLeftArrowCharCode:
			bzf_key.button = BzfKeyEvent::Left;
			break;
		case kRightArrowCharCode:
			bzf_key.button = BzfKeyEvent::Right;
			break;
		case kHomeCharCode:
			bzf_key.button = BzfKeyEvent::Home;
			break;
		case kEndCharCode:
			bzf_key.button = BzfKeyEvent::End;
			break;
		case kPageUpCharCode:
			bzf_key.button = BzfKeyEvent::PageUp;
			break;
		case kPageDownCharCode:
			bzf_key.button = BzfKeyEvent::PageDown;
			break;
		case kHelpCharCode:
			bzf_key.button = BzfKeyEvent::Insert;
			break;
		case kDeleteCharCode:
			bzf_key.button = BzfKeyEvent::Delete;
			break;
		case kFunctionKeyCharCode:
			...
			case kF1KeyCode:
				...
				break;
			default:
				fprintf( stderr, "Uknown function key code: 0x%lX\n", keycode );
				break;
			}
			break;
...
char *GetMacOSXDataPath( void )
{
	...
	if(( appBundle = ::CFBundleGetMainBundle()) == NULL || ( resourceURL = ::CFBundleCopyResourcesDirectoryURL( appBundle )) == NULL )
	{
		return NULL;
	}
	if( !::CFURLGetFileSystemRepresentation( resourceURL, true, reinterpret_cast < UInt8 * > ( basePath ), sizeof( basePath )))
	{
		string = NULL;
	}
	else
