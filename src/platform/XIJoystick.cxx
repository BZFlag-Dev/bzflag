...
XIJoystick::XIJoystick(): device( NULL ), devices( NULL ), buttonPressType( 0 ), buttonReleaseType( 0 )
{
...
void XIJoystick::initJoystick( const char *joystickName )
{
	...
	if( device )
	{
		...
		for( ip = device->classes, i = 0; i < d->num_classes; i++, ip++ )
		{
			...
			{
				case ButtonClass:
					DeviceButtonPress( device, bPress, event_list[cnt] );
					cnt++;
					DeviceButtonRelease( device, bRelease, event_list[cnt] );
					cnt++;
