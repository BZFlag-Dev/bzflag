...
LinuxDisplayMode::LinuxDisplayMode(): display( NULL ), numResolutions( 0 ), lastResolution(  - 1 ), resolutions( NULL ), origNumResolutions( 0 ), origResolutions( NULL )
{
	// do nothing
}
...
XDisplayMode::ResInfo **LinuxDisplayMode::init( XDisplay *_display, int &numModes, int &currentMode )
{
	...
	if( !XF86VidModeQueryExtension( display->getRep()->getDisplay(),  &eventbase, &errorbase ))
		return NULL;

	// get available resolutions
	if( !XF86VidModeGetAllModeLines( display->getRep()->getDisplay(), display->getRep()->getScreen(),  &numResolutions, &origResolutions ))
		return NULL;

	// get current resolution
	int dotclock;
	XF86VidModeModeLine mode;
	XF86VidModeGetModeLine( display->getRep()->getDisplay(), display->getRep()->getScreen(),  &dotclock, &mode );
	...
	for( current = 0; current < numResolutions; current++ )
	{
		...
		if( dotclock == ( int )r->dotclock && mode.hdisplay == r->hdisplay && mode.hsyncstart == r->hsyncstart && mode.hsyncend == r->hsyncend && mode.htotal == r->htotal && mode.vdisplay == r->vdisplay && mode.vsyncstart == r->vsyncstart && mode.vsyncend == r->vsyncend && mode.vtotal == r->vtotal && mode.flags == r->flags )
			break;
	}
...
bool LinuxDisplayMode::doSet( int index, bool position )
{
	...
	// change resolution
	if( XF86VidModeSwitchToMode( display->getRep()->getDisplay(), display->getRep()->getScreen(), resolutions[index] ))
	{
		if( position )
		{
			...
			XF86VidModeSetViewPort( display->getRep()->getDisplay(), display->getRep()->getScreen(), 0, 0 );
		}
