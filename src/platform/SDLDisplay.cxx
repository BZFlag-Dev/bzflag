...
SDLDisplay::SDLDisplay(): fullScreen( false ), base_width( 640 ), base_height( 480 ), canGrabMouse( true ), oldFullScreen( false ), oldWidth( 0 ), oldHeight( 0 )
{
	...
	SDL_Rect **modeList = SDL_ListModes( NULL, SDL_HWSURFACE | SDL_OPENGL | SDL_FULLSCREEN | SDL_HWPALETTE );
	if( !modeList )
	...
	if(( modeList != ( SDL_Rect ** )0 ) && ( modeList != ( SDL_Rect ** ) - 1 ))
	{
		for( int i = 1; modeList[i]; i++ )
			if(( modeList[i - 1]->w != modeList[i]->w ) || ( modeList[i - 1]->h != modeList[i]->h ))
				_numResolutions++;
	};
...
bool SDLDisplay::setupEvent( BzfEvent &_event, const SDL_Event &event )const
{
	...
	switch( event.type )
	{
		...
		case SDL_MOUSEMOTION:
			 ...
			 */
			if(( fullScreen ) || ( sdlver->major > 1 ) || ( sdlver->minor > 2 ) || ( sdlver->patch > 6 ))
			{
				my = event.motion.y;
			}
			else
...
bool SDLDisplay::getKey( const SDL_Event &sdlEvent, BzfKeyEvent &key )const
{
	...
	if( mod &KMOD_NUM )
	{
		if((( key.button >= BzfKeyEvent::Kp0 ) && ( key.button <= BzfKeyEvent::Kp9 )) || ( key.button == BzfKeyEvent::Kp_Enter ))
			key.button = BzfKeyEvent::NoButton;
	}
...
bool SDLDisplay::createWindow()
{
	...
	if(( width == oldWidth ) && ( height == oldHeight ) && ( fullScreen == oldFullScreen ))
		return true;
...
void SDLVisual::setRGBA( int minRed, int minGreen, int minBlue, int minAlpha )
{
...
SDLWindow::SDLWindow( const SDLDisplay *_display, SDLVisual* ): BzfWindow( _display ), x(  - 1 ), y(  - 1 ), hasGamma( true ){}
