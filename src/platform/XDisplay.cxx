...
XDisplay::Rep::Rep( const char *displayName ): refCount( 1 ), display( NULL ), screen( 0 )
{
	// open display
...
XDisplay::XDisplay( const char *displayName, XDisplayMode *_mode ): rep( NULL ), mode( _mode )
{
	// open display
	...
	if( rep->getDisplay())
	{
		...
		if( !resInfo )
		{
			...
			resInfo[0] = new ResInfo( "default", DisplayWidth( rep->getDisplay(), rep->getScreen()), DisplayHeight( rep->getDisplay(), rep->getScreen()), 0 );
			numModes = 1;
			currentMode = 0;
		}
		...
		case ClientMessage:
			{
				...
				if( cme->format == 32 )
				{
					if(( Atom )cme->data.l[0] == XInternAtom( rep->getDisplay(), "WM_DELETE_WINDOW", true ))
					{
...
bool XDisplay::getKey( const XEvent &xevent, BzfKeyEvent &key )const
{
	char buf[3];
	KeySym keysym;
