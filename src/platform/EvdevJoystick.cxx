...
void EvdevJoystick::scanForJoysticks( std::map < std::string, EvdevJoystickInfo >  &joysticks )
{
	joysticks.clear();
	...
	while(( dent = readdir( inputdir )))
	{
		...
		if( !fd )
		{
			...
			if( fd )
				DEBUG4( "Opened event device %s as read-only.\n", info.filename.c_str());
		} 
		else
		{
...
bool EvdevJoystick::isJoystick( struct EvdevJoystickInfo &info )
{
	...
	if( !test_bit( EV_KEY, info.evbit ))
		return false;
	if( !( test_bit( BTN_TRIGGER, info.keybit ) || test_bit( BTN_A, info.keybit ) || test_bit( BTN_1, info.keybit )))
		return false;

	if( !test_bit( EV_ABS, info.evbit ))
		return false;
	if( !( test_bit( ABS_X, info.absbit ) && test_bit( ABS_Y, info.absbit )))
		return false;
...
void EvdevJoystick::getJoy( int &x, int &y )
{
	if( currentJoystick )
	{
		...
		for( axis = 0; axis < 9; axis++ )
		{
			...
			value = value * 2000 / ( currentJoystick->axis_info[axis].maximum - currentJoystick->axis_info[axis].minimum );
			value -= 1000;
...
void EvdevJoystick::getJoyDevices( std::vector < std::string >  &list )const
{
...
static const std::string anames[9] = 
{
	"X", "Y", "Z", "Rx", "Ry", "Rz", "Throttle", "Rudder", "Wheel"
};

void EvdevJoystick::getJoyDeviceAxes( std::vector < std::string >  &list )const
{
	list.clear();
...
bool EvdevJoystick::ffHasRumble()const
{
	...
	if( !currentJoystick )
		...
		return test_bit( EV_FF, currentJoystick->evbit ) && test_bit( FF_RUMBLE, currentJoystick->ffbit ) && !currentJoystick->readonly;
	#else 
	return false;
	#endif 
}
...
//-------------------------------------------------------------------------
	...
	#ifdef HAVE_FF_EFFECT_RUMBLE
void EvdevJoystick::ffRumble( int count, float delay, float duration, float strong_motor, float weak_motor )
{
	if( !ffHasRumble())
		return ;
...
bool EvdevJoystick::ffHasDirectional()const
{
	...
	if( !currentJoystick )
		...
		return test_bit( EV_FF, currentJoystick->evbit ) && test_bit( FF_PERIODIC, currentJoystick->ffbit ) && test_bit( FF_CONSTANT, currentJoystick->ffbit ) && !currentJoystick->readonly;
	#else 
	return false;
	#endif 
} 
	...
	#ifdef HAVE_FF_EFFECT_DIRECTIONAL
void EvdevJoystick::ffDirectionalConstant( int count, float delay, float duration, float x_direction, float y_direction, float strength )
{
	if( !ffHasDirectional())
		return ;
	...
	if( count > 0 )
	{
		...
		ff_rumble->direction = ( int )( 0xFFFF *( 1.0 / ( 2.0 *M_PI )) *atan2( x_direction,  - y_direction ) + 0.5 );
...
void EvdevJoystick::ffDirectionalConstant( int, float, float, float, float, float ){}
	...
	#ifdef HAVE_FF_EFFECT_DIRECTIONAL
void EvdevJoystick::ffDirectionalPeriodic( int count, float delay, float duration, float x_direction, float y_direction, float amplitude, float period, PeriodicType type )
{
	if( !ffHasDirectional())
		return ;
...
//-------------------------------------------------------------------------
	...
	#else 
void EvdevJoystick::ffDirectionalPeriodic( int, float, float, float, float, float, float, PeriodicType ){}
	#endif 

	#ifdef HAVE_FF_EFFECT_DIRECTIONAL
void EvdevJoystick::ffDirectionalResistance( float time, float coefficient, float saturation, ResistanceType type )
{
	if( !ffHasDirectional())
		return ;
	 ...
	 * effect we were playing, if any.
	 */
	if(( ff_rumble->type != FF_SPRING ) && ( ff_rumble->type != FF_FRICTION ) && ( ff_rumble->type != FF_DAMPER ))
	{
		ffResetEffect();
	} 
	else if( ff_rumble->id !=  - 1 )
	...
	if( 1 > 0 )
	{
		...
		ff_rumble->u.condition[0].right_saturation = ff_rumble->u.condition[0].left_saturation = ( int )( 0x7FFF *saturation + 0.5f );
		ff_rumble->u.condition[0].right_coeff = ff_rumble->u.condition[0].left_coeff = ( int )( 0x7FFF *coefficient + 0.5f );
