...
WinWindow::WinWindow( const WinDisplay *_display, WinVisual *_visual ): BzfWindow( _display ), display( _display ), visual( *_visual ), inDestroy( false ), hwndChild( NULL ), hRC( NULL ), hDC( NULL ), hDCChild( NULL ), inactiveDueToDeactivate( false ), inactiveDueToDeactivateAll( false ), useColormap( false ), hasGamma( false ), has3DFXGamma( false ), gammaVal( 1.0f ), prev( NULL ), next( NULL )
{
	// make window
	hwnd = CreateWindow( "BZFLAG", "bzflag", WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP | WS_BORDER | WS_CAPTION, 0, 0, 640, 480, NULL, NULL, display->getRep()->hInstance, NULL );
	if( hwnd == NULL )
		return ;
...
void WinWindow::setFullscreen()
{
	...
	if( display->isFullScreenOnly())
		MoveWindow( hwnd, 0, 0, display->getFullWidth(), display->getFullHeight(), FALSE );
	else
...
void WinWindow::setGamma( float newGamma )
{
	...
	else
	{
		...
		for( int i = 0; i < 256; i++ )
		{
			...
			map[0][i] = map[1][i] = map[2][i] = map[3][i] = map[4][i] = map[5][i] = ( WORD )v + (( WORD )v << 8 );
		}
...
void WinWindow::createChild()
{
	...
	// make window
	hwndChild = CreateWindow( "BZFLAG", "opengl", WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_CHILD | WS_VISIBLE, 0, 0, width, height, hwnd, NULL, display->getRep()->hInstance, NULL );
	if( hwndChild == NULL )
		return ;
...
float WinWindow::getComponentFromIndex( int i, UINT nbits, UINT shift )
{
...
void WinWindow::makeColormap( const PIXELFORMATDESCRIPTOR &pfd )
{
	// compute number of colors
	const int n = 1 << pfd.cColorBits;

	// make and initialize a logical palette with room for all colors
	LOGPALETTE *logicalPalette = ( LOGPALETTE* )( void* )new char[sizeof( LOGPALETTE ) + n * sizeof( PALETTEENTRY )];
	...
	for( i = 0; i < n; i++ )
	{
		logicalPalette->palPalEntry[i].peRed = getIntensityValue( getComponentFromIndex( i, pfd.cRedBits, pfd.cRedShift ));
		logicalPalette->palPalEntry[i].peGreen = getIntensityValue( getComponentFromIndex( i, pfd.cGreenBits, pfd.cGreenShift ));
		logicalPalette->palPalEntry[i].peBlue = getIntensityValue( getComponentFromIndex( i, pfd.cBlueBits, pfd.cBlueShift ));
	...
	if( matchGDI )
	{
		...
		for( i = 0; i < n; i++ )
		{
			...
			for( j = 0; j < NUM_GDI_COLORS; j++ )
			if( p->peRed == gdi[j].peRed && p->peGreen == gdi[j].peGreen && p->peBlue == gdi[j].peBlue )
			{
				exactMatch[j] = TRUE;
				p->peFlags = 1;
				break;
