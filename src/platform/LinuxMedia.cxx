...
LinuxMedia::LinuxMedia(): BzfMedia(), audioReady( false ), audioBufferSize( defaultAudioRate ), audioPortFd(  - 1 ), queueIn(  - 1 ), queueOut(  - 1 ), outputBuffer( NULL ), childProcID( 0 ), audio8Bit( false ), getospaceBroken( false )
{
	// do nothing
}
...
{
	bool flag = false;
	if( !access( "/dev/dsp", W_OK ))
		flag = true;
	if( !access( "/dev/sound/dsp", W_OK ))
		flag = true;
	return flag;
}
...
bool LinuxMedia::openIoctl( int cmd, void *value, bool req )
{
	if( audioPortFd ==  - 1 )
		return false;
...
bool LinuxMedia::openAudioHardware()
{
	...
	if( noSetFragment || !openIoctl( SNDCTL_DSP_GETBLKSIZE, &audioBufferSize, false ) || audioBufferSize > ( 1 << fragmentSize ))
	{
	...
	if( audioPortFd !=  - 1 )
	{
		...
		if( !openIoctl( SNDCTL_DSP_GETOSPACE, &info, false ))
		{
			...
			chunksPending = 0;
			chunksPerSecond = ( double )getAudioOutputRate() / ( double )getAudioBufferChunkSize();
		}
	}
...
bool LinuxMedia::startAudioThread( void( *proc )( void* ), void *data )
{
...
void LinuxMedia::writeAudioFrames8Bit( const float *samples, int numFrames )
{
	...
	while( numSamples > 0 )
	{
		...
		for( int j = 0; j < limit; j++ )
		{
			if( samples[j] <=  - 32767.0 )
				...
				smOutputBuffer[j] = 255;
			else
				smOutputBuffer[j] = char(( samples[j] + 32767 ) / 257 );
		}
...
void LinuxMedia::writeAudioFrames16Bit( const float *samples, int numFrames )
{
	int numSamples = 2 * numFrames;
	int limit = 0;
...
void LinuxMedia::writeAudioFrames( const float *samples, int numFrames )
{
	if( audio8Bit )
	...
	if( getospaceBroken )
	{
		if( chunksPending == 0 )
			chunkTime = getTime();
		chunksPending += ( numFrames + getAudioBufferChunkSize() - 1 ) / getAudioBufferChunkSize();
	}
}
...
void LinuxMedia::audioSleep( bool checkLowWater, double endTime )
{
	...
	if( checkLowWater )
	{
		...
		do
		{
			...
			tv.tv_sec = 0;
			tv.tv_usec = 50000;
			if( select( maxFd, &commandSelectSet, 0, 0, &tv ))
				break;
