...
XWindow::XWindow( const XDisplay *_display, XVisual *_visual ): BzfWindow( _display ), display( _display->getRep()), window( None ), colormap( None ), context( NULL ), noWM( false ), defaultColormap( true ), prev( NULL ), next( NULL ), colormapPixels( NULL ), xsh( XAllocSizeHints()), gammaVal( 1.0 )
{
	// get desired visual
	...
	// make a colormap
	colormap = XCreateColormap( display->getDisplay(), display->getRootWindow(), visual.visual, AllocNone );
	...
	windowAttrib.event_mask = ExposureMask | StructureNotifyMask | PointerMotionMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask;
	window = XCreateWindow( display->getDisplay(), display->getRootWindow(), 0, 0, 1, 1, 0, visual.depth, InputOutput, visual.visual, CWBackPixel | CWBorderPixel | CWColormap | CWEventMask,  &windowAttrib );
	if( window == None )
	{
	...
	if( visual.c_class == DirectColor )
	{
		...
		if( XAllocColorPlanes( display->getDisplay(), colormap, true, &pixel, 1, rBits, gBits, bBits, &rMask, &gMask, &bMask ))
			defaultColormap = false;
	}

	// these shouldn't happen because we request an RGBA visual, but Mesa
	// doesn't play by the usual rules.
	else if( visual.c_class == GrayScale || ( visual.c_class == PseudoColor && visual.depth == 8 ))
	{
		// allocate colors
		unsigned long mask;
		colormapPixels = new unsigned long[visual.colormap_size];
		if( colormapPixels && XAllocColorCells( display->getDisplay(), colormap, true, &mask, 0, colormapPixels, visual.colormap_size ))
			defaultColormap = false;
	}
	if( !defaultColormap )
	{
...
void XWindow::showWindow( bool show )
{
	if( show )
	{
		...
		if( noWM )
		{
			XSetInputFocus( display->getDisplay(), window, RevertToPointerRoot, CurrentTime );
			if( !defaultColormap )
...
public:
		...
		long *mwmInfo;
		XGetWindowProperty( display->getDisplay(), display->getRootWindow(), a, 0, 4, false, a, &type, &format, &nitems, &bytes_after, ( unsigned char ** ) &mwmInfo );
		if( mwmInfo )
		{
			...
			if( XQueryTree( display->getDisplay(), mwmWindow, &root, &parent,  &children, &numChildren ))
			{
				XFree( children );
	...
	if( isMWMRunning )
	{
		...
		a = XInternAtom( display->getDisplay(), "_MOTIF_WM_HINTS", false );
		{
			...
			unsigned long nitems;
			unsigned long bytes_after;
			XGetWindowProperty( display->getDisplay(), window, a, 0, 4, false, a, &type, &format, &nitems, &bytes_after, ( unsigned char ** ) &xhints );
			if( xhints )
			{
				hints[0] = xhints[0];
				hints[1] = xhints[1];
		...
		XChangeProperty( display->getDisplay(), window, a, a, 32, PropModeReplace, ( unsigned char* ) &hints, 4 );
		noWM = false;
	}
	...
	else
	{
		...
		XChangeWindowAttributes( display->getDisplay(), window, CWOverrideRedirect, &attr );
		noWM = true;
	}
...
#ifdef __linux__
	{
		...
		if( env &&  *env != tolower( 'w' ))
		{
			// Full screen Mesa mode
	...
	XSetWMNormalHints( display->getDisplay(), window, xsh );
	XMoveResizeWindow( display->getDisplay(), window, xsh->x, xsh->y, xsh->base_width, xsh->base_height );
	if( !noWM )
	{
		XSetWindowAttributes attr;
...
void XWindow::getMouse( int &x, int &y )const
{
	...
	unsigned int mask;
	int mx, my;
	XQueryPointer( display->getDisplay(), window,  &rootWindow, &childWindow,  &rootX, &rootY, &mx, &my, &mask );
	x = mx;
	y = my;
}
...
{
	XGrabPointer( display->getDisplay(), window, true, 0, GrabModeAsync, GrabModeAsync, window, None, CurrentTime );
}
...
void XWindow::hideMouse()
{
	...
	if( cursor == None )
	{
		...
		if( font )
		{
			...
			cursor = XCreateGlyphCursor( display->getDisplay(), font, font, ' ', ' ', &color, &color );
...
void XWindow::loadColormap()
{
	...
	else if( visual.c_class == GrayScale )
	{
		...
		{
			const unsigned short v = getIntensityValue(( float )i / ( float )( visual.colormap_size - 1 ));
...
void XWindow::countBits( unsigned long mask, int &num, int &offset )
{
	num = 0;
	offset = 0;
