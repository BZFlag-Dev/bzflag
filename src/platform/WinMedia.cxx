...
WinMedia::WinMedia( WinWindow *_window ): window( _window->getHandle()), audioReady( false ), audioInterface( NULL ), audioPrimaryPort( NULL ), audioPort( NULL ), outputBuffer( NULL ), audioCommandBuffer( NULL ), audioCommandEvent( NULL ), audioCommandMutex( NULL ), audioThread( NULL ){}
...
bool WinMedia::openAudio()
{
	...
	audioBufferParams.dwFlags = DSBCAPS_PRIMARYBUFFER | DSBCAPS_CTRLFREQUENCY | DSBCAPS_GETCURRENTPOSITION2;
	HRESULT status = audioInterface->CreateSoundBuffer( &audioBufferParams,  &audioPrimaryPort, NULL );
	if( FAILED( status ))
	{
		audioBufferParams.dwFlags = DSBCAPS_PRIMARYBUFFER | DSBCAPS_GETCURRENTPOSITION2;
		status = audioInterface->CreateSoundBuffer( &audioBufferParams,  &audioPrimaryPort, NULL );
	}
	if( FAILED( status ))
	{
	...
	audioBufferParams.dwFlags = DSBCAPS_CTRLFREQUENCY | DSBCAPS_GETCURRENTPOSITION2;
	status = audioInterface->CreateSoundBuffer( &audioBufferParams,  &audioPort, NULL );
	if( FAILED( status ))
	{
...
void WinMedia::closeAudio()
{
	...
	if( audioCommandEvent )
		CloseHandle( audioCommandEvent );
	if( audioCommandMutex )
		CloseHandle( audioCommandMutex );
...
bool WinMedia::startAudioThread( void( *proc )( void* ), void *data )
{
	if( audioThread || !proc )
		return false;
	threadProc = proc;
...
void WinMedia::writeSoundCommand( const void *msg, int len )
{
	...
	if( queueFilled < 0 )
		queueFilled += audioCommandBufferLen;
...
void WinMedia::writeAudioFrames( const float *samples, int numFrames )
{
	...
	if( numSamples > audioBufferChunkSize )
		numSamples = audioBufferChunkSize;
	...
	void *ptr1,  *ptr2;
	DWORD size1, size2;
	HRESULT result = audioPort->Lock( audioWritePtr *audioBytesPerSample, numSamples *audioBytesPerSample,  &ptr1, &size1, &ptr2, &size2, 0 );
	if( result == DSERR_BUFFERLOST )
	{
		...
		if( result == DS_OK )
			result = audioPort->Lock( audioWritePtr *audioBytesPerSample, numSamples *audioBytesPerSample,  &ptr1, &size1, &ptr2, &size2, 0 );
	}
...
void WinMedia::audioSleep( bool checkLowWater, double maxTime )
{
