...
{
printf("BeOSWindow::showWindow(%s)\n", on?"true":"false");
//	bWindow->Lock();
	if (on)
		bWindow->Show();
	else
...
{
printf("BeOSWindow::yieldCurrent()\n");
	if (!oglContextInitialized)
		return;
	if (bWindow != NULL) {
		thread_id tid = find_thread(NULL);
...
{
printf("BeOSWindow::releaseCurrent()\n");
	if (!oglContextInitialized)
		return;
	if (bWindow != NULL) {
		thread_id tid = find_thread(NULL);
