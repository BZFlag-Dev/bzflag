...
public:
	...
	Resolution( const char *comboName, int numFormats, XSGIvcVideoFormatInfo *formats );
	~Resolution();
...
Resolution::Resolution( XSGIvcVideoFormatInfo *format ): valid( true ), data( NULL )
{
...
Resolution::Resolution( const char *comboName, int numFormats, XSGIvcVideoFormatInfo *formats ): valid( true ), data( NULL )
{
...
Resolution::~Resolution()
{
	...
	if( combination )
		...
		delete ( XSGIvcVideoFormatInfo* )data;
}
...
void Resolution::setConfig( int index, const XSGIvcVideoFormatInfo *format )
{
...
bool Resolution::setFormat( Display *dpy, int screen )const
{
	...
	{
		status = XSGIvcLoadVideoFormat( dpy, screen, 0, ( XSGIvcVideoFormatInfo* )data, XSGIVC_QVFHeight | XSGIVC_QVFWidth | XSGIVC_QVFSwapbufferRate, false );
	}

	XSync( dpy, false );
	XSetErrorHandler( prevErrorHandler );
	if( !status || errorHappened )
		(( Resolution* )this )->valid = false;
	return valid;
}
...
static int resolutionCompare( const void *_a, const void *_b )
{
	...
	if( aNumChannels < bNumChannels )
		return  - 1;
	if( aNumChannels > bNumChannels )
		return 1;
	for( int i = 0; i < aNumChannels; i++ )
	{
		if( aConfig[i].width < bConfig[i].width )
			return  - 1;
		if( aConfig[i].width > bConfig[i].width )
			return 1;
		if( aConfig[i].height < bConfig[i].height )
			return  - 1;
		if( aConfig[i].height > bConfig[i].height )
			return 1;
		if( aConfig[i].refresh < bConfig[i].refresh )
			return  - 1;
		if( aConfig[i].refresh > bConfig[i].refresh )
			return 1;
	}
...
XDisplayMode::ResInfo **SGIDisplayMode::init( XDisplay *_display, int &numModes, int &currentMode )
{
	...
	if( !XSGIvcQueryVersion( rep->getDisplay(), &major, &minor ))
		return NULL;
	if( !XSGIvcQueryVideoScreenInfo( rep->getDisplay(), rep->getScreen(), &screenInfo ))
		return NULL;
	if( screenInfo.numChannels < 1 )
		return NULL;
	...
	for( i = 0; i < numVideoChannels; i++ )
	{
		...
		if( XSGIvcQueryChannelInfo( rep->getDisplay(), rep->getScreen(), i, &channelInfo ))
		{
	...
	if( screenInfo.flags &XSGIVC_SIFFormatPerChannel )
	{
		...
		for( i = 0; i < numVideoChannels; i++ )
		{
			errorHappened = false;
			int numFormats = 0;
			XSGIvcVideoFormatInfo *formats = XSGIvcListVideoFormats( rep->getDisplay(), rep->getScreen(), i,  &pattern, 0, false, 4096, &numFormats );
			if( !formats || errorHappened )
				numFormats = 0;
		...
		for( i = 0; i < numVideoChannels; i++ )
		{
			...
			for( int j = 0; j < numVideoFormats[i]; j++ )
			{
				...
				if(( name1[0] && name2[0] && strcmp( name1, name2 ) == 0 ) || resolutionCompare( videoFormats[i] + j, channelFormat + i ) == 0 )
				{
	...
	if( screenInfo.flags &XSGIVC_SIFFormatCombination )
	{
		errorHappened = false;
		int numCombos;
		char **comboNames = XSGIvcListVideoFormatsCombinations( rep->getDisplay(), rep->getScreen(), "*", 4096, &numCombos );
		if( !comboNames || errorHappened )
			numCombos = 0;
		...
		for( i = 0; i < numCombos; i++ )
		{
			...
			XSGIvcVideoFormatInfo *formats = XSGIvcListVideoFormatsInCombination( rep->getDisplay(), rep->getScreen(), comboNames[i], &numFormats );
			if( formats && numFormats != 0 && !errorHappened )
			{
				// make format for combo
				videoCombos[numVideoCombos++] = new Resolution( comboNames[i], numFormats, formats );
	...
	for( i = 0; i < numVideoCombos; i++ )
	{
		...
		if( defaultVideoCombo && strcmp( defaultVideoCombo, videoCombos[i]->getName()) == 0 )
...
bool SGIDisplayMode::set( int index )
{
	...
	if( resolutions[index]->setFormat( display->getRep()->getDisplay(), display->getRep()->getScreen()))
	{
