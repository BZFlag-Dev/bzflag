...
XVisual::XVisual( const XDisplay *_display ): display( _display->getRep()), attributeCount( 0 ), visual( NULL )
{
	display->ref();
...
void XVisual::setLevel( int level )
{
	...
	if( index ==  - 1 )
		appendAttribute( GLX_LEVEL, level );
	else
		editAttribute( index, level );
}
...
void XVisual::setDoubleBuffer( bool on )
{
	...
	{
		if( index ==  - 1 )
			appendAttribute( GLX_DOUBLEBUFFER, GLX_USE_GL );
	}
}
...
void XVisual::setIndex( int minDepth )
{
	...
	if( index ==  - 1 )
		appendAttribute( GLX_BUFFER_SIZE, minDepth );
	else
		editAttribute( index, minDepth );
}
...
void XVisual::setRGBA( int minRed, int minGreen, int minBlue, int minAlpha )
{
	int index = findAttribute( GLX_RGBA );
	if( index ==  - 1 )
		appendAttribute( GLX_RGBA, GLX_USE_GL );
	index = findAttribute( GLX_RED_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_RED_SIZE, minRed );
	else
		editAttribute( index, minRed );
	index = findAttribute( GLX_GREEN_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_GREEN_SIZE, minGreen );
	else
		editAttribute( index, minGreen );
	index = findAttribute( GLX_BLUE_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_BLUE_SIZE, minBlue );
	else
		editAttribute( index, minBlue );
	...
	if( minAlpha == 0 )
	{
		...
		appendAttribute( GLX_ALPHA_SIZE, minAlpha );
	else
		editAttribute( index, minAlpha );
}
...
void XVisual::setDepth( int minDepth )
{
	...
	if( minDepth == 0 )
	{
		...
		appendAttribute( GLX_DEPTH_SIZE, minDepth );
	else
		editAttribute( index, minDepth );
}
...

{
	...
	if( minDepth == 0 )
	{
		...
		appendAttribute( GLX_STENCIL_SIZE, minDepth );
	else
		editAttribute( index, minDepth );
}
...
void XVisual::setAccum( int minRed, int minGreen, int minBlue, int minAlpha )
{
	int index = findAttribute( GLX_ACCUM_RED_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_ACCUM_RED_SIZE, minRed );
	else
		editAttribute( index, minRed );
	index = findAttribute( GLX_ACCUM_GREEN_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_ACCUM_GREEN_SIZE, minGreen );
	else
		editAttribute( index, minGreen );
	index = findAttribute( GLX_ACCUM_BLUE_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_ACCUM_BLUE_SIZE, minBlue );
	else
		editAttribute( index, minBlue );
	index = findAttribute( GLX_ACCUM_ALPHA_SIZE );
	if( index ==  - 1 )
		appendAttribute( GLX_ACCUM_ALPHA_SIZE, minAlpha );
	else
		editAttribute( index, minAlpha );
}
...
void XVisual::setStereo( bool on )
{
	...
	{
		if( index ==  - 1 )
			appendAttribute( GLX_STEREO, GLX_USE_GL );
	}
}
...
void XVisual::setMultisample( int minSamples )
{
	...
	if( index ==  - 1 )
		appendAttribute( GLX_SAMPLES_SGIS, minSamples );
	else
		editAttribute( index, minSamples );
#endif 
}
...
bool XVisual::build()
{
	if( !visual && getenv( "MESA_RGB_VISUAL" ) == NULL )
	{
		...
		int numVisuals;
		XVisualInfo *visualList = XGetVisualInfo( display->getDisplay(), visualMask,  &visualTemplate,  &numVisuals );
		if( numVisuals > 0 && visualList != NULL )
		{
			...
			for( int i = 0; i < numVisuals; i++ )
			{
				...
				if( glXGetConfig( display->getDisplay(), visualList + i, GLX_USE_GL, &attrib ) != 0 || attrib == GL_FALSE )
					continue;
				...
				if( visualClassIsBetter( visualList[i].c_class, visualList[bestVisual].c_class ))
				{
					bestVisual = i;
					continue;
				}
			...
			if( bestVisual !=  - 1 )
			{
				visual = XGetVisualInfo( display->getDisplay(), VisualAllMask, visualList + bestVisual,  &numVisuals );
				if( numVisuals == 0 )
					visual = NULL;
			}
	...
	if( !visual )
	{
		visual = glXChooseVisual( display->getDisplay(), display->getScreen(), attributes );
	}
...
bool XVisual::matchRequirements( XVisualInfo *v )const
{
	...
	int value;
	if( glXGetConfig( display->getDisplay(), v, GLX_RGBA, &value ) != 0 || ( findAttribute( GLX_RGBA ) !=  - 1 ) != value )
		return false;
	if( glXGetConfig( display->getDisplay(), v, GLX_DOUBLEBUFFER, &value ) != 0 || ( findAttribute( GLX_DOUBLEBUFFER ) !=  - 1 ) != value )
		return false;
	if( glXGetConfig( display->getDisplay(), v, GLX_STEREO, &value ) != 0 || ( findAttribute( GLX_STEREO ) !=  - 1 ) != value )
		return false;
