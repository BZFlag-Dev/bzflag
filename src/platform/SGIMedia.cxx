...
SGIMedia::SGIMedia(): BzfMedia(), audioReady( false ), audioPort( NULL ), queueIn(  - 1 ), queueOut(  - 1 ), outputBuffer( NULL ), childProcID( 0 ), iotimer_addr( NULL )
{
	...
	if( addr !=  - 1 )
	{
		...
		iotimer_addr = ( volatile unsigned int* )mmap( 0, poffmask, PROT_READ, MAP_PRIVATE, fd, ( off_t )raddr );
		iotimer_addr = ( unsigned int* )(( __psunsigned_t )iotimer_addr + ( phys_addr &poffmask ));
...
bool SGIMedia::startAudioThread( void( *proc )( void* ), void *data )
{
...
void SGIMedia::writeAudioFrames( const float *samples, int numFrames )
{
...
void SGIMedia::audioSleep( bool checkLowWater, double endTime )
{
	...
	// wait
	select( maxFd, &commandSelectSet, checkLowWater ? &audioSelectSet: NULL, NULL, ( struct timeval* )( endTime >= 0.0 ? &tv: NULL ));
}
