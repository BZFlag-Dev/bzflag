...
WinVisual::WinVisual( const WinDisplay *_display ): display( _display->getRep()), pixelFormat(  - 1 ), hDC( NULL )
{
	display->ref();
...
WinVisual::WinVisual( const WinVisual &visual ): BzfVisual(), display( visual.display ), pfd( visual.pfd ), pixelFormat( visual.pixelFormat ), hDC( NULL )
{
	display->ref();
}
...
void WinVisual::setRGBA( int minRed, int minGreen, int minBlue, int minAlpha )
{
	pfd.iPixelType = PFD_TYPE_RGBA;
...
void WinVisual::setAccum( int minRed, int minGreen, int minBlue, int minAlpha )
{
...
bool WinVisual::build()
{
	if( pixelFormat ==  - 1 )
	{
		if( hDC == NULL )
		{
			HWND hwnd = CreateWindow( "BZFLAG", "bzflag", WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP, 0, 0, 1, 1, NULL, NULL, display->hInstance, NULL );
			if( hwnd == NULL )
				return false;
			hDC = GetDC( hwnd );
...
int WinVisual::get( HDC _hDC, const PIXELFORMATDESCRIPTOR **_pfd )
{
	hDC = _hDC;
	build();
	hDC = NULL;
