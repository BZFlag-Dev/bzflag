...
WinDisplay::Rep::Rep( const char* ): refCount( 1 ), hInstance( NULL )
{
...
LONG WINAPI WinDisplay::Rep::windowProc( HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam )
{
	switch( msg )
	{
		case WM_DESTROY:
...
WinDisplay::WinDisplay( const char *displayName, const char* ): rep( NULL ), hwnd( NULL ), using3Dfx( false ), fullWidth( 0 ), fullHeight( 0 ), resolutions( NULL ), translated( false ), charCode( 0 )
{
	...
	if( isValid() && !isFullScreenOnly())
	{
		...
		if( !resInfo )
		{
			...
			resInfo[0] = new ResInfo( "default", GetDeviceCaps( hDC, HORZRES ), GetDeviceCaps( hDC, VERTRES ), 0 );
...
bool WinDisplay::windowsEventToBZFEvent( MSG &msg, BzfEvent &event )const
{
	...
	switch( msg.message )
	{
		...
		case WM_MOUSEMOVE:
			...
			case WM_APP + 0: event.type = BzfEvent::Map;
			break;

			case WM_APP + 1: event.type = BzfEvent::Unmap;
			break;
		...
		case WM_RBUTTONDOWN:
			...
			{
			case WM_LBUTTONDOWN:
				event.keyDown.button = BzfKeyEvent::LeftMouse;
				break;
			case WM_MBUTTONDOWN:
				event.keyDown.button = BzfKeyEvent::MiddleMouse;
				break;
			case WM_RBUTTONDOWN:
				event.keyDown.button = BzfKeyEvent::RightMouse;
				break;
			default:
				return false;
			}
		...
		case WM_RBUTTONUP:
			...
			{
			case WM_LBUTTONUP:
				event.keyUp.button = BzfKeyEvent::LeftMouse;
				break;
			case WM_MBUTTONUP:
				event.keyUp.button = BzfKeyEvent::MiddleMouse;
				break;
			case WM_RBUTTONUP:
				event.keyUp.button = BzfKeyEvent::RightMouse;
				break;
			default:
				return false;
			}
...
{
	MSG msg;
	if( PeekMessage( &msg, NULL, 0, 0, 0 ) ==  - 1 )
		return false;
...
bool WinDisplay::getKey( const MSG &msg, BzfKeyEvent &key )const
{
	key.shift = 0;
	...
	{
		MSG cmsg;
		if( PeekMessage( &cmsg, NULL, 0, 0, PM_NOREMOVE ) && ( cmsg.message == WM_CHAR || cmsg.message == WM_SYSCHAR ))
		{
	...
	if( key.button == BzfKeyEvent::Delete )
		key.ascii = 0;
...
bool WinDisplay::doSetResolution( int index )
{
	...
	// change resolution
	const bool changed = ( ChangeDisplaySettings( &dm, CDS_FULLSCREEN ) == DISP_CHANGE_SUCCESSFUL );
...
BzfDisplay::ResInfo **WinDisplay::getVideoFormats( int &numModes, int &currentMode )
{
	HDC hDC = GetDC( GetDesktopWindow());
	...
	typedef struct
	{
		//On WinXP, DEVMODE buffer overruns are occuring, if the DEVMODE data structure
	...
	for( i = numResolutions - 1; i >= 0; i-- )
	{
		...
		if( r->width == currentWidth && r->height == currentHeight && r->depth == currentDepth )
		{
	...
	if( current !=  - 1 )
	{
		...
		for( i = 0; i < numResolutions; i++ )
		{
			...
			if( r->refresh == 0 )
				...
				sprintf( name, "%dx%d @%dHz %d bits", r->width, r->height, r->refresh, r->depth );
...
bool WinDisplay::canChangeDepth()
{
	...
	if( vinfo.dwMajorVersion == 4 )
	{
		if( vinfo.dwPlatformId == VER_PLATFORM_WIN32_NT )
			return true;
		if( vinfo.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS && LOWORD( vinfo.dwBuildNumber ) >= OSR2_BUILD_NUMBER )
			return true;
	}
	return false;
}
