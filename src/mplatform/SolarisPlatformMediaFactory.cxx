...
#include "SolarisPlatformMediaFactory.h"
#include "XDisplay.h"
...
{
	if (instance == NULL)
		instance = new SolarisPlatformMediaFactory;
	return instance;
}

SolarisPlatformMediaFactory::SolarisPlatformMediaFactory() :
								audio_fd(-1),
...
BzfDisplay*				SolarisPlatformMediaFactory::createDisplay(
...
bool					SolarisPlatformMediaFactory::openAudio()
{
	int fd[2];
...
void					SolarisPlatformMediaFactory::closeAudio()
{
	close(audio_fd);
	close(audioctl_fd);
}

bool					SolarisPlatformMediaFactory::startAudioThread(
...
void					SolarisPlatformMediaFactory::stopAudioThread()
{
	if (childProcID != 0)
...
bool					SolarisPlatformMediaFactory::hasAudioThread() const
{
...
void					SolarisPlatformMediaFactory::writeSoundCommand(const void* cmd, int len)
{
	if (!audio_ready) return;
	write(queueIn, cmd, len);
}

bool					SolarisPlatformMediaFactory::readSoundCommand(void* cmd, int len)
{
	return (read(queueOut, cmd, len)==len);
}

int						SolarisPlatformMediaFactory::getAudioOutputRate() const
{
	return defaultAudioRate;
}

int						SolarisPlatformMediaFactory::getAudioBufferSize() const
{
	return NumChunks * (audioBufferSize >> 1);
}

int						SolarisPlatformMediaFactory::getAudioBufferChunkSize() const
{
	return audioBufferSize>>1;
}

bool					SolarisPlatformMediaFactory::isAudioTooEmpty() const
{
	ioctl(audioctl_fd, AUDIO_GETINFO, &a_info);
	return ((int)a_info.play.eof >= eof_written - audioLowWaterMark);
}

void					SolarisPlatformMediaFactory::writeAudioFrames(
...
void					SolarisPlatformMediaFactory::audioSleep(
