...
#include "WinPlatformMediaFactory.h"
...
{
	if (instance == NULL)
		instance = new WinPlatformMediaFactory;
	return instance;
}
...
void					(*WinPlatformMediaFactory::threadProc)(void*);
void*					WinPlatformMediaFactory::threadData;

WinPlatformMediaFactory::WinPlatformMediaFactory() :
								window(NULL),
								audioReady(false),
								audioInterface(NULL),
...
BzfDisplay*				WinPlatformMediaFactory::createDisplay(
...
BzfVisual*				WinPlatformMediaFactory::createVisual(
								const BzfDisplay* display)
{
	return new WinVisual((const WinDisplay*)display);
}

BzfWindow*				WinPlatformMediaFactory::createWindow(
								const BzfDisplay* display, BzfVisual* visual)
{
	WinWindow* w = new WinWindow((const WinDisplay*)display, (WinVisual*)visual);
	window = w->getHandle();
	return w;
}

bool					WinPlatformMediaFactory::openAudio()
{
	// don't re-initialize
	if (audioReady)
		return false;
...
void					WinPlatformMediaFactory::closeAudio()
{
...
bool					WinPlatformMediaFactory::startAudioThread(
...
void					WinPlatformMediaFactory::stopAudioThread()
{
	if (!audioThread)
		return;
...
bool					WinPlatformMediaFactory::hasAudioThread() const
{
	return true;
}

DWORD WINAPI				WinPlatformMediaFactory::audioThreadInit(void*)
{
...
void					WinPlatformMediaFactory::writeSoundCommand(const void* msg, int len)
{
	if (!audioReady)
		return;
...
bool					WinPlatformMediaFactory::readSoundCommand(void* msg, int len)
{
...
int						WinPlatformMediaFactory::getAudioOutputRate() const
{
...
int						WinPlatformMediaFactory::getAudioBufferSize() const
{
	return audioBufferSize / audioNumChannels;
}

int						WinPlatformMediaFactory::getAudioBufferChunkSize() const
{
	return audioBufferChunkSize / audioNumChannels;
}

bool					WinPlatformMediaFactory::isAudioTooEmpty() const
{
...
void					WinPlatformMediaFactory::writeAudioFrames(
...
void					WinPlatformMediaFactory::audioSleep(
								bool checkLowWater, double maxTime)
{
	...
	if (checkLowWater) {
		...
		} while (maxTime < 0.0 || (PLATFORM->getClock() - start) < maxTime);
	}
