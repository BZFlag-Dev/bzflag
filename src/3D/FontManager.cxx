...
template <> FontManager *Singleton<FontManager>::_instance = ( FontManager* )0;
...
FontManager::FontManager(): Singleton < FontManager > (), opacity( 1.0f ), dimFactor( 0.2f ), darkness( 1.0f )
{
	faceNames.clear();
	fontFaces.clear();
...
void FontManager::drawString( float x, float y, float z, int faceID, float size, const std::string &text, const float *resetColor )
{
	if( text.size() == 0 )
		return ;
	...
	if( !pFont )
	{
		DEBUG2( "Could not find applicable font size for rendering; font face ID %d, ""requested size %f\n", faceID, size );
		return ;
	}
	...
	{
		underlineColor[0] *darkDim, underlineColor[1] *darkDim, underlineColor[2] *darkDim, opacity
	};
...
void FontManager::drawString( float x, float y, float z, const std::string &face, float size, const std::string &text, const float *resetColor )
{
...
float FontManager::getStrLength( int faceID, float size, const std::string &text, bool alreadyStripped )
{
	if( text.size() == 0 )
		return 0.0f;
	...
	if( !pFont )
	{
		DEBUG2( "Could not find applicable font size for sizing; font face ID %d, ""requested size %f\n", faceID, size );
		return 0.0f;
	}
...
float FontManager::getStrLength( const std::string &face, float size, const std::string &text, bool alreadyStripped )
{
...
float FontManager::getStrHeight( int faceID, float size, const std::string & /* text */ )
{
...
float FontManager::getStrHeight( std::string face, float size, const std::string &text )
{
