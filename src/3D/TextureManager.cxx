...
template <> TextureManager *Singleton<TextureManager>::_instance = ( TextureManager* )0;
...
bool TextureManager::getColorAverages( int texId, float rgba[4], bool factorAlpha )const
{
	TextureIDMap::const_iterator it = textureIDs.find( texId );
	if( it == textureIDs.end())
	{
		DEBUG1( "getColorAverages: Unable to find texture (by id): %d\n", texId );
		return false;
	}
