...
BSPSceneDatabase::Node::Node( bool _dynamic, SceneNode *_node ): dynamic( _dynamic ), count( 0 ), node( _node ), front( NULL ), back( NULL )
{
	// do nothing
}
...
BSPSceneDatabase::BSPSceneDatabase(): root( NULL ), depth( 0 )
{
...
bool BSPSceneDatabase::insertStatic( int level, Node *_root, SceneNode *node, bool dontFree )
{
...
void BSPSceneDatabase::insertDynamic( int level, Node *_root, SceneNode *node )
{
	GLfloat d;
...
void BSPSceneDatabase::insertNoPlane( int level, Node *_root, SceneNode *node )
{
	// dynamic nodes should only be inserted after all static nodes
	assert( _root->dynamic == false );
