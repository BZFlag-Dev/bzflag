...
int Octree::getFrustumList( SceneNode **list, int listSize, const Frustum *frustum )const
{
	if( !root )
	{
		return 0;
...
int Octree::getRadarList( SceneNode **list, int listSize, const Frustum *frustum )const
{
...
int Octree::getShadowList( SceneNode **list, int listSize, int planeCount, const float( *planes )[4] )const
{
	if( !root )
	{
		return 0;
...
OctreeNode::OctreeNode( unsigned char _depth, const Extents &exts, SceneNode **_list, int _listSize )
{
	int i;
