...
void Octree::addNodes(SceneNode** list, int listSize, int depth, int elements) {
  ...
  debugf(2, "Octree total elements = %i\n", totalElements);
...
void Octree::draw() const {
  ...
  // It should still exist in SceneRender.cpp
...
OctreeNode::OctreeNode(unsigned char _depth, const Extents& exts,
  ...
  if (((int)depth >= maxDepth) || (listSize <= minElements)) {
    resizeCell();
    //debugf(4,"LEAF NODE: depth = %d, items = %i\n", depth, count);
    return;
  }
  ...
  //debugf(4,"BRANCH NODE: depth = %d, squeezed = %i\n", depth, childCount);
...
void OctreeNode::getShadowList() const {
  IntersectLevel level =
    testAxisBoxOcclusion(extents, ShadowPlanes, ShadowCount);
  if (level == Outside) {
    return;
  }
