...
void ZSceneDatabase::setupCullList()
{
	...
	if(( culledList == NULL ) || ( currentDepth != cullDepth ) || ( currentElements != cullElements ))
	{
...
static void setupShadowPlanes( const Frustum *frustum, const float *sunDir, int &planeCount, float planes[4][4] )
{
	...
	if( eye[2] > 20.0f )
	{
		...
		const float hlen = sqrtf(( frustum->getSide( 3 )[0] *frustum->getSide( 3 )[0] ) + ( frustum->getSide( 3 )[1] *frustum->getSide( 3 )[1] ));
...
void ZSceneDatabase::addShadowNodes( SceneRenderer &renderer )
{
	...
	if( octree )
	{
		culledCount = octree->getShadowList( culledList, staticCount, planeCount, planes );
	}
