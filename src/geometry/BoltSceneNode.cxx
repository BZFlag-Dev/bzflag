...
void BoltSceneNode::BoltRenderNode::renderGeoGMBolt()
{
    ...
    auto finColor  = glm::vec4(glm::vec3(noseColor.r) * 0.5f, 1.0f);
...
void            BoltSceneNode::BoltRenderNode::render()
{
    ...
    if (blackFog)
        glFogfv(GL_FOG_COLOR, glm::value_ptr(glm::vec4(0.0f)));
    ...
    if (drawBillboardShot)
    {
        ...
        if (sceneNode->texturing)
        {
            ...
            if ((shotLength > 0) && (sceneNode->length > 1.0e-6f))
            {
                ...
                auto vel = glm::vec3(
                        sceneNode->velocity[0],
                        sceneNode->velocity[1],
                        sceneNode->velocity[2]);
