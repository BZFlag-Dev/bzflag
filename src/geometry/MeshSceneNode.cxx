...
inline int MeshSceneNode::calcNormalLod( const ViewFrustum &vf )
{
	...
	const float dist = ( d[0]*( s[0] - e[0] )) + ( d[1]*( s[1] - e[1] )) + ( d[2]*( s[2] - e[2] ));
...
void MeshSceneNode::notifyStyleChange()
{
	...
	for( int lod = 0; lod < lodCount; lod++ )
	{
		...
		for( int set = 0; set < lodNode.count; set++ )
		{
			...
			if(( drawSet.triangleCount < 100 ) || !BZDBCache::lighting || mat.bzmat->getNoLighting())
			{
				extPtr = NULL;
			}
			...
			if( !mat.needsSorting )
			{
				setNode.node = new OpaqueRenderNode( drawMgr, &xformList, normalize, mat.colorPtr, lod, set, extPtr, drawSet.triangleCount );
				mat.animRepos = false;
			}
			else
			{
				...
				setNode.node = new AlphaGroupRenderNode( drawMgr, &xformList, normalize, mat.colorPtr, lod, set, extPtr, setPos, drawSet.triangleCount );
				if(( fabsf( drawSet.sphere[0] ) > 0.001f ) && ( fabsf( drawSet.sphere[1] ) > 0.001f ) && ( mat.color[3] != 0.0f ) && ( drawInfo->getAnimationInfo() != NULL ))
				{
					animRepos = true;
					mat.animRepos = true;
				}
			...
			setNode.radarNode = new OpaqueRenderNode( drawMgr, &xformList, normalize, mat.colorPtr, lod, set, extPtr, drawSet.triangleCount );
		}
	}
	return ;
}
...
void MeshSceneNode::updateMaterial( MeshSceneNode::MeshMaterial *mat )
{
	...
	{
		OpenGLMaterial oglMaterial( bzmat->getSpecular(), bzmat->getEmission(), bzmat->getShininess());
...
void MeshSceneNode::getRenderNodes( std::vector < RenderSet >  &rnodes )
{
	...
	if( lodCount > 0 )
	{
		...
		for( int set = 0; set < lodNode.count; set++ )
		{
			...
			RenderSet rs = 
			{
				setNode.node, &setNode.meshMat.gstate
			};
			rnodes.push_back( rs );
		}
	}
...
void MeshSceneNode::setLodScale( int pixelsX, float fovx, int pixelsY, float fovy )
{
