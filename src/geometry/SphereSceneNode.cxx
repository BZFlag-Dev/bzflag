...
SphereLodSceneNode::SphereLodSceneNode( const GLfloat pos[3], GLfloat _radius ): SphereSceneNode( pos, _radius ), renderNode( this )
{
	if( !initialized )
	{
		initialized = true;
...
SphereLodSceneNode::SphereLodRenderNode::SphereLodRenderNode( const SphereLodSceneNode *_sceneNode ): sceneNode( _sceneNode )
{
	return ;
}
...
void SphereLodSceneNode::SphereLodRenderNode::render()
{
	...
	static const GLdouble groundPlane[] = 
	{
		0.0, 0.0, 1.0, 0.0
	};
...
SphereBspSceneNode::SphereBspSceneNode( const GLfloat pos[3], GLfloat _radius ): SphereSceneNode( pos, _radius ), renderNode( this ), parts( NULL ){}
...
SceneNode **SphereBspSceneNode::getParts( int &numParts )
{
	...
	for( i = 0; i < SphereLowRes; i++ )
		if( radius *SphereBspRenderNode::lgeom[SphereLowRes *i][2] + mySphere[2] < 0.01f )
			break;
...
void SphereBspSceneNode::addRenderNodes( SceneRenderer &renderer )
{
	...
	if( BZDBCache::blend )
	{
		...
		renderNode.setBaseIndex( int( float( numSlices ) *( 1.0f + 0.5f * azimuth / M_PI )) % numSlices );
	}
...
GLfloat SphereBspSceneNode::SphereBspRenderNode::geom[NumSlices *( SphereRes + 1 )][3];
GLfloat SphereBspSceneNode::SphereBspRenderNode::lgeom[SphereLowRes *( SphereLowRes + 1 )][3];

SphereBspSceneNode::SphereBspRenderNode::SphereBspRenderNode( const SphereBspSceneNode *_sceneNode ): sceneNode( _sceneNode ), highResolution( false ), baseIndex( 0 )
{
...
void SphereBspSceneNode::SphereBspRenderNode::setHighResolution( bool _highResolution )
{
...
void SphereBspSceneNode::SphereBspRenderNode::setBaseIndex( int _baseIndex )
{
...
SphereFragmentSceneNode::SphereFragmentSceneNode( int _theta, int _phi, SphereBspSceneNode *_parentSphere ): parentSphere( _parentSphere ), renderNode( _parentSphere, _theta, _phi )
{
...
void SphereFragmentSceneNode::move()
{
	...
	setCenter( pSphere[0] + pRadius * vertex[0], pSphere[1] + pRadius * vertex[1], pSphere[2] + pRadius * vertex[2] );
	setRadius(( GLfloat )( 4.0 *M_PI * M_PI * pSphere[3] ) / GLfloat( SphereLowRes *SphereLowRes ));
}
...
SphereFragmentSceneNode::FragmentRenderNode::FragmentRenderNode( const SphereBspSceneNode *_sceneNode, int _theta, int _phi ): sceneNode( _sceneNode ), theta( _theta ), phi( _phi )
{
...
const GLfloat *SphereFragmentSceneNode::FragmentRenderNode::getPosition()const
{
