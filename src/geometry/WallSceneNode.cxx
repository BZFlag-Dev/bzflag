...
WallSceneNode::WallSceneNode(): numLODs( 0 ), elementAreas( NULL ), style( 0 )
{
	noPlane = false;
	dynamicColor = NULL;
	color[3] = 1.0f;
...
void WallSceneNode::setPlane( const GLfloat _plane[4] )
{
	...
	const float n = 1.0f / sqrtf(( _plane[0] *_plane[0] ) + ( _plane[1] *_plane[1] ) + ( _plane[2] *_plane[2] ));
...
bool WallSceneNode::cull( const ViewFrustum &frustum )const
{
	...
	const float eyedot = ( eye[0] *plane[0] ) + ( eye[1] *plane[1] ) + ( eye[2] *plane[2] ) + plane[3];
	if( eyedot <= 0.0f )
	{
		return true;
	}
	...
	for( i = 0; i < planeCount; i++ )
	{
		...
		d[i] = ( mySphere[0] *norm[0] ) + ( mySphere[1] *norm[1] ) + ( mySphere[2] *norm[2] ) + norm[3];
		if( d[i] < 0.0f )
		{
			d2[i] = d[i] *d[i];
			if( d2[i] > mySphere[3] )
...
int WallSceneNode::pickLevelOfDetail( const SceneRenderer &renderer )const
{
	...
	for( int i = 0; i < numLights; i++ )
	{
		...
		GLfloat pd = pos[0] *plane[0] + pos[1] *plane[1] + pos[2] *plane[2] + plane[3];
		...
		GLfloat ld = ( pos[0] - mySphere[0] )*( pos[0] - mySphere[0] ) + ( pos[1] - mySphere[1] )*( pos[1] - mySphere[1] ) + ( pos[2] - mySphere[2] )*( pos[2] - mySphere[2] );
	...
	if( bestLOD > BZDBCache::maxLOD )
		bestLOD = ( int )BZDBCache::maxLOD;
...
{
	const GLfloat d = plane[0] *eye[0] + plane[1] *eye[1] + plane[2] *eye[2] + plane[3];
	return d *d;
}
...
void WallSceneNode::setModulateColor( GLfloat r, GLfloat g, GLfloat b, GLfloat a )
{
	modulateColor[0] = r;
	modulateColor[1] = g;
	modulateColor[2] = b;
...
void WallSceneNode::setLightedModulateColor( const GLfloat *rgba )
{
...
int WallSceneNode::splitWall( const GLfloat *splitPlane, const GLfloat3Array &vertices, const GLfloat2Array &texcoords, SceneNode * &front, SceneNode * &back ) // const
{
	int i;
	...
	if( firstFront != lastBack )
	{
		...
		splitEdge( dists[firstFront], dists[lastBack], vertices[firstFront], vertices[lastBack], texcoords[firstFront], texcoords[lastBack], splitVertex, splitUV );
	...
	{
		GLfloat splitVertex[3], splitUV[2];
		splitEdge( dists[firstBack], dists[lastFront], vertices[firstBack], vertices[lastFront], texcoords[firstBack], texcoords[lastFront], splitVertex, splitUV );
...
void WallSceneNode::splitEdge( float d1, float d2, const GLfloat *p1, const GLfloat *p2, const GLfloat *uv1, const GLfloat *uv2, GLfloat *p, GLfloat *uv ) // const
{
