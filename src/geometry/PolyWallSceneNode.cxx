...
PolyWallSceneNode::Geometry::Geometry( PolyWallSceneNode *_wall, const GLfloat3Array &_vertex, const GLfloat2Array &_uv, const GLfloat *_normal ): wall( _wall ), normal( _normal ), vertex( _vertex ), uv( _uv )
{
	// do nothing
}
...
PolyWallSceneNode::PolyWallSceneNode( const GLfloat3Array &vertex, const GLfloat2Array &uv )
{
	const int count = vertex.getSize();
	...
	for( i = 1; i < count; i++ )
	{
		...
		nLen = myPlane[0] *myPlane[0] + myPlane[1] *myPlane[1] + myPlane[2] *myPlane[2];
	...
	myPlane[3] =  - ( myPlane[0] *vertex[0][0] + myPlane[1] *vertex[0][1] + myPlane[2] *vertex[0][2] );
	setPlane( myPlane );
	...
	for( i = 0; i < count; i++ )
	{
		GLfloat r = ( mySphere[0] - vertex[i][0] )*( mySphere[0] - vertex[i][0] ) + ( mySphere[1] - vertex[i][1] )*( mySphere[1] - vertex[i][1] ) + ( mySphere[2] - vertex[i][2] )*( mySphere[2] - vertex[i][2] );
		if( r > mySphere[3] )
			mySphere[3] = r;
	}
	setSphere( mySphere );
...
int PolyWallSceneNode::split( const float *_plane, SceneNode * &front, SceneNode * &back )const
{
...
void PolyWallSceneNode::addRenderNodes( SceneRenderer &renderer )
{
...
void PolyWallSceneNode::addShadowNodes( SceneRenderer &renderer )
{
