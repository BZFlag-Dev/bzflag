...
bool					Intersect::testCircleCircle(
								const float* o1, float r1,
...
bool					Intersect::testSphereSphere(
								const float* o1, float r1,
								const float* o2, float r2)
{
	const float x = o1[0] - o2[0];
...
float					Intersect::getNormalOrigRect(
								const float* p, float dx, float dy)
{
...
bool					Intersect::testOrigRectCircle(
								float dx, float dy,
...
Ray						Intersect::rayMinusRay(
								const Ray& r1, float t1,
...
float					Intersect::timeRayHitsOrigBox(
...
float					Intersect::timeRayHitsBlock(
								const Ray& r, const float* p1,
								float angle, float dx,
								float dy, float dz)
{
	// translate origin
...
float					Intersect::timeAndSideRayHitsOrigRect(
								const float* p, const float* v,
...
float					Intersect::timeAndSideRayHitsRect(
								const Ray& r,
								const float* p1, float angle,
								float dx, float dy, int& side)
{
...
bool					Intersect::testOrigRectRect(
...
bool					Intersect::testRectRect(
								const float* p1, float angle1,
								float dx1, float dy1,
