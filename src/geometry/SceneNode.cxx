...
void __stdcall SceneNode::noColor4f( GLfloat, GLfloat, GLfloat, GLfloat ){}
...
{
	return ( eye[0] - sphere[0] )*( eye[0] - sphere[0] ) + ( eye[1] - sphere[1] )*( eye[1] - sphere[1] ) + ( eye[2] - sphere[2] )*( eye[2] - sphere[2] );
}
...
int SceneNode::split( const float *, SceneNode * &, SceneNode * & )const
{
	// can't split me
	return 1;
}
...
bool SceneNode::cull( const ViewFrustum &view )const
{
	...
	for( int i = 0; i < planeCount; i++ )
	{
		...
		const GLfloat d = ( sphere[0] *norm[0] ) + ( sphere[1] *norm[1] ) + ( sphere[2] *norm[2] ) + norm[3];
...
GLfloat2Array::GLfloat2Array( const GLfloat2Array &a ): size( a.size )
{
...
GLfloat3Array::GLfloat3Array( const GLfloat3Array &a ): size( a.size )
{
