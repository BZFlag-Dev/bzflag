...
QuadWallSceneNode::Geometry::Geometry( QuadWallSceneNode *_wall, int uCount, int vCount, const GLfloat base[3], const GLfloat uEdge[3], const GLfloat vEdge[3], const GLfloat *_normal, float uOffset, float vOffset, float uRepeats, float vRepeats ): wall( _wall ), style( 0 ), ds( uCount ), dt( vCount ), dsq( uCount / 4 ), dsr( uCount % 4 ), normal( _normal ), vertex(( uCount + 1 )*( vCount + 1 )), uv(( uCount + 1 )*( vCount + 1 ))
{
...
QuadWallSceneNode::QuadWallSceneNode( const GLfloat base[3], const GLfloat uEdge[3], const GLfloat vEdge[3], float uOffset, float vOffset, float uRepeats, float vRepeats, bool makeLODs )
{
...
QuadWallSceneNode::QuadWallSceneNode( const GLfloat base[3], const GLfloat uEdge[3], const GLfloat vEdge[3], float uRepeats, float vRepeats, bool makeLODs )
{
...
void QuadWallSceneNode::init( const GLfloat base[3], const GLfloat uEdge[3], const GLfloat vEdge[3], float uOffset, float vOffset, float uRepeats, float vRepeats, bool makeLODs )
{
	...
	myPlane[3] =  - ( myPlane[0] *base[0] + myPlane[1] *base[1] + myPlane[2] *base[2] );
	setPlane( myPlane );
	mySphere[0] = 0.5f *( uEdge[0] + vEdge[0] );
	mySphere[1] = 0.5f *( uEdge[1] + vEdge[1] );
	mySphere[2] = 0.5f *( uEdge[2] + vEdge[2] );
	mySphere[3] = mySphere[0] *mySphere[0] + mySphere[1] *mySphere[1] + mySphere[2] *mySphere[2];
	mySphere[0] += base[0];
	mySphere[1] += base[1];
	mySphere[2] += base[2];
	setSphere( mySphere );
	...
	// get length of sides
	const float uLength = sqrtf( float( uEdge[0] *uEdge[0] + uEdge[1] *uEdge[1] + uEdge[2] *uEdge[2] ));
	const float vLength = sqrtf( float( vEdge[0] *vEdge[0] + vEdge[1] *vEdge[1] + vEdge[2] *vEdge[2] ));
	...
	vElements = 1;
	areas[level] = area;
	nodes[level++] = new Geometry( this, uElements, vElements, base, uEdge, vEdge, getPlane(), uOffset, vOffset, uRepeats, vRepeats );
	shadowNode = new Geometry( this, uElements, vElements, base, uEdge, vEdge, getPlane(), uOffset, vOffset, uRepeats, vRepeats );
...
int QuadWallSceneNode::split( const float *_plane, SceneNode * &front, SceneNode * &back )const
{
