...
TankSceneNode::TankSceneNode( const GLfloat pos[3], const GLfloat forward[3] ): leftTreadOffset( 0.0f ), rightTreadOffset( 0.0f ), leftWheelOffset( 0.0f ), rightWheelOffset( 0.0f ), useDimensions( false ), useOverride( false ), onlyShadows( false ), clip( false ), inTheCockpit( false ), tankRenderNode( this ), shadowRenderNode( this ), tankSize( TankGeometryEnums::Normal )
{
...
void TankSceneNode::addRenderNodes( SceneRenderer &renderer )
{
	...
	const float size = mySphere[3] *( view.getAreaFactor() / getDistance( view.getEye()));
	...
	bool narrow = false;
	if(( tankSize == Narrow ) && ( !useDimensions || ( dimensions[1] < 0.01f )) && BZDBCache::animatedTreads && BZDBCache::zbuffer )
	{
		narrow = true;
	}
...
void TankSceneNode::addLight( SceneRenderer &renderer )
{
	...
	{
		// the real light
		jumpJetsRealLight.setColor( jumpJetsScale *1.5f * 2.0f, jumpJetsScale *1.0f * 2.0f, jumpJetsScale *0.5f * 2.0f );
		renderer.addLight( jumpJetsRealLight );
		// the ground lights
		for( int i = 0; i < 4; i++ )
		{
			jumpJetsGroundLights[i].setColor( jumpJetsLengths[i] *1.5f * 0.5f, jumpJetsLengths[i] *1.0f * 0.5f, jumpJetsLengths[i] *0.5f * 0.5f );
...
void TankSceneNode::setJumpJets( float scale )
{
	...
	if(( scale > 0.0f ) && BZDBCache::zbuffer && BZDBCache::texture )
	{
		...
		for( int i = 0; i < 4; i++ )
		{
			...
			const float v[2] = 
			{
				jm[0] *scaleFactor[0], jm[1] *scaleFactor[1]
			};
...
TankIDLSceneNode::TankIDLSceneNode( const TankSceneNode *_tank ): tank( _tank ), renderNode( this )
{
	static const GLfloat defaultPlane[4] = 
	{
		1.0f, 0.0f, 0.0f, 0.0f
	};
	move( defaultPlane );
...
void TankIDLSceneNode::move( const GLfloat _plane[4] )
{
	...
	setCenter( s[0] + 1.5f * BZDBCache::tankLength *plane[0], s[1] + 1.5f * BZDBCache::tankLength *plane[1], s[2] + 1.5f * BZDBCache::tankLength *plane[2] );
	return ;
}
...
TankIDLSceneNode::IDLRenderNode::IDLRenderNode( const TankIDLSceneNode *_sceneNode ): sceneNode( _sceneNode )
{
	// do nothing
	return ;
}
...
{
	static const GLfloat innerColor[4] = 
	{
		1.0f, 1.0f, 1.0f, 0.75f
	};
	static const GLfloat outerColor[4] = 
	{
		1.0f, 1.0f, 1.0f, 0.0f
	};
	...
	plane[2] = _plane[2];
	plane[3] = ( sphere[0] *_plane[0] + sphere[1] *_plane[1] + sphere[2] *_plane[2] + _plane[3] );
	...
	for( int i = 0; i < numFaces; i++ )
	{
		...
		for( j = 0; j < numVertices; j++ )
			d[j] = idlVertex[face[j]][0] *plane[0] + idlVertex[face[j]][1] *plane[1] + idlVertex[face[j]][2] *plane[2] + plane[3];
		...
		for( j = 0, k = numVertices - 1; j < numVertices; k = j++ )
		{
			if(( d[k] < 0.0f && d[j] >= 0.0f ) || ( d[k] >= 0.0f && d[j] < 0.0f ))
			{
				...
				cross[crossings][0] = ( 1.0f - t ) *idlVertex[face[k]][0] + t *idlVertex[face[j]][0];
				cross[crossings][1] = ( 1.0f - t ) *idlVertex[face[k]][1] + t *idlVertex[face[j]][1];
				cross[crossings][2] = ( 1.0f - t ) *idlVertex[face[k]][2] + t *idlVertex[face[j]][2];
				if( ++crossings == 2 )
					break;
			}
		}
...
TankSceneNode::TankRenderNode::centerOfGravity[LastTankPart][3] = 
{
	...
	,  // barrel
	{
		0.125f, 0.0f, 2.5f *0.68f
	}

	,  // turret
	{
		0.000f,  + 0.7f, 0.5f *0.68f
	}
...
TankSceneNode::TankRenderNode::TankRenderNode( const TankSceneNode *_sceneNode ): sceneNode( _sceneNode ), isShadow( false ), above( false ), towards( false )
{
...
void TankSceneNode::TankRenderNode::sortOrder( bool _above, bool _towards, bool _left )
{
	above = _above;
	towards = _towards;
	left = _left;
...
void TankSceneNode::TankRenderNode::render()
{
	...
	const bool switchLights = BZDBCache::lighting && !isShadow && ( drawLOD == HighTankLOD );
	if( switchLights )
	{
...
void TankSceneNode::TankRenderNode::renderPart( TankPart part )
{
	...
	if( isExploding )
	{
		...
		glTranslatef( cog[0] + ( explodeFraction *vel[0] ), cog[1] + ( explodeFraction *vel[1] ), cog[2] + ( explodeFraction *vel[2] ));
...
void TankSceneNode::TankRenderNode::setupPartColor( TankPart part )
{
	...
	switch( part )
	{
		...
		default:
			{
				// avoid warnings about unused enumerated values
				break;
			}
	}
	return ;
...
void TankSceneNode::TankRenderNode::renderLights()
{
	...
	glBegin( GL_POINTS );
	{
		...
		myColor3fv( lights[0] );
		glVertex3f( lights[0][3] *scale[0], lights[0][4] *scale[1], lights[0][5] *scale[2] );
		myColor3fv( lights[1] );
		glVertex3f( lights[1][3] *scale[0], lights[1][4] *scale[1], lights[1][5] *scale[2] );
		myColor3fv( lights[2] );
		glVertex3f( lights[2][3] *scale[0], lights[2][4] *scale[1], lights[2][5] *scale[2] );
	}
	glEnd();
