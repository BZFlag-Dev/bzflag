...
BillboardSceneNode::BillboardSceneNode( const GLfloat pos[3] ): show( false ), hasAlpha( false ), hasTexture( false ), hasTextureAlpha( false ), looping( false ), lightSource( false ), width( 1.0f ), height( 1.0f ), cu( 1 ), cv( 1 ), t( 0.0f ), duration( 0.0f ), renderNode( this )
{
...
void BillboardSceneNode::setFrame()
{
	...
	else
	{
		...
		if( frame >= cu *cv )
			frame = cu * cv - 1;
		renderNode.setFrame(( float )( frame % cu ) / ( float )cu, ( float )( frame / cu ) / ( float )cv );
	}
}
...
void BillboardSceneNode::prepLight()
{
	if( !lightSource )
		return ;
	const float s = ( t <= lightCutoffTime || lightCutoffTime >= duration ) ? 1.0f : ( 1.0f - ( t - lightCutoffTime ) / ( duration - lightCutoffTime ));
	light.setColor( lightColor[0] *lightScale * s, lightColor[1] *lightScale * s, lightColor[2] *lightScale * s );
}
...
BillboardSceneNode::BillboardRenderNode::BillboardRenderNode( const BillboardSceneNode *_sceneNode ): sceneNode( _sceneNode )
{
	setFrame( 0.0f, 0.0f );
...
void BillboardSceneNode::BillboardRenderNode::render()
{
	...
	glPushMatrix();
	{
		glTranslatef( sphere[0] + d * dir[0], sphere[1] + d * dir[1], sphere[2] + d * dir[2] );
