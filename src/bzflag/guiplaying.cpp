...
// access silencePlayers from bzflag.cpp
...
{
#if defined(FREEZING)
  if ((key.unicode == '`' && pressed) && (!haveBinding && key.modifiers)) {
  ...
  if (!haveBinding &&
	...
	PlayerId  to;
	if ((key.modifiers == BzfKeyEvent::ControlKey) && world->allowTeams()) {
...
static void doMotion()
{
  ...
  if (myTank->getFlagType() == Flags::ReverseControls) {
    speed  = -speed;
    angVel = -angVel;
  }
...
static int bzModsToEventMods(int mods)
{
  int eMods = 0;
  if (mods & BzfKeyEvent::AltKey)     { eMods |= EventHandler::AltKeyModBit;   }
  if (mods & BzfKeyEvent::ControlKey) { eMods |= EventHandler::CtrlKeyModBit;  }
  if (mods & BzfKeyEvent::ShiftKey)   { eMods |= EventHandler::ShiftKeyModBit; }
  // NOTE - EventHandler::RepeatKeyModBit is never being set
  return eMods;
}
...
static bool handleEvent(const BzfEvent& event)
{
  ...
  switch (event.type) {
    case BzfEvent::KeyUp: {
      const BzfKeyEvent& kEv = event.keyUp;
      switch (kEv.button) {
        ...
        case BzfKeyEvent::WheelDown: {
          return false; // only send the KeyDown version
        }
        ...
        case BzfKeyEvent::MouseButton10: {
          ...
          const int button = kEv.button - BzfKeyEvent::LeftMouse;
        ...
        default: {
          ...
          if (!HUDui::getFocus() && (kEv.button != 27)) {
            return eventHandler.KeyRelease(false, keysym, mods);
          }
          else {
    ...
    case BzfEvent::KeyDown: {
      const BzfKeyEvent& kEv = event.keyDown;
      switch (kEv.button) {
        case BzfKeyEvent::F12: {
          return false;
        }
        ...
        default: {
          ...
          if (!HUDui::getFocus() && (kEv.button != 27)) {
            ...
            // the HUDui already owns the event, so taken = true
            eventHandler.KeyPress(true, keysym, mods); // ignored return value
            if (kEv.unicode) {
              eventHandler.UnicodeText(true, kEv.unicode); // ignored return value
            }
            return false;
          }
        }
...
void handleKilledMessage(void *msg, bool human, bool &checkScores)
{
  ...
  if (killerLocal) {
	...
	if (myTank->isAutoPilot()) {
	  if (killerPlayer) {
	    ...
	    if (shot != NULL)
	      teachAutoPilot(shot->getFlagType(), 1);
	  }
	}
      }
    }
  ...
  if (World::getWorld()->allowTeams()) {
	...
	if (shot && shot->getFlagType() == Flags::Genocide &&
...
void handleFlagTransferred(Player *fromTank, Player *toTank, int flagIndex, ShotType shotType)
{
  ...
  if (f.type->flagTeam != ::NoTeam) {
    if ((toTank->getTeam() == myTank->getTeam()) &&
        (f.type->flagTeam != myTank->getTeam())) {
      SOUNDSYSTEM.play(SFX_TEAMGRAB, pos, false, false);
    } else if ((fromTank->getTeam() == myTank->getTeam()) &&
               (f.type->flagTeam == myTank->getTeam())) {
      hud->setAlert(1, "Flag Alert!!!", 3.0f, true);
      SOUNDSYSTEM.play(SFX_ALERT);
    }
...
void handleMessage(void *msg)
{
  ...
  // the text is good to go, trigger the event
  eventHandler.RecvChatMsg((char*)msg, src, dst, type == ActionMessage);

  // do a little more filtering
...
#define QUERY_COMPARE(x) if (query == #x) { queryID = x; }

  // string queries
  QUERY_COMPARE(GL_VENDOR);
  QUERY_COMPARE(GL_VERSION);
  QUERY_COMPARE(GL_RENDERER);
  QUERY_COMPARE(GL_EXTENSIONS);
  QUERY_COMPARE(GL_SHADING_LANGUAGE_VERSION);
  if (queryID != 0) {
  ...
  // integer queries
  QUERY_COMPARE(GL_MAX_LIGHTS);
  QUERY_COMPARE(GL_MAX_ATTRIB_STACK_DEPTH);
  QUERY_COMPARE(GL_MAX_CLIENT_ATTRIB_STACK_DEPTH);
  QUERY_COMPARE(GL_MAX_MODELVIEW_STACK_DEPTH);
  QUERY_COMPARE(GL_MAX_PROJECTION_STACK_DEPTH);
  QUERY_COMPARE(GL_MAX_TEXTURE_STACK_DEPTH);
  QUERY_COMPARE(GL_MAX_TEXTURE_SIZE);
  QUERY_COMPARE(GL_MAX_TEXTURE_UNITS);
  QUERY_COMPARE(GL_MAX_TEXTURE_COORDS);
  QUERY_COMPARE(GL_MAX_TEXTURE_IMAGE_UNITS);
  QUERY_COMPARE(GL_MAX_VERTEX_TEXTURE_IMAGE_UNITS);
  QUERY_COMPARE(GL_MAX_COMBINED_TEXTURE_IMAGE_UNITS);
  QUERY_COMPARE(GL_MAX_ELEMENTS_VERTICES);
  QUERY_COMPARE(GL_MAX_ELEMENTS_INDICES);
  QUERY_COMPARE(GL_MAX_DRAW_BUFFERS);
  QUERY_COMPARE(GL_MAX_SAMPLES);
  QUERY_COMPARE(GL_RED_BITS);
  QUERY_COMPARE(GL_BLUE_BITS);
  QUERY_COMPARE(GL_GREEN_BITS);
  QUERY_COMPARE(GL_ALPHA_BITS);
  QUERY_COMPARE(GL_DEPTH_BITS);
  QUERY_COMPARE(GL_STENCIL_BITS);
  if (queryID != 0) {
    GLint value;
			...
			const ShotPath *hit, int phydrv)
{
  ...
  if (flag != Flags::Null) {
    if (myTank->isAutoPilot())
      teachAutoPilot(myTank->getFlagType(), -1);
...
static void checkEnvironment()
{
  ...
  if (myTank->onSolidSurface()) {
    ...
    FlagType* flagd = myTank->getFlagType();
  ...
  if (hit) {
    ...
    gotBlowedUp(myTank, PhysicsDriverDeath, ServerPlayer,
                NULL, myTank->getDeathPhysicsDriver());
  }
  else {
...
bool inLockRange(float angle, float distance, float bestDistance, RemotePlayer *player)
{
  if (player->isPaused() || player->isNotResponding() ||
      player->getFlagType() == Flags::Stealth)
...
bool inLookRange(float angle, float distance, float bestDistance, RemotePlayer *player)
{
  ...
  if (player->getFlagType() == Flags::Stealth ||
      player->getFlagType() == Flags::Cloaking)
    return myTank->getFlagType() == Flags::Seer;
...
void setLookAtMarker(void)
{
  ...
  if (bestTarget->getFlagType() != Flags::Null) {
    std::string flagName = bestTarget->getFlagType()->flagAbbv;
...
void setTarget()
{
  ...
  for (int i = 0; i < curMaxPlayers; i++) {
    ...
    if (a < BZDB.eval(BZDBNAMES.LOCKONANGLE) &&	// about 8.5 degrees
      ((myTank->getFlagType() == Flags::GuidedMissile) ||		// am i locking on?
      tankHasShotType(myTank, Flags::GuidedMissile)) &&
      remotePlayers[i]->getFlagType() != Flags::Stealth &&		// can't lock on stealth
	...
	lockedOn = true;
      ...
      ((remotePlayers[i]->getFlagType() != Flags::Stealth) ||
       (myTank->getFlagType() == Flags::Seer)) && // can't "see" stealth unless have seer
  ...
  if (lockedOn) {
    ...
    if (!forbidIdentify) {
      ...
      if (bestTarget->getFlagType() != Flags::Null) {
        msg += ") with ";
        msg += bestTarget->getFlagType()->flagName;
      } else {
        msg += ")";
      }
    }
  ...
  else if (myTank->getFlagType() == Flags::Colorblindness) {
...
static void setHuntTarget()
{
  ...
  if (bestTarget->isHunted() &&
      (myTank->getFlagType() != Flags::Blindness) &&
      (myTank->getFlagType() != Flags::Colorblindness) &&
      (bestTarget->getFlagType() != Flags::Stealth)) {
...
static void drawUI()
{
  ...
  if (GfxBlockMgr::cursor.notBlocked()) {
    renderRoamMouse();
  }
...
static void addDynamicSceneNodes()
{
  ...
  const bool seerView = (myTank->getFlagType() == Flags::Seer);
  const bool showTreads = bzdbShowTreads ||
                          thirdPersonVars.b3rdPerson;
  ...
  for (int i = 0; i < curMaxPlayers; i++) {
    if (remotePlayers[i]) {
      const bool colorblind = (myTank->getFlagType() == Flags::Colorblindness);
      remotePlayers[i]->addShots(scene, colorblind);

      TeamColor effectiveTeam = RogueTeam;
      if (!colorblind) {
        if ((remotePlayers[i]->getFlagType() == Flags::Masquerade)
      ...
      if ((myTank->getFlagType() == Flags::Seer) || showPlayer) {
...
void drawFrame(const float dt)
{
  ...
  if (!myTank) {
    ...
    if (myTank->getFlagType() == Flags::WideAngle) {
      fov *= 2.0f;
    }
...
static void updatePauseCountdown(float dt)
{
  ...
  if (pauseCountdown > 0.0f) {
	...
	const FlagType *flagd = myTank->getFlagType();
	if (flagd->flagTeam != NoTeam) {
	  serverLink->sendDropFlag(myTank->getPosition());
	  myTank->setShotType(StandardShot);
	}
...
void doTankMotions(const float /*dt*/)
{
  ...
  if (myTank) {
	...
	setHuntTarget(); //spot hunt target
      ...
      if (!myTank->isObserver() &&
	  ((fireButton && myTank->getFlagType() == Flags::MachineGun) ||
	   (myTank->getFlagType() == Flags::TriggerHappy))) {
	myTank->fireShot();
      }
...
void startPlaying()
{
  ...
  explosions.clear();
  for (size_t e = 0; e < prototypeExplosions.size(); e++) {
    delete prototypeExplosions[e];
  }
