...
{
	if( key.button == BzfKeyEvent::PageUp || key.button == BzfKeyEvent::PageDown || key.ascii == 13 )
		return true;
...
HelpMenu::HelpMenu( const char *title ): HUDDialog()
{
	...
	listHUD.push_back( createLabel( "Page Down for next page", "Page Up for previous page" ));
...
HUDuiControl *HelpMenu::createLabel( const char *string, const char *label )
{
...
public:
	...
	static void onScanCB( const std::string &name, bool press, const std::string &cmd, void *userData );
...
void Help1Menu::onScan( const std::string &name, bool press, const std::string &cmd )
{
	if( !press )
		return ;
...
void Help1Menu::onScanCB( const std::string &name, bool press, const std::string &cmd, void *userData )
{
...
Help5Menu::Help5Menu(): HelpMenu( "Good Flags" )
{
	...
	for( FlagSet::iterator it = fs.begin(); it != fs.end(); it++ )
	{
		...
		if((( *it )->flagQuality != FlagGood ) || (( *it )->flagTeam != NoTeam ) || ( strcmp(( *it )->flagName, "" ) == 0 ))
		{
			continue;
		}
...
Help6Menu::Help6Menu(): HelpMenu( "Bad Flags" )
{
	...
	for( FlagSet::iterator it = fs.begin(); it != fs.end(); it++ )
	{
		...
		if((( *it )->flagQuality != FlagBad ) || (( *it )->flagTeam != NoTeam ) || ( strcmp(( *it )->flagName, "" ) == 0 ))
		{
			continue;
		}
