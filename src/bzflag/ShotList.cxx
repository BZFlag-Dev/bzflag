...
{
    auto itr = std::find_if(GlobalShotList.begin(), GlobalShotList.end(), [&](const ShotPath::Ptr& shot)
    {
        return shot->getFiringInfo().shot.id == shotID;
    });
...
ShotPath::Vec GetShotsForPlayer(PlayerId player)
{
    ...
    std::copy_if(GlobalShotList.begin(), GlobalShotList.end(),
                 std::back_inserter(playerShots), [&](const ShotPath::Ptr& shot)
    {
        return shot->getPlayer() == player;
    });
...
void HandleShotUpdate(int shotID, const ShotUpdate& update, uint16_t code, const void* msg)
{
    ...
    auto itr = std::find_if(GlobalShotList.begin(), GlobalShotList.end(), [&](const ShotPath::Ptr& shot)
    {
        return shot != nullptr ? shot->getFiringInfo().shot.id == shotID : false;
    });
    if (itr != GlobalShotList.end()
            && *itr == nullptr)      // ignore bogus shots (those with a bad index or for shots that don't exist)
        return ;
