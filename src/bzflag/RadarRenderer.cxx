...
RadarRenderer::RadarRenderer( const SceneRenderer &, World *_world ): world( _world ), x( 0 ), y( 0 ), w( 0 ), h( 0 ), dimming( 0.0f ), decay( 0.01f ), jammed( false ), multiSampled( false )
{
...
void RadarRenderer::render( SceneRenderer &renderer, bool blank, bool observer )
{
	...
	glOrtho(  - xCenter * xUnit, ( xSize - xCenter ) *xUnit,  - yCenter * yUnit, ( ySize - yCenter ) *yUnit,  - ( maxHeight + 10.0 ), ( maxHeight + 10.0 ));
	...
	if( jammed && ( bzfrand() > decay ))
	{
		...
		if(( noiseTexture >= 0 ) && ( renderer.useQuality() > _LOW_QUALITY ))
		{
			...
			static float np[] = 
			{
				0, 0, 1, 1, 1, 1, 0, 0, 0.5f, 0.5f, 1.5f, 1.5f, 1.5f, 1.5f, 0.5f, 0.5f, 0.25f, 0.25f, 1.25f, 1.25f, 1.25f, 1.25f, 0.25f, 0.25f, 0, 0.5f, 1, 1.5f, 1, 1.5f, 0, 0.5f, 0.5f, 0, 1.5f, 1, 1.4f, 1, 0.5f, 0, 0.75f, 0.75f, 1.75f, 1.75f, 1.75f, 1.75f, 0.75f, 0.75f, 
			};
	...
	else if( myTank )
	{
		...
		for( i = 0; i < curMaxPlayers; i++ )
		{
			...
			{
				glColor3fv( Team::getRadarColor( myTank->getFlag() == Flags::Colorblindness ? RogueTeam : player->getTeam()));
			}
		...
		// draw antidote flag
		const float *antidotePos = LocalPlayer::getMyTank()->getAntidoteLocation();
		if( antidotePos )
		{
	...
	if( world->allowTeamFlags())
	{
		for( i = 1; i < NumTeams; i++ )
		{
			for( int j = 0;; j++ )
			{
				...
				glVertex2f( base[0] + r * cosf( base[3] + beta ), base[1] + r * sinf( base[3] + beta ));
				glVertex2f( base[0] + r * cosf(( float )( base[3] - beta + M_PI )), base[1] + r * sinf(( float )( base[3] - beta + M_PI )));
				glVertex2f( base[0] + r * cosf(( float )( base[3] + beta + M_PI )), base[1] + r * sinf(( float )( base[3] + beta + M_PI )));
				glVertex2f( base[0] + r * cosf( base[3] - beta ), base[1] + r * sinf( base[3] - beta ));
				glEnd();
			}
