...
template <> Roaming *Singleton<Roaming>::_instance = ( Roaming* )0;

Roaming::Roaming(): view( roamViewDisabled ), targetManual(  - 1 ), targetWinner(  - 1 ), targetFlag(  - 1 )
{
	resetCamera();
}
	...
	else
	{
		...
		else
		{
			...
			for( i = 0; i < world->getCurMaxPlayers(); ++i )
			{
				...
				if(( j ==  - 1 ) || ( world->getPlayer( j ) && ( world->getPlayer( j )->getTeam() != ObserverTeam )))
				{
...
void Roaming::buildRoamingLabel( void )
{
	...
	if( world && tracked )
	{
		...
		switch( view )
		{
			...
			case roamViewFlag:
				{
					roamingLabel = std::string( "Tracking " ) + world->getFlag( targetFlag ).type->flagName + " Flag";
					break;
				}
			default:
				{
