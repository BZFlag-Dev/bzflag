...
public:
	enum State
	{
		Okay = 0, SocketError = 1, Rejected = 2, BadVersion = 3, Hungup = 4,  // only used by Winsock
		CrippledVersion = 5, Refused = 6
	};

	enum Abilities
	{
		Nothing = 0, CanDoUDP = 1, SendScripts = 2, SendTextures = 4, HasMessageLink = 8
	};
	...
	void sendEnter( PlayerId, PlayerType, TeamColor, const char *name, const char *email, const char *token );
	...
	#ifdef HAVE_KRB5
	void sendKerberosTicket( const char *principal, const krb5_data *ticket );
	#endif 
	void sendKilled( const PlayerId victim, const PlayerId shooter, int reason, int shotId, const FlagType *flag, int phydrv );
	...
	void sendMessage( const PlayerId &to, char message[MessageLen] );
