...
ServerLink::ServerLink( const Address &serverAddress, int port ): state( SocketError ),  // assume failure
fd(  - 1 ),  // assume failure
udpLength( 0 ), oldNeedForSpeed( false ), previousFill( 0 )
{
	int i;
	...
	if( p )
		setsockopt( fd, p->p_proto, TCP_NODELAY, ( SSOType ) &off, sizeof( off ));
	// changed
...
void ServerLink::flush()
{
...
#endif 
			sendto( urecvfd, ( const char* )txbuf, previousFill, 0,  &usendaddr, sizeof( usendaddr ));
	...
	else
	{
		...
		if( r == SOCKET_ERROR )
		{
			...
			if( e == WSAENETRESET || e == WSAECONNABORTED || e == WSAECONNRESET || e == WSAETIMEDOUT )
				state = Hungup;
			r = 0;
		}
#endif 
...
void ServerLink::send( uint16_t code, uint16_t len, const void *msg )
{
	...
	if(( needForSpeed != oldNeedForSpeed ) || ( previousFill + len + 4 > MaxPacketLen ))
		flush();
...
int ServerLink::read( uint16_t &code, uint16_t &len, void *msg, int blockTime )
{
	...
	if(( urecvfd >= 0 ) /* && ulinkup */ )
	{
		...
		if( UDEBUGMSG )
			printError( "Fallback to normal TCP receive" );
		len = 0;
		code = MsgNull;
...
void ServerLink::sendEnter( PlayerId _id, PlayerType type, TeamColor team, const char *name, const char *email, const char *token )
{
	if( state != Okay )
		return ;
...
bool ServerLink::readEnter( std::string &reason, uint16_t &code, uint16_t &rejcode )
{
	// wait for response
	uint16_t len;
	char msg[MaxPacketLen];
...
void ServerLink::sendKilled( const PlayerId victim, const PlayerId killer, int reason, int shotId, const FlagType *flagType, int phydrv )
{
...
void ServerLink::sendEndShot( const PlayerId &source, int shotId, int reason )
{
...
void ServerLink::sendHit( const PlayerId &source, const PlayerId &shooter, int shotId )
{
	char msg[80];
	void *buf = msg;
	buf = nboPackUByte( buf, source );
...
void ServerLink::sendUDPlinkRequest()
{
	if(( server_abilities &CanDoUDP ) != CanDoUDP )
		return ;
	// server does not support udp (future list server test)
...
#ifdef HAVE_KRB5
void ServerLink::sendKerberosTicket( const char *principal, const krb5_data *ticket )
{
	char msg[MaxPacketLen];
	void *buf = msg;
