...
ControlPanelMessage::ControlPanelMessage( const std::string &_string ): string( _string ), numlines( 0 ){}
...
void ControlPanelMessage::breakLines( float maxLength, int fontFace, float fontSize )
{
	...
	while( lineLen > 0 )
	{
		...
		{
			n = 0;
			while(( n < lineLen ) && ( fm.getStrLength( fontFace, fontSize, std::string( msg, n + 1 )) < maxLength ))
			{
				if( msg[n] == ESC_CHAR )
				{
					// clear the cumulative codes when we hit a reset
					// the reset itself will start the new cumulative string.
					if(( strncmp( msg + n, ANSI_STR_RESET, strlen( ANSI_STR_RESET )) == 0 ) || ( strncmp( msg + n, ANSI_STR_RESET_FINAL, strlen( ANSI_STR_RESET_FINAL )) == 0 ))
						cumulativeANSICodes = "";
					...
					if(( n < lineLen ) && ( msg[n] == '[' ))
					{
						cumulativeANSICodes += msg[n];
						n++;
						while(( n < lineLen ) && (( msg[n] == ';' ) || (( msg[n] >= '0' ) && ( msg[n] <= '9' ))))
						{
							cumulativeANSICodes += msg[n];
							n++;
						}
...
ControlPanel::ControlPanel( MainWindow &_mainWindow, SceneRenderer &_renderer ): tabsOnRight( true ), tabs( NULL ), totalTabWidth( 0 ), window( _mainWindow ), resized( false ), numBuffers( 2 ), changedMessage( 0 ), radarRenderer( NULL ), renderer( &_renderer ), fontFace( 0 ), dimming( 1.0f ), du( 0 ), dv( 0 ), messageMode( MessageAll )
{
	setControlColor();
...
void ControlPanel::render( SceneRenderer &_renderer )
{
	...
	int ay = ( _renderer.getPanelOpacity() == 1.0f || !showTabs ) ? 0: int( lineHeight + 4 );

	glScissor( x + messageAreaPixels[0] - 1, y + messageAreaPixels[1], messageAreaPixels[2] + 1, messageAreaPixels[3] + ay );
	OpenGLGState::resetState();
	...
	if( _renderer.getPanelOpacity() > 0.0f )
	{
		...
		messageAreaPixels[1], messageAreaPixels[0] + messageAreaPixels[2], messageAreaPixels[1] + messageAreaPixels[3] );
		...
		if( showTabs )
		{
			...
			for( unsigned int tab = 0; tab < tabs->size(); tab++ )
			{
				...
				if( tabsOnRight )
				{
					...
					glRecti( messageAreaPixels[0] + messageAreaPixels[2] - totalTabWidth + drawnTabWidth, messageAreaPixels[1] + messageAreaPixels[3] - int( lineHeight + 4 ) + ay, messageAreaPixels[0] + messageAreaPixels[2] - totalTabWidth + drawnTabWidth + int( tabTextWidth[tab] ),  //+ drawnTabWidth + int(tabTextWidth[tab]),
					messageAreaPixels[1] + messageAreaPixels[3] + ay );
				}
				else
				{
					// draw the tabs on the left side
					glRecti( messageAreaPixels[0] + drawnTabWidth, messageAreaPixels[1] + messageAreaPixels[3] - int( lineHeight + 4 ) + ay, messageAreaPixels[0] + drawnTabWidth + int( tabTextWidth[tab] ), messageAreaPixels[1] + messageAreaPixels[3] + ay );
				}
	...
	if( messagesOffset > 0 && messageMode >= 0 )
	{
		...
		if( lines > 0 )
		{
			...
			glRecti( messageAreaPixels[0], messageAreaPixels[1] + int( offset *( float )messageAreaPixels[3] ), messageAreaPixels[0] + 2, top );
	...
	if( showTabs )
	{
		...
		for( unsigned int tab = 0; tab < tabs->size(); tab++ )
		{
			...
			if( tabsOnRight )
			{
				...
				fm.drawString( messageAreaPixels[0] + messageAreaPixels[2] - totalTabWidth + drawnTabWidth + floorf( fontSize ), messageAreaPixels[1] + messageAreaPixels[3] - floorf( lineHeight + 2.0f ) + ay, 0.0f, fontFace, ( float )fontSize, ( *tabs )[tab] );
			}
			else
			{
				// draw the tabs on the left side (with one letter padding)
				fm.drawString( messageAreaPixels[0] + drawnTabWidth + floorf( fontSize ), messageAreaPixels[1] + messageAreaPixels[3] - floorf( lineHeight + 2.0f ) + ay, 0.0f, fontFace, ( float )fontSize, ( *tabs )[tab] );
			}
			drawnTabWidth += long( tabTextWidth[tab] );
		}
	}
	...
	glScissor( x + messageAreaPixels[0], y + messageAreaPixels[1], messageAreaPixels[2], messageAreaPixels[3] - ( showTabs ? int( lineHeight + 4 ): 0 ) + ay );
	...
	glScissor( x + messageAreaPixels[0] - 2, y + messageAreaPixels[1] - 2, messageAreaPixels[2] + 3, messageAreaPixels[3] + 33 );
	OpenGLGState::resetState();
	...
	glBegin( GL_POINTS );
	{
		...
		glVertex2f(( float )( x + messageAreaPixels[0] + messageAreaPixels[2] ), ( float )( y + messageAreaPixels[1] - 1 ));
		glVertex2f(( float )( x + messageAreaPixels[0] + messageAreaPixels[2] ), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] ));
		glVertex2f(( float )( x + messageAreaPixels[0] - 1 ), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] ));
		...
		for( int tab = 0; tab < ( int )tabs->size(); tab++ )
		{
			if( messageMode == MessageModes( tab ))
			{
				if( tabsOnRight )
				{
					glVertex2f(( float )( x + messageAreaPixels[0] + messageAreaPixels[2] - totalTabWidth + tabPosition ), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] + ay ));
					glVertex2f(( float )( x + messageAreaPixels[0] + messageAreaPixels[2] - totalTabWidth + tabPosition + long( tabTextWidth[tab] )), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] + ay ));
				}
				else
				{
					glVertex2f(( float )( x + messageAreaPixels[0] + tabPosition ), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] + ay ));
					glVertex2f(( float )( x + messageAreaPixels[0] + tabPosition + long( tabTextWidth[tab] )), ( float )( y + messageAreaPixels[1] + messageAreaPixels[3] + ay ));
				}
			}
...
void ControlPanel::resize()
{
	...
	if( radarRenderer )
		radarRenderer->setShape( radarAreaPixels[0], radarAreaPixels[1], radarAreaPixels[2], radarAreaPixels[3] );
...
void ControlPanel::addMessage( const std::string &line, const int realmode )
{
...
void ControlPanel::saveMessages( const std::string &filename, bool stripAnsi )const
{
