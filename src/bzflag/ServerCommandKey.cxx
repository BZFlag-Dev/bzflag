...
const ServerCommandKey::Mode ServerCommandKey::nonAdminModes[8] = 
{
	LagStats, IdleStats, FlagHistory, Report, Password, Register, Identify, ClientQuery
};
...
void ServerCommandKey::updatePrompt()
{
	...
	if( mode >= Kick && mode <= Ghost )
	{
		// more complicated modes here
		if( recipient )
		{
		...
		{
			// no recipient -- we are in editing mode -- kick or ban
	...
	{
		// not kick or ban stuff -- should be less complicated
		...
		switch( mode )
		{
			...
			case Report:
				...
				break;
			case Register:
				hud->setComposing( "Register your nick [enter pass]:", true );
				break;
			case Identify:
				hud->setComposing( "Login [enter pass]:", true );
				break;
			case Setpass:
				hud->setComposing( "Set your password [enter pass]:", true );
				break;
			case Grouplist:
...
bool ServerCommandKey::keyPress( const BzfKeyEvent &key )
{
	...
	{
		if(( key.button == BzfKeyEvent::Up ) || ( key.button == BzfKeyEvent::Down ) || ( key.button == BzfKeyEvent::Left ) || ( key.button == BzfKeyEvent::Right ))
			return true;
	}
	...
	if( sendIt )
	{
		...
		if( mode >= Kick && mode <= Ghost )
		{
			// handle more complicated modes
			if( troll )
			{
				// cases where we select recipient with keys
		...
		else
		{
			...
			switch( mode )
			{
				...
				case Report:
					...
					break;
				case Identify:
					sendMsg = "/identify " + message;
					break;
				case Setpass:
					sendMsg = "/setpass " + message;
					break;
				case Grouplist:
					sendMsg = "/grouplist";
					break;
