...
public:
	...
	enum InputMethod
	{
		// what device am I using to move around
		Keyboard = 0, Mouse, Joystick
	};
	...
	const Obstacle *getHitBuilding( const float *pos, float angle, bool phased, bool &expel )const;
	const Obstacle *getHitBuilding( const float *oldPos, float oldAngle, const float *pos, float angle, bool phased, bool &expel );
