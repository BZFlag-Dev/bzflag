...
void *WorldBuilder::unpack( void *buf )
{
	...
	if( uncompress(( Bytef* )uncompressedWorld, &destLen, ( Bytef* )compressedWorld, compressedSize ) != Z_OK )
	{
	...
	if( world->gameStyle &ClassicCTFGameStyle )
	{
		...
		for( i = 0; i < bases.size(); i++ )
		{
			...
			setBase(( TeamColor )base->getTeam(), base->getPosition(), base->getRotation(), base->getWidth(), base->getBreadth(), base->getHeight());
		}
	}
	else
	{
...
void WorldBuilder::setBase( TeamColor team, const float *pos, float rotation, float w, float b, float h )
{
