		...
		if( now - lastShot >= ( 1.0f / World::getWorld()->getMaxShots()))
		{
			...
			for( int t = 0; t < curMaxPlayers; t++ )
			{
				if( t != myTank->getId() && player[t] && player[t]->isAlive() && !player[t]->isPaused() && !player[t]->isNotResponding() && myTank->validTeamTarget( player[t] ))
				{
					...
					if( player[t]->isPhantomZoned() && !myTank->isPhantomZoned() && ( myTank->getFlag() != Flags::SuperBullet ))
						continue;
...
TopLevelPlan::TopLevelPlan(): Plan( 0 ){}
...
GotoPointPlan::GotoPointPlan( float *pt ): Plan( 20.0f )
{
...
WeavePlan::WeavePlan( int pID, bool right ): Plan( 10.0 )
{
	playerID = pID;
	weaveRight = right;
}
...
HuntPlayerPlan::HuntPlayerPlan(): Plan( 300.0f )
{
...
CaptureFlagPlan::CaptureFlagPlan(): Plan( 1200.0f ){}
