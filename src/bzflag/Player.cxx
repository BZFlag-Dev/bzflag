...
Player::Player( const PlayerId &_id, TeamColor _team, const char *name, const char *_email, const PlayerType _type ): lastObstacle( NULL ), handicap( 0.0f ), notResponding( false ), hunted( false ), id( _id ), admin( false ), registered( false ), verified( false ), playerList( false ), lastVisualTeam( NoTeam ), team( _team ), type( _type ), flagType( Flags::Null ), fromTeleporter( 0 ), toTeleporter( 0 ), teleporterProximity( 0.0f ), wins( 0 ), losses( 0 ), tks( 0 ), localWins( 0 ), localLosses( 0 ), localTks( 0 ), autoPilot( false ), deltaTime( 0.0 ), offset( 0.0 ), deadReckoningState( 0 ), oldStatus( 0 ), oldZSpeed( 0.0f )
{
...
{
	return (( float )localWins - localLosses ) / (( localWins + localLosses > 5 ) ? localWins + localLosses: 5 );
}
...
void Player::setExplode( const TimeKeeper &t )
{
	if( !isAlive())
		return ;
	explodeTime = t;
	setStatus(( getStatus() | short( PlayerState::Exploding ) | short( PlayerState::Falling )) &~( short( PlayerState::Alive ) | short( PlayerState::Paused )));
...
void Player::updateTrackMarks()
{
	...
	if( isAlive() && !isFalling() && !isPhantomZoned())
	{
		...
		if( lifeTime > TrackMarks::updateTime )
		{
			...
			const float dist = dimensions[0]*(( fullLength - treadHeight ) / fullLength );
			...
			if( drawMark )
			{
				TrackMarks::addMark( markPos, dimensionsScale[1], state.azimuth, state.phydrv );
				lastTrackDraw = TimeKeeper::getCurrent();
			}
		}
	}
...
void Player::updateDimensions( float dt, bool local )
{
	...
	if(( dimensionsScale[0] == dimensionsTarget[0] ) && ( dimensionsScale[1] == dimensionsTarget[1] ) && ( dimensionsScale[2] == dimensionsTarget[2] ))
	{
		useDimensions = false;
	}
	else
...
bool Player::hitObstacleResizing()
{
	...
	const ObsList *olist = COLLISIONMGR.boxTest( getPosition(), getAngle(), dims[0], dims[1], dims[2] );

	for( int i = 0; i < olist->count; i++ )
	{
		const Obstacle *obs = olist->list[i];
		const bool onTop = obs->isFlatTop() && (( obs->getPosition()[2] + obs->getHeight()) <= getPosition()[2] );
		if( !obs->isDriveThrough() && !onTop && obs->inBox( getPosition(), getAngle(), dims[0], dims[1], dims[2] ))
		{
			return true;
		}
	}
...
void Player::setVisualTeam( TeamColor visualTeam )
{
	...
	static const GLfloat tankSpecular[3] = 
	{
		0.1f, 0.1f, 0.1f
	};
	static GLfloat tankEmissive[3] = 
	{
		0.0f, 0.0f, 0.0f
	};
	static float tankShininess = 20.0f;
	static GLfloat rabbitEmissive[3] = 
	{
		0.0f, 0.0f, 0.0f
	};
...
void Player::addToScene( SceneDatabase *scene, TeamColor effectiveTeam, bool inCockpit, bool seerView, bool showTreads, bool showIDL )
{
	...
	if( isAlive())
	{
		...
		if( isCrossingWall())
		{
			...
			const Obstacle *obstacle = World::getWorld()->hitBuilding( state.pos, a, dimensions[0], dimensions[1], dimensions[2] );
			if( obstacle && obstacle->isCrossing( state.pos, a, dimensions[0], dimensions[1], dimensions[2], plane ) || World::getWorld()->crossingTeleporter( state.pos, a, dimensions[0], dimensions[1], dimensions[2], plane ))
			{
	...
	{
		float t = float(( TimeKeeper::getTick() - explodeTime ) / BZDB.eval( StateDatabase::BZDB_EXPLODETIME ));
		if( t > 1.0f )
		{
	...
	{
		pausedSphere->move( state.pos, 1.5f *BZDBCache::tankRadius *dimensionsScale[0] );
...
void Player::getDeadReckoning( float *predictedPos, float *predictedAzimuth, float *predictedVel, float dt )const
{
	*predictedAzimuth = inputAzimuth;
	...
	else if( inputStatus &PlayerState::Falling )
	{
		...
		predictedPos[2] = inputPos[2] + ( inputVel[2] *dt ) + ( 0.5f *BZDBCache::gravity *dt * dt );
	}
	else
	{
...
bool Player::isDeadReckoningWrong()const
{
	...
	if(( fabsf( state.pos[0] - predictedPos[0] ) > positionTolerance ) || ( fabsf( state.pos[1] - predictedPos[1] ) > positionTolerance ) || ( fabsf( state.pos[2] - predictedPos[2] ) > positionTolerance ))
	{
		if( debugLevel >= 4 )
		{
