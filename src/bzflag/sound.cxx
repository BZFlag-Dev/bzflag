...
#define	SFX_COUNT		countof(soundFiles)
...
void					openSound(const char*)
{
	...
	// initialize buffers
	for (unsigned int i = 0; i < SFX_COUNT; i++) {
	...
	// watch for volume changes and get current volume
...
static bool				allocAudioSamples()
{
	...
	for (unsigned int i = 0; i < SFX_COUNT; i++) {
		// read it
		int numFrames, rate;
		float* samples = MediaFile::readSound(soundFiles[i], &numFrames, &rate);
		if (samples != NULL) {
			if (resampleAudio(samples, numFrames, rate, soundSamples + i))
				anyFile = true;
			delete[] samples;
		}
	}
...
static void				sendSound(SoundCommand* s)
{
	if (!usingAudio)
		return;
	MPLATFORM->writeSoundCommand(s, sizeof(SoundCommand));
}
...
static void				onVolumeChange(const BzfString& name, void*)
{
	if (name == "audioVolume")
		...
		setSoundMute(BZDB->isTrue("audioMute"));
}
...
static PlatformMediaFactory*	media = NULL;
