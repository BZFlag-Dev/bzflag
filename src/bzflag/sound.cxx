...
static int resampleAudio( const float *in, int frames, int rate, AudioSamples *out );
...
{
	"fire", "explosion", "ricochet", "flag_grab", "flag_drop", "flag_won", "flag_lost", "flag_alert", "jump", "land", "teleport", "laser", "shock", "pop", "explosion", "flag_grab", "boom", "killteam", "phantom", "missile", "lock", "teamgrab", "hunt", "hunt_select", "steamroller", "thief", "burrow", "message_private", "message_team", "message_admin", "flap", "bounce"
};
...
void openSound( const char* )
{
	...
	{
		fadeIn[i] = fadeIn[i + 1] = sinf(( float )( M_PI / 2.0 *( double )i / ( double )( FadeDuration - 2 )));
...
static int resampleAudio( const float *in, int frames, int rate, AudioSamples *out )
{
	...
	const int safetyMargin = ( int )( 1.5f + InterAuralDistance / SpeedOfSound * ( float )PlatformFactory::getMedia()->getAudioOutputRate());

	// fill in samples structure
	out->length = 2 * frames;
	out->mlength = out->length >> 1;
	out->dmlength = double( out->mlength - 1 );
	out->duration = ( float )out->mlength / ( float )PlatformFactory::getMedia()->getAudioOutputRate();
...
void moveSoundReceiver( float x, float y, float z, float t, int discontinuity )
{
	if( soundLevel <= 0 )
	{
		return ;
...
void playSound( int soundCode, const float pos[3], bool important, bool localSound )
{
	if( localSound )
	{
...
void playWorldSound( int soundCode, const float pos[3], bool important )
{
	if( soundLevel <= 0 )
	{
		return ;
	...
	SoundCommand s;
	if(( int )soundSamples.size() <= soundCode )
		return ;
	if( soundSamples[soundCode].length == 0 )
		return ;
...
void playFixedSound( int soundCode, float x, float y, float z )
{
	if( soundLevel <= 0 )
	{
		return ;
...
{
	if(( e->flags &SEF_FIXED ) || !( e->flags &SEF_WORLD ))
		return 0;
	...
	else
	{
		...
		if( e->flags &SEF_IGNORING )
		{
			...
			e->ptrFracLeft = timeFromFront * ( float )PlatformFactory::getMedia()->getAudioOutputRate();
			...
			if( !positionDiscontinuity && timeFromFront < 0.0f )
				timeFromFront = 0.0f;
			e->ptrFracRight = timeFromFront * ( float )PlatformFactory::getMedia()->getAudioOutputRate();
	...
	if( !mutingOn && numSamples != 0 )
	{
		...
		if( numSamples <= FadeDuration )
		{
			for( n = 0; n < numSamples; n += 2 )
			{
				...
				scratch[n] += src[n]*( fadeIn[fs] *volumeAtten + fadeOut[fs] *e->lastLeftAtten );
				scratch[n + 1] += src[n + 1]*( fadeIn[fs] *volumeAtten + fadeOut[fs] *e->lastRightAtten );
			}
		}
		else
		{
			...
			{
				scratch[n] += src[n]*( fadeIn[n] *volumeAtten + fadeOut[n] *e->lastLeftAtten );
				scratch[n + 1] += src[n + 1]*( fadeIn[n] *volumeAtten + fadeOut[n] *e->lastRightAtten );
			}
			if( volumeAtten == 1.0f )
			{
...
static int addWorldContribution( SoundEvent *e, long *len )
{
	...
	for( n = 0; !fini && n < FadeDuration; n += 2 )
	{
		...
		scratch[n] += fsampleL *( fadeIn[n] *leftAtten + fadeOut[n] *e->lastLeftAtten );
		scratch[n + 1] += fsampleR *( fadeIn[n] *rightAtten + fadeOut[n] *e->lastRightAtten );
