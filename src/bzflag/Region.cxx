...
BzfRegion::BzfRegion( int sides, const float p[][2] ): mailbox( 0 ), target( 0 ), A( 0.0, 0.0 )
{
...
bool BzfRegion::isInside( const float p[2] )const
{
	...
	const float *p2 = NULL;
	for( size_t i = 0; i < count; p1 = p2, i++ )
	{
		p2 = corners[i].get();
		...
		else
		{
			...
			if(( p[1] - p1[1] )*( p2[0] - p1[0] ) / ( p2[1] - p1[1] ) >= p[0] - p1[0] + tolerance )
				inside = !inside;
		}
	}
	return inside;
...
int BzfRegion::classify( const float e1[2], const float e2[2] )const
{
	...
	if( toRight + onEdge == ( int )count )
		return 1;
	// all to right
	if( toRight != 0 )
		return 0;
	// not all to left -- split
...
BzfRegion *BzfRegion::orphanSplitRegion( const float e1[2], const float e2[2] )
{
	...
	for( i = 0; split < 2 && i < count; i++ )
	{
		...
		if( lastSign != newSign )
		{
			...
			edge[split] = i;
			etsplit[split] = (( p1[1] - p2[1] )*( p1[0] - e1[0] ) + ( p2[0] - p1[0] )*( p1[1] - e1[1] )) / delta;
			split++;
		}
		lastPVect = pVect;
		lastSign = newSign;
	...
	if( split != 2 || ( etsplit[0] <= 0.0 && etsplit[1] <= 0.0 ) || ( etsplit[0] >= 1.0 && etsplit[1] >= 1.0 ))
		return 0;
	...
	RegionPoint n2( p1[0] + tsplit[1]*( p2[0] - p1[0] ), p1[1] + tsplit[1]*( p2[1] - p1[1] ));
	p1 = corners[edge[0]].get();
	p2 = corners[( edge[0] + 1 ) % count].get();
	RegionPoint n1( p1[0] + tsplit[0]*( p2[0] - p1[0] ), p1[1] + tsplit[0]*( p2[1] - p1[1] ));
	...
	// cutting edge.
	{
		...
		while( true )
		{
			...
			if( i == edge[1] )
				break;
			if( neighbors[i] )
				neighbors[i]->setNeighbor( this, newRegion );
		}
...
void BzfRegion::splitEdge( const BzfRegion *oldNeighbor, BzfRegion *newNeighbor, const RegionPoint &p, bool onRight )
{
...
void BzfRegion::setNeighbor( const BzfRegion *oldNeighbor, BzfRegion *newNeighbor )
{
	const size_t count = corners.size();
	for( size_t i = 0; i < count; i++ )
	if( neighbors[i] == oldNeighbor )
...
void BzfRegion::setPathStuff( float _distance, BzfRegion *_target, const float _a[2], int mailboxIndex )
{
	distance = _distance;
	target = _target;
	A = RegionPoint( _a );
