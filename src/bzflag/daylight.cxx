...
static double getGreenwichSideral( double julianDay )
{
	...
	const double greenwichMidnight = fmod(( 0.00002581 *T + 2400.051262 ) *T + 6.6460656, 24.0 );
...
static void gettruePosition( double julianDay, float latitude, float longitude, double sx, double sy, double sz, float pos[3] )
{
	// get local sidereal time
	const float localSidereal = ( float )( getGreenwichSideral( julianDay ) - longitude *M_PI / 180.0 );
...
void getCelestialTransform( double julianDay, float latitude, float longitude, float xform[4][4] )
{
	// get local sidereal time
	const float localSidereal = ( float )( getGreenwichSideral( julianDay ) - longitude *M_PI / 180.0 );
...
void getSunPosition( double julianDay, float latitude, float longitude, float pos[3] )
{
	double T = ( julianDay - epoch ) / 36525.0;
	double geometricMeanLongitude = radPerDeg * (( 0.0003025 *T + 36000.76892 ) *T + 279.69668 );
	geometricMeanLongitude = fmod( geometricMeanLongitude, 2.0 *M_PI );

	double meanAnomaly = radPerDeg *( 358.47583 + (( 0.0000033 *T + 0.000150 ) *T + 35999.04975 ) *T );
	meanAnomaly = fmod( meanAnomaly, 2.0 *M_PI );

	//  double eccentricity = 0.01675104 +
	//		T * (-0.000000126 * T - 0.0000418);

	double C = radPerDeg * ( sin( meanAnomaly )*( 1.919460 - ( 0.004789 + 0.000014 * T ) *T ) + sin( 2.0 *meanAnomaly )*( 0.020094 - 0.0001 * T ) + sin( 3.0 *meanAnomaly ) *0.000293 );
	C = fmod( C, 2.0 *M_PI );
	...
	double obliquity = radPerDeg * ( 23.452294 + (  - 0.0130125 + (  - 0.00000164 + 0.000000503 *T ) *T ) *T );
...
void getMoonPosition( double julianDay, float latitude, float longitude, float pos[3] )
{
	double T = ( julianDay - epoch ) / 36525.0;
	double e = 1.0 + (  - 0.002495 - 0.00000752 * T ) *T;
	double meanLongitude = radPerDeg * ( 270.434164 + ( 481267.8831 + (  - 0.001133 + 0.0000019 * T ) *T ) *T );
	meanLongitude = fmod( meanLongitude, 2.0 *M_PI );

	double meanAnomaly = radPerDeg * ( 296.104608 + ( 477198.8491 + ( 0.009192 + 0.0000144 * T ) *T ) *T );
	meanAnomaly = fmod( meanAnomaly, 2.0 *M_PI );

	double meanElongation = radPerDeg * ( 350.737486 + ( 445267.1142 + (  - 0.001436 + 0.0000019 * T ) *T ) *T );
	meanElongation = fmod( meanElongation, 2.0 *M_PI );
	double meanElongation2 = 2.0 * meanElongation;

	double distFromAscendingNode = radPerDeg * ( 11.250889 + ( 483202.0251 + (  - 0.003211 - 0.0000003 * T ) *T ) *T );
	distFromAscendingNode = fmod( distFromAscendingNode, 2.0 *M_PI );

	// get sun's meanAnomaly
	double solMeanAnomaly = radPerDeg *( 358.47583 + (( 0.0000033 *T + 0.000150 ) *T + 35999.04975 ) *T );
	solMeanAnomaly = fmod( solMeanAnomaly, 2.0 *M_PI );

	// get moon's geocentric latitude and longitude
	double geocentricLongitude = meanLongitude + radPerDeg * ( 6.288750 *sin( meanAnomaly ) + 1.274018 *sin( meanElongation2 - meanAnomaly ) + 0.658309 *sin( meanElongation2 ) + 0.213616 *sin( 2.0 *meanAnomaly ) +  - 0.185596 * sin( solMeanAnomaly ) *e+ - 0.114336 * sin( 2.0 *distFromAscendingNode ));
	double geocentricLatitude = radPerDeg * ( 5.128189 *sin( distFromAscendingNode ) + 0.280606 *sin( meanAnomaly + distFromAscendingNode ) + 0.277693 *sin( meanAnomaly - distFromAscendingNode ) + 0.173238 *sin( meanElongation2 - distFromAscendingNode ) + 0.055413 *sin( meanElongation2 + distFromAscendingNode - meanAnomaly ) + 0.046272 *sin( meanElongation2 - distFromAscendingNode - meanAnomaly ) + 0.032573 *sin( meanElongation2 + distFromAscendingNode ) + 0.017198 *sin( 2.0 *meanAnomaly + distFromAscendingNode ));
	...
	double sx, sy, sz;
	sx = cos( geocentricLatitude ) *sin( geocentricLongitude ) *cos( obliquity ) - sin( geocentricLatitude ) *sin( obliquity );
	sy = sin( geocentricLatitude ) *cos( obliquity ) + cos( geocentricLatitude ) *sin( geocentricLongitude ) *sin( obliquity );
...
static void lerpColor( GLfloat out[3], const GLfloat t0[3], const GLfloat t1[3], float t )
{
...
void getSunColor( const float sunDir[3], GLfloat color[3], GLfloat ambient[3], GLfloat &brightness )
{
	static const GLfloat highSunColor[3] = 
	{
		1.75f, 1.75f, 1.4f
	};
	static const GLfloat lowSunColor[3] = 
	{
		0.75f, 0.27f, 0.0f
	};
	static const GLfloat moonColor[3] = 
	{
		0.4f, 0.4f, 0.4f
	};
	static const GLfloat nightAmbient[3] = 
	{
		0.3f, 0.3f, 0.3f
	};
	static const GLfloat dayAmbient[3] = 
	{
		0.35f, 0.5f, 0.5f
	};
	...
	{
		const GLfloat t = ( sunDir[2] - dawnElevation ) / ( dayElevation - dawnElevation );
	...
	{
		const GLfloat t = ( sunDir[2] - duskElevation ) / ( dayElevation - duskElevation );
...
{
	static const GLfloat nightColor[3] = 
	{
		0.04f, 0.04f, 0.08f
	};
	static const GLfloat zenithColor[3] = 
	{
		0.25f, 0.55f, 0.86f
	};
	static const GLfloat horizonColor[3] = 
	{
		0.43f, 0.75f, 0.95f
	};
	static const GLfloat sunrise1Color[3] = 
	{
		0.30f, 0.12f, 0.08f
	};
	static const GLfloat sunrise2Color[3] = 
	{
		0.47f, 0.12f, 0.08f
	};
	...
	{
		// twilight
		const float t = ( sunDir[2] - nightElevation ) / ( twilightElevation - nightElevation );
		sky[0][0] = nightColor[0];
		sky[0][1] = nightColor[1];
		sky[0][2] = nightColor[2];
	...
	{
		// sunrise or sunset
		const float t = ( sunDir[2] - twilightElevation ) / ( dawnElevation - twilightElevation );
		sky[0][0] = nightColor[0];
		sky[0][1] = nightColor[1];
		sky[0][2] = nightColor[2];
	...
	else if( sunDir[2] < dayElevation )
	{
		...
		const float t = ( sunDir[2] - dawnElevation ) / ( dayElevation - dawnElevation );
