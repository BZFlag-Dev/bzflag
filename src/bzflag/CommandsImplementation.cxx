...
bool CommandList::operator()( const char * /*cmdLine*/ )
{
	...
	addMessage( NULL, ANSI_STR_UNDERLINE ANSI_STR_FG_BLACK "Client-side Commands" );

	const char *prefix = ANSI_STR_FG_YELLOW ANSI_STR_PULSATING "[CLIENT->] " ANSI_STR_RESET;
	...
	addMessage( NULL, ANSI_STR_UNDERLINE ANSI_STR_FG_BLACK "Server-side Commands" );
...
static void listSetVars( const std::string &name, void *varDispPtr )
{
	...
	if(( serverVar && varDisp->server ) || ( !serverVar && varDisp->client ))
	{
		...
		{
			sprintf( message, "%s %s%s %s%f %s%s", varDisp->prefix.c_str(), ColorStrings[RedColor].c_str(), name.c_str(), ColorStrings[GreenColor].c_str(), BZDB.eval( name ), ColorStrings[BlueColor].c_str(), BZDB.get( name ).c_str());
		}
		else
		{
			sprintf( message, "%s %s <%f> %s", name.c_str(), varDisp->prefix.c_str(), BZDB.eval( name ), BZDB.get( name ).c_str());
		}
...
bool DiffCommand::operator()( const char *commandLine )
{
	...
	if( !foundVar )
	{
		...
		{
			addMessage( LocalPlayer::getMyTank(), "no differing variables with that pattern", 2 );
		}
	}
	return true;
}
...
bool LocalSetCommand::operator()( const char *commandLine )
{
	...
	if( tokens.size() == 1 )
	{
		...
		if( !debug && (( strstr( tokens[0].c_str(), "*" ) != NULL ) || ( strstr( tokens[0].c_str(), "?" ) != NULL )))
		{
		...
		if( !foundVar )
		{
			if( debug )
			{
				addMessage( LocalPlayer::getMyTank(), "no matching client variables", 2 );
			}
			else
			{
				addMessage( LocalPlayer::getMyTank(), "undefined client variable", 2 );
...
bool RoamPosCommand::operator()( const char *commandLine )
{
	...
	if( tokens.size() == 1 )
	{
		...
		else if( TextUtils::tolower( tokens[0] ) == "send" )
		{
			...
			if( myTank != NULL )
			{
				...
				float fakeVel[3] = 
				{
					camPtr->theta, camPtr->phi, camPtr->zoom
				};
	...
	{
		addMessage( NULL, "/roampos  ""[ \"reset\" | degrees | {x y z [theta [phi [ zoom ]]] } | \"send\" ]" );

		const Roaming::RoamingCamera *cam = ROAM.getCamera();
		char buffer[MessageLen];
		snprintf( buffer, MessageLen, "  <%.3f, %.3f, %.3f> theta = %.3f, phi = %.3f, zoom = %.3f", cam->pos[0], cam->pos[1], cam->pos[2], cam->theta, cam->phi, cam->zoom );
		addMessage( NULL, buffer );
	}
...
bool SaveWorldCommand::operator()( const char *commandLine )
{
	...
	{
		snprintf( buffer, 256, "World saved:  %s %s%s%s", fullname.c_str(), meshprims ? " [meshprims]" : "", ungrouped ? " [ungrouped]" : "", wavefront ? " [wavefront]" : "" );
	}
	else
	{
