...
template <> EffectsRenderer *Singleton<EffectsRenderer>::_instance = ( EffectsRenderer* )0;

// utils for geo
static void drawRingYZ( float rad, float z, float topsideOffset = 0, float bottomUV = 0, float ZOffset = 0, float topUV = 1.0f, int segments = 32 );
static void drawRingXY( float rad, float z, float topsideOffset = 0, float bottomUV = 0, float topUV = 1.0f, int segments = 32 );
...
void RingSpawnEffect::drawRing( int n, float coreAlpha )
{
	...
	if( age > ( ringRange *( n - 1 )))
	// this ring in?
	{
		if( age < ringRange *n )
		// the ring is still coming in
		{
...
static void drawRingXY( float rad, float z, float topsideOffset, float bottomUV, float topUV, int segments )
{
...
static void drawRingYZ( float rad, float z, float topsideOffset, float bottomUV, float ZOffset, float topUV, int segments )
{
	for( int i = 0; i < segments; i++ )
	{
		float thisAng = 360.0f / segments * i;
