...
{
	NULL, "Got shot by ", "Got flattened by ", "Team flag was captured by ", "Teammate hit with Genocide by ", "Tank Self Destructed", "Tank Rusted"
};
static bool gotBlowedUp( BaseLocalPlayer *tank, BlowedUpReason reason, PlayerId killer, const ShotPath *hit = NULL, int physicsDriver =  - 1 );
...
void selectNextRecipient( bool forward, bool robotIn )
{
	...
	while( true )
	{
		...
		if( i == rindex )
			break;
		if( player[i] && ( robotIn || player[i]->getPlayerType() != ComputerPlayer ))
		{
...
{
	return grabMouseAlways && !unmapped && ( myTank == NULL || !myTank->isPaused() || myTank->isAutoPilot());
}
...
{
	return (( tank != NULL ) && ( tank == LocalPlayer::getMyTank()) || ( ROAM.isRoaming() && ( ROAM.getMode() == Roaming::roamViewFP ) && ( ROAM.getTargetTank() == tank )));
}
...
bool setVideoFormat( int index, bool test )
{
	...
	// ignore if no change
	if( display->getResolution() == index )
		return true;
	...
	if( test )
		testVideoFormatTimer = testDuration;
	else if( shouldGrabMouse())
...
static void doKeyPlaying( const BzfKeyEvent &key, bool pressed, bool haveBinding )
{
	...
	if( key.ascii == 0 && key.button >= BzfKeyEvent::F1 && key.button <= BzfKeyEvent::F10 && ( key.shift &( BzfKeyEvent::ControlKey + BzfKeyEvent::AltKey )) != 0 && !haveBinding )
	{
...
static void doKey( const BzfKeyEvent &key, bool pressed )
{
	...
	if( HUDui::getFocus())
	{
		if(( pressed && HUDui::keyPress( key )) || ( !pressed && HUDui::keyRelease( key )))
		{
			return ;
		}
	}
...
static void doEvent( BzfDisplay *disply )
{
	...
	switch( event.type )
	{
		case BzfEvent::Quit: CommandsStandard::quit();
		break;

		case BzfEvent::Redraw: mainWindow->getWindow()->callExposeCallbacks();
		sceneRenderer->setExposed();
		break;

		case BzfEvent::Resize: if( mainWindow->getWidth() != event.resize.width || mainWindow->getHeight() != event.resize.height )
		{
		...
		// already paused.
		if( !pausedByUnmap && ( pauseCountdown == 0.0f ) && myTank && myTank->isAlive() && !myTank->isPaused() && !myTank->isAutoPilot() && !BZDB.isTrue( "noUnmapPause" ))
		{
			// handy for testing
		...
		case BzfEvent::KeyUp: doKey( event.keyUp, false );
		break;

		case BzfEvent::KeyDown: doKey( event.keyDown, true );
		break;

		case BzfEvent::MouseMove: if( myTank && myTank->isAlive() && ( myTank->getInputMethod() != LocalPlayer::Mouse ) && ( BZDB.isTrue( "allowInputChange" )))
...
void addMessage( const Player *_player, const std::string &msg, int mode, bool highlight, const char *oldColor )
{
...
static Player *addPlayer( PlayerId id, void *msg, int showMessage )
{
	...
	// add player
	if( PlayerType( type ) == TankPlayer || PlayerType( type ) == ComputerPlayer || PlayerType( type ) == ChatPlayer )
	{
		player[i] = new RemotePlayer( id, TeamColor( team ), callsign, email, PlayerType( type ));
...
static void printIpInfo( const Player *_player, const Address &addr, const std::string note )
{
	if( _player == NULL )
	{
		return ;
	...
	if( BZDBCache::colorful )
		message += ColorStrings[CyanColor];
	message += "\t from: ";
	if( BZDBCache::colorful )
		message += colorStr;
	message += addrStr;
...
static bool removePlayer( PlayerId id )
{
	...
	while(( playerIndex >= 0 ) && ( playerIndex + 1 == curMaxPlayers ) && ( player[playerIndex] == NULL ))
	{
		playerIndex--;
		curMaxPlayers--;
	}
...
int curlProgressFunc( void * /*clientp*/, double dltotal, double dlnow, double /*ultotal*/, double /*ulnow*/ )
{
	...
	if( display->isEventPending())
	{
		if( display->peekEvent( event ))
		{
			switch( event.type )
			{
				case BzfEvent::Quit: return 1; // terminate the curl call
				case BzfEvent::KeyDown: display->getEvent( event ); // flush the event
				if( event.keyDown.ascii == 27 )
				{
					return 1; // terminate the curl call
				}
				break;
				case BzfEvent::KeyUp: display->getEvent( event ); // flush the event
				break;
				case BzfEvent::MouseMove: display->getEvent( event ); // flush the event
				break;
				case BzfEvent::Unset: case BzfEvent::Map: case BzfEvent::Unmap: case BzfEvent::Redraw: case BzfEvent::Resize: 
...
void WorldDownLoader::start( char *hexDigest )
{
	...
	{
		HUDDialogStack::get()->setFailedMessage(( "Loading world from " + worldUrl ).c_str());
...
void WorldDownLoader::finalization( char *data, unsigned int length, bool good )
{
	if( good )
	{
		...
		{
			std::ostream *cache = FILEMGR.createDataOutStream( worldCachePath, true, true );
			if( cache != NULL )
			{
...
static void dumpMissingFlag( char *buf, uint16_t len )
{
	...
	HUDDialogStack::get()->setFailedMessage( TextUtils::format( "Flags not supported by this client: {1}",  &args ).c_str());
}
...
static bool processWorldChunk( void *buf, uint16_t len, int bytesLeft )
{
	...
	HUDDialogStack::get()->setFailedMessage( TextUtils::format( "Downloading World (%2d%% complete/%d kb remaining)...", ( 100 *doneSize / totalSize ), bytesLeft / 1024 ).c_str());
	return bytesLeft == 0;
}
...
static void handleServerMessage( bool human, uint16_t code, uint16_t len, void *msg )
{
	...
	switch( code )
	{
		...
		case MsgGrabFlag:
			{
				...
				else
				{
					...
					if( fd->flagTeam != NoTeam && fd->flagTeam != tank->getTeam() && (( tank && ( tank->getTeam() == myTank->getTeam()))) && ( Team::isColorTeam( myTank->getTeam())))
					{
		...
		case MsgHandicap:
			{
				...
				for( uint8_t s = 0; s < numHandicaps; s++ )
				{
					...
					if( sPlayer )
					{
						...
						float normalizedHandicap = float( handicap ) / BZDB.eval( StateDatabase::BZDB_HANDICAPSCOREDIFF );
		...
		case MsgMessage:
			{
				...
				if( fromServer )
				{
					...
					if( !strncmp(( char* )msg, passwdRequest, strlen( passwdRequest )))
					{
						...
						{
							TextUtils::format( "%s@%s:%d", startupInfo.callsign, startupInfo.serverName, startupInfo.serverPort ), TextUtils::format( "%s:%d", startupInfo.serverName, startupInfo.serverPort ), TextUtils::format( "%s@%s", startupInfo.callsign, startupInfo.serverName ), TextUtils::format( "%s", startupInfo.serverName ), TextUtils::format( "%s", startupInfo.callsign ), "@" // catch-all for all callsign/server/ports
						}
						;
						...
						for( size_t j = 0; j < countof( passwdKeys ); j++ )
						{
							if( BZDB.isSet( passwdKeys[j] ))
							{
								char messageBuffer[MessageLen];
								memset( messageBuffer, 0, MessageLen );
								std::string passwdResponse = "/identify " + BZDB.get( passwdKeys[j] );
								addMessage( 0, ( "Autoidentifying with password stored for " + passwdKeys[j] ).c_str(), 2, false );
				...
				if( toAll || toAdmin || srcPlayer == myTank || dstPlayer == myTank || dstTeam == myTank->getTeam())
				{
					// message is for me
					std::string colorStr;
					if( srcPlayer == NULL )
					...
					bool isAction = false;
					if(( text[0] == '*' ) && ( text[1] == ' ' ) && ( text[text.size() - 1] == '*' ) && ( text[text.size() - 2] == '\t' ))
					{
						isAction = true;
		...
		case MsgNewPlayer:
			...
			robots[i] = new RobotPlayer( id, callsign, serverLink, myTank->getEmailAddress());
			robots[i]->setTeam( AutomaticTeam );
			serverLink->sendEnter( id, ComputerPlayer, robots[i]->getTeam(), robots[i]->getCallSign(), robots[i]->getEmailAddress(), "" );
			if( !numRobots )
			{
				makeObstacleList();
...
bool addExplosion( const float *_pos, float size, float duration, bool grounded )
{
	// ignore if no prototypes available;
	if( prototypeExplosions.size() == 0 )
		return false;

	// don't show explosions if quality isn't at least medium
	if( sceneRenderer->useQuality() < _MEDIUM_QUALITY )
		return false;
	...
	// turn up the volume
	newExplosion->setLightColor( 1.0f *lightGain, 0.8f *lightGain, 0.5f *lightGain );
	while( boom-- )
	{
		// pick a random prototype explosion
...
void processInputEvents( float maxProcessingTime )
{
	if( mainWindow && display )
	{
		...
		while( display->isEventPending() && !CommandsStandard::isQuit() && ( TimeKeeper::getCurrent() - start < maxProcessingTime ))
		{
			// process one event
			doEvent( display );
		}
...
static bool gotBlowedUp( BaseLocalPlayer *tank, BlowedUpReason reason, PlayerId killer, const ShotPath *hit, int phydrv )
{
	...
	if( reason != GotShot || flag != Flags::Shield )
	{
		...
		if( reason == GotShot || reason == GotRunOver || reason == GenocideEffect || reason == SelfDestruct || reason == WaterDeath || reason == PhysicsDriverDeath )
			serverLink->sendKilled( tank->getId(), killer, reason, shotId, flagType, phydrv );
	}
		...
		else
		{
			...
			if( reason >= GotShot && reason <= GenocideEffect )
			{
				...
				else
				{
					...
					if( hit )
						team = hit->getTeam();
					if(( myTank->getTeam() == team ) && ( team != RogueTeam ) && ( team != ObserverTeam ))
					{
...
void setTarget()
{
	...
	for( int i = 0; i < curMaxPlayers; i++ )
	{
		...
		tankHasShotType( myTank, Flags::GuidedMissile )) && player[i]->getFlag() != Flags::Stealth &&  // can't lock on stealth
...
static void setHuntTarget()
{
	...
	if( bestTarget->isHunted() && myTank->getFlag() != Flags::Blindness && myTank->getFlag() != Flags::Colorblindness && bestTarget->getFlag() != Flags::Stealth )
	{
		if( myTank->getTarget() == NULL )
		{
			// Don't interfere with GM lock display
...
static void addObstacle( std::vector < BzfRegion * >  &rgnList, const Obstacle &obstacle )
{
	...
	for( size_t k = 0; k < numRegions; k++ )
	{
		...
		int side[4];
		if(( side[0] = region->classify( p[0], p[1] )) == 1 || ( side[1] = region->classify( p[1], p[2] )) == 1 || ( side[2] = region->classify( p[2], p[3] )) == 1 || ( side[3] = region->classify( p[3], p[0] )) == 1 )
			continue;
...
static void makeObstacleList()
{
	const float tankRadius = BZDBCache::tankRadius;
	int i;
	for( std::vector < BzfRegion * > ::iterator itr = obstacleList.begin(); itr != obstacleList.end(); ++itr )
		delete ( *itr );
	obstacleList.clear();
...
static void setRobotTarget( RobotPlayer *robot )
{
	...
	if( player[j] && player[j]->getId() != robot->getId() && player[j]->isAlive() && (( robot->getTeam() == RogueTeam ) || player[j]->getTeam() != robot->getTeam()))
	{

		if( player[j]->isPhantomZoned() && !robot->isPhantomZoned())
			continue;

		if( World::getWorld()->allowTeamFlags() && ( robot->getTeam() == RedTeam && player[j]->getFlag() == Flags::RedTeam ) || ( robot->getTeam() == GreenTeam && player[j]->getFlag() == Flags::GreenTeam ) || ( robot->getTeam() == BlueTeam && player[j]->getFlag() == Flags::BlueTeam ) || ( robot->getTeam() == PurpleTeam && player[j]->getFlag() == Flags::PurpleTeam ))
		{
			bestTarget = player[j];
			break;
		}
	...
	if( myTank->isAlive() && (( robot->getTeam() == RogueTeam ) || myTank->getTeam() != robot->getTeam()))
	{
...
static void updateRobots( float dt )
{
	...
	for( i = 0; i < numRobots; i++ )
	{
		if( !gameOver && robots[i] && !robots[i]->isAlive() && !robots[i]->isExploding() && pickTarget )
		{
	...
	for( i = 0; i < numRobots; i++ )
	{
		if( robots[i] && robots[i]->isAlive() && ( pickTarget || !robots[i]->getTarget() || !robots[i]->getTarget()->isAlive()))
		{
...
static void checkEnvironment( RobotPlayer *tank )
{
	...
	else
	{
		...
		if((( myTank->getFlag() == Flags::Steamroller ) || (( tank->getFlag() == Flags::Burrow ) && myTank->isAlive() && !myTank->isPhantomZoned())) && !myTank->isPaused())
		{
...
static void enteringServer( void *buf )
{
	...
	{
		teamMsg = TextUtils::format( "%s team was unavailable, you were joined ", Team::getName( myTank->getTeam()));
		...
		{
			teamMsg += TextUtils::format( "to the %s", Team::getName(( TeamColor )team ));
		}
	}
	else
	{
...
static void cleanWorldCache()
{
	...
	while( true )
	{
		...
		if( h != INVALID_HANDLE_VALUE )
		{
			...
			{
				if(( oldestFile == NULL ) || ( CompareFileTime( &oldestTime, &findData.ftLastAccessTime ) > 0 ))
				{
					if( oldestFile )
					{
						free( oldestFile );
		...
		if( directory )
		{
			...
			while(( contents = readdir( directory )))
			{
				...
				if( S_ISREG( statbuf.st_mode ) && ( filename.size() > 4 ) && ( filename.substr( filename.size() - 4 ) == ".bwc" ))
				{
...
static void saveRobotInfo( Playerid id, void *msg )
{
	...
	if( robots[i] )
	{
		...
		std::cerr << "id " << id.port << ':' << id.number << ':' << callsign << ' ' << robots[i]->getId().port << ':' << robots[i]->getId().number << ':' << robots[i]->getCallsign() << std::endl;
...
void leaveGame()
{
	...
	updateDaylight( epochOffset,  *sceneRenderer );
	lastEpochOffset = epochOffset;
	BZDB.set( StateDatabase::BZDB_SYNCTIME, BZDB.getDefault( StateDatabase::BZDB_SYNCTIME ));
	...
	sceneRenderer->getViewFrustum().setProjection(( float )( 60.0 *M_PI / 180.0 ), NearPlaneNormal, FarPlaneDefault, FarDeepPlaneDefault, mainWindow->getWidth(), mainWindow->getHeight(), mainWindow->getViewHeight());
...
static void joinInternetGame( const struct in_addr *inAddress )
{
	...
	// open server
	ServerLink *_serverLink = new ServerLink( serverAddress, startupInfo.serverPort );
	...
	if( serverLink->getState() != ServerLink::Okay )
	{
		switch( serverLink->getState())
		{
			case ServerLink::BadVersion: 
			{
				...
				strncpy( versionError + strlen( versionError ) - 8, serverLink->getVersion(), 8 );
			...
			HUDDialogStack::get()->setFailedMessage( "Game is full or over.  Try again later." );
			break;

			case ServerLink::SocketError: HUDDialogStack::get()->setFailedMessage( "Error connecting to server." );
			break;

			case ServerLink::CrippledVersion: 
			// can't connect to (otherwise compatible) non-crippled server
			HUDDialogStack::get()->setFailedMessage( "Cannot connect to full version server." );
			break;

			default:
				HUDDialogStack::get()->setFailedMessage( TextUtils::format( "Internal error connecting to server (error code %d).", serverLink->getState()).c_str());
				break;
		}
		return ;
	}
...
static void joinInternetGame2()
{
	...
	// make local player
	myTank = new LocalPlayer( serverLink->getId(), startupInfo.callsign, startupInfo.email );
	...
	serverLink->sendEnter( myTank->getId(), TankPlayer, myTank->getTeam(), myTank->getCallSign(), myTank->getEmailAddress(), startupInfo.token );
	startupInfo.token[0] = '\0';
	...
	if( BZDB.isTrue( "trackShots" ))
	{
		...
		if( sp != NULL )
		{
			...
			if( speed > 0.0f )
			{
				...
				const float dir[3] = 
				{
					ilen *vel[0], ilen *vel[1], ilen *vel[2]
				};
				float topDir[3] = 
				{
					1.0f, 0.0f, 0.0f
...
static void setupFarPlane()
{
	...
	const bool mapFog = ( BZDB.get( StateDatabase::BZDB_FOGMODE ) != "none" ) && BZDB.isTrue( "fogEffect" );

	float farDist = FarPlane;

	if( mapFog && ( BZDB.get( "_cullDist" ) == "fog" ) && !BZDB.isTrue( "_fogNoSky" ))
	{
...
void drawFrame( const float dt )
{
	...
	static const float defaultPos[3] = 
	{
		0.0f, 0.0f, 0.0f
	};
	static const float defaultDir[3] = 
	{
		1.0f, 0.0f, 0.0f
	};
	...
	if( !unmapped )
	{
		...
		if( devDriving || ROAM.isRoaming())
		{
			...
			if( !( ROAM.getMode() == Roaming::roamViewFree ) && ( ROAM.getTargetTank() || ( devDriving && myTank )))
			{
				Player *target;
				if( !devDriving )
				{
				...
				else if( ROAM.getMode() == Roaming::roamViewFlag )
				{
					...
					{
						targetPoint[2] -= ( BZDBCache::tankHeight - BZDB.eval( StateDatabase::BZDB_MUZZLEHEIGHT ));
					}
				}
		...
		viewFrustum.setProjection( fov, NearPlane, FarPlane, FarDeepPlane, mainWindow->getWidth(), mainWindow->getHeight(), mainWindow->getViewHeight());
		...
		if( scene && myTank )
		{
			...
			for( i = 0; i < curMaxPlayers; i++ )
			{
				if( player[i] )
				{
					...
					const bool inCockpt = ROAM.isRoaming() && !devDriving && ( ROAM.getMode() == Roaming::roamViewFP ) && ROAM.getTargetTank() && ( ROAM.getTargetTank()->getId() == i );
					const bool showPlayer = !inCockpt || showTreads;

					// add player tank if required
					player[i]->addToScene( scene, effectiveTeam, inCockpt, seerView, showPlayer, showPlayer /*showIDL*/ );
				}
			}
		...
		sceneRenderer->setDim( HUDDialogStack::get()->isActive() || insideDim || ( myTank && !ROAM.isRoaming() && !devDriving ) && !myTank->isAlive() && !myTank->isExploding());
		...
		else
		{
			...
			if( zoomFactor != 1 )
			{
				...
				glScissor( mainWindow->getOriginX(), mainWindow->getOriginY(), mainWindow->getWidth(), mainWindow->getHeight());

				// set pixel copy destination
				glMatrixMode( GL_PROJECTION );
				glLoadIdentity();
				glOrtho(  - 0.25, ( GLdouble )mainWindow->getWidth() - 0.25,  - 0.25, ( GLdouble )mainWindow->getHeight() - 0.25,  - 1.0, 1.0 );
				glMatrixMode( GL_MODELVIEW );
				glPushMatrix();
				glLoadIdentity();
				glRasterPos2i( 0, 0 );
...
static void playingLoop()
{
	...
	while( !CommandsStandard::isQuit())
	{
		...
		if( mainWindow->haveJoystick())
		{
			...
			{
				BzfKeyEvent::BZ_Button_1, BzfKeyEvent::BZ_Button_2, BzfKeyEvent::BZ_Button_3, BzfKeyEvent::BZ_Button_4, BzfKeyEvent::BZ_Button_5, BzfKeyEvent::BZ_Button_6, BzfKeyEvent::BZ_Button_7, BzfKeyEvent::BZ_Button_8, BzfKeyEvent::BZ_Button_9, BzfKeyEvent::BZ_Button_10, BzfKeyEvent::BZ_Button_11, BzfKeyEvent::BZ_Button_12, 
			};
...
static void timeConfigurations()
{
	...
	if( timeNoBlendNoZ > MaxFrameTime && timeNoBlendZ > MaxFrameTime && timeBlendNoZ > MaxFrameTime && timeBlendZ > MaxFrameTime )
	{
		if( timeNoBlendNoZ < timeNoBlendZ && timeNoBlendNoZ < timeBlendNoZ && timeNoBlendNoZ < timeBlendZ )
		{
			// no depth, no blending definitely fastest
			BZDB.set( "zbuffer", "1" );
			BZDB.set( "blend", "1" );
		}
		if( timeNoBlendZ < timeBlendNoZ && timeNoBlendZ < timeBlendZ )
		{
	...
	if( timeBlendNoZ > timeNoBlendNoZ || timeBlendNoZ > timeNoBlendZ && timeBlendZ > timeNoBlendNoZ || timeBlendZ > timeNoBlendZ )
	{
		BZDB.set( "blend", "0" );
	}
	...
	if( timeConfiguration( false ) > MaxFrameTime || timeConfiguration( true ) > MaxFrameTime )
	{
		BZDB.set( "texture", "0" );
		tm.setMaxFilter( OpenGLTexture::Off );
		sceneRenderer->setQuality( 0 );
	...
	if( timeConfiguration( true ) < MaxFrameTime )
		return ;
...
static void findFastConfiguration()
{
	...
	static const GLfloat eyePoint[3] = 
	{
		0.0f, 0.0f, muzzleHeight
	};
	static const GLfloat targetPoint[3] = 
	{
		0.0f, 10.0f, muzzleHeight
	};
	sceneRenderer->getViewFrustum().setProjection(( float )( 45.0 *M_PI / 180.0 ), NearPlaneNormal, FarPlaneDefault, FarDeepPlaneDefault, mainWindow->getWidth(), mainWindow->getHeight(), mainWindow->getViewHeight());
	sceneRenderer->getViewFrustum().setView( eyePoint, targetPoint );
	...
	// database.
	static const GLfloat base[3] = 
	{
		 - 10.0f, 10.0f, 0.0f
	};
	static const GLfloat sEdge[3] = 
	{
		20.0f, 0.0f, 0.0f
	};
	static const GLfloat tEdge[3] = 
	{
		0.0f, 0.0f, 10.0f
	};
	...
	WallSceneNode *node = new QuadWallSceneNode( base, sEdge, tEdge, 1.0f, 1.0f, true );
	node->setColor( color );
...
void startPlaying( BzfDisplay *_display, SceneRenderer &renderer )
{
	// initalization
	display = _display;
	...
	// but this works on every system so far.
	{
		...
		{
			case SceneRenderer::Stacked: case SceneRenderer::Stereo: 
#ifndef USE_GL_STEREO
			...
			case SceneRenderer::ThreeChannel: default:
	...
	if( debugLevel >= 1 )
	{
		...
		GetVersionEx( &info );
		DEBUG1( "Running on Windows %s%d.%d %s\n", ( info.dwPlatformId == VER_PLATFORM_WIN32_NT ) ? "NT " : "", info.dwMajorVersion, info.dwMinorVersion, info.szCSDVersion );
	}
#endif 
	...
	// send informative header to the console
	{
		...
		tmpString += "BZFlag(playground) version: ";
