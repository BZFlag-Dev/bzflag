				...
				const CommandManager::ArgList& args)
{
	...
	if(args[0] == "rotate") {
		...
		if(args[1] == "left") {
			myTank->roamDPos[1] = 4.0f * BZDB->eval("_tankSpeed");
		}
		else if(args[1] == "right") {
			myTank->roamDPos[1] = -4.0f * BZDB->eval("_tankSpeed");
		}
		else if(args[1] == "forward") {
			myTank->roamDPos[0] = 4.0f * BZDB->eval("_tankSpeed");
		}
		else if(args[1] == "backward") {
			myTank->roamDPos[0] = -4.0f * BZDB->eval("_tankSpeed");
		}
		else if(args[1] == "up") {
			myTank->roamDPos[2] = 4.0f * BZDB->eval("_tankSpeed");
		}
		else if(args[1] == "down") {
			myTank->roamDPos[2] = -4.0f * BZDB->eval("_tankSpeed");
		}
...
*/
		...
		case MsgKilled: {
			...
			if (victimLocal) {
				...
				explodePos[0] = pos[0];
				explodePos[1] = pos[1];
				explodePos[2] = pos[2] + BZDB->eval("_muzzleHeight");
								     ...
								     double fractionOfTries)
{
	...
	static const float		MinRange = 2.5f * BZDB->eval("_shotSpeed");	// meters

	// anything beyond this range is okay at any angle
	static const float		MaxRange = 5.0f * BZDB->eval("_shotSpeed");	// meters
	...
	do {
		do {
			if (restartOnBase) {
				...
				const float x = (base[4] - 2.0f * BZDB->eval("_tankRadius")) * ((float)bzfrand() - 0.5f);
				const float y = (base[5] - 2.0f * BZDB->eval("_tankRadius")) * ((float)bzfrand() - 0.5f);
				...
				if (world->allowTeamFlags()) {
					startPoint[0] = 0.4f * BZDB->eval("_worldSize") * ((float)bzfrand() - 0.5f);
					startPoint[1] = 0.4f * BZDB->eval("_worldSize") * ((float)bzfrand() - 0.5f);
				}
				else {
					startPoint[0] = (BZDB->eval("_worldSize") - 2.0f * BZDB->eval("_tankRadius")) * ((float)bzfrand() - 0.5f);
					startPoint[1] = (BZDB->eval("_worldSize") - 2.0f * BZDB->eval("_tankRadius")) * ((float)bzfrand() - 0.5f);
				}
			}
			startAzimuth = 2.0f * M_PI * (float)bzfrand();
		} while (world->inBuilding(startPoint, 2.0f * BZDB->eval("_tankRadius")));
		...
		for (int i = 0; i < maxPlayers; i++) {
			...
			for (int j = 0; j < maxShots; j++) {
				...
				const float t = Intersect::timeRayHitsBlock(
								...
								4.0f * BZDB->eval("_tankLength"), 4.0f * BZDB->eval("_tankWidth"),
								2.0f * BZDB->eval("_tankHeight"));
	...
	for (int i = 0; i < numFlags; i++) {
		...
		if (flag.status == FlagOnTank) {
			...
			if (tank) {
				...
				flag.position[2] = pos[2] + BZDB->eval("_tankHeight");
			}
		}
								...
								float size, float duration)
{
	...
	int boom = 1;
	if (size >= 3.0f * BZDB->eval("_tankLength"))
...
static void				checkEnvironment()
{
	...
	if (flagId != NoFlag && int(flagId) >= int(FirstTeamFlag) &&
		...
		if (TimeKeeper::getTick()-lastGrabSent > 0.2) {
			...
			const float radius2 = (radius + BZDB->eval("_flagRadius")) * (radius + BZDB->eval("_flagRadius"));
			for (int i = 0; i < numFlags; i++) {
		...
		float minDist = BZDB->eval("_identifyRange") * BZDB->eval("_identifyRange");
		int closestFlag = -1;
		for (int i = 0; i < numFlags; i++) {
		...
		for (i = 0; i < maxPlayers; i++)
		  ...
		  !player[i]->isPaused()) {
				...
				if (!(flagId == PhantomZoneFlag && myTank->isFlagActive())) {
					const float radius = myRadius + BZDB->eval("_srRadiusMult") * player[i]->getRadius();
...
*/
	...
	eyePoint[0] = 0.0f;
	eyePoint[1] = 0.0f;
	eyePoint[2] = 0.0f + BZDB->eval("_muzzleHeight");
	...
	SCENEMGR->getView().setProjection(60.0f,
								...
								1.1f, 2.0f * BZDB->eval("_worldSize"));
...
static void				playingLoop()
{
	...
	while (!CommandsStandard::isQuit()) {
		...
		if(myTank != NULL) {
			...
			if(myTank->roamPos[2] < BZDB->eval("_muzzleHeight"))
				myTank->roamPos[2] = BZDB->eval("_muzzleHeight");
...
*/
			...
			// set the view parameters
			{
				if(BZDB->get("playerType") == "tank" || myTank == NULL) {
					...
					eye[0]   = pos[0];
					eye[1]   = pos[1];
					eye[2]   = pos[2] + BZDB->eval("_muzzleHeight");
					focus[0] = eye[0] + dir[0];
					focus[1] = eye[1] + dir[1];
					focus[2] = eye[2] + dir[2];
					...
					if(player[myTank->roamTrackTank] && myTank->RoamView != LocalPlayer::RoamViewFree) {
					  ...
					  if(myTank->RoamView == LocalPlayer::RoamViewTrack) {
					    ...
					    eye[2] = target->getPosition()[2] + BZDB->eval("_muzzleHeight") * 6;
					    focus[0] = target->getPosition()[0];
					    focus[1] = target->getPosition()[1];
					    focus[2] = target->getPosition()[2];
					  }
					    ...
					    eye[2] = target->getPosition()[2] + BZDB->eval("_muzzleHeight");
