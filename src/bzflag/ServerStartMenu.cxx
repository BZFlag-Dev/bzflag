...
void ServerStartMenu::scanWorldFiles( const std::string &searchDir, std::vector < std::string >  *items )
{
#ifdef _WIN32
...
void ServerStartMenu::setSettings( const char *_settings )
{
	...
	{
		strncpy( settings, _settings, 12 );
		settings[12] = _settings[13];
		settings[13] = _settings[14];
		return ;
	}

	if( strlen( _settings ) != strlen( settings ))
		return ;
...
void ServerStartMenu::execute()
{
	...
	if( _focus == start )
	{
		...
		if((( HUDuiList* )listHUD[15] )->getIndex() != 0 )
		{
			// not random
			args[arg++] = "-world";
			std::vector < std::string > fileList = (( HUDuiList* )listHUD[15] )->getList();
			std::string filename = fileList[(( HUDuiList* )listHUD[15] )->getIndex()].c_str();
		...
		{
			static const char *rabbitStyles[] = 
			{
				"random", "score", "killer"
			};
			args[arg++] = "-rabbit";
		...
		if( superflagOption != 0 )
		{
			...
			// max superflags
			static const char *numFlagsList[] = 
			{
				"10", "20", "30", "40"
			};
		...
		if((( HUDuiList* )listHUD[10] )->getIndex() == 1 )
			args[arg++] = "-sa";
		...
		{
			"", "-time", "-time", "-time", "-time", "-mps", "-mps", "-mps", "-mts", "-mts", "-mts", "-mts"
		};
		static const char *gameOverValue[] = 
		{
			"", "300", "900", "3600", "10800", "3", "10", "25", "3", "10", "25", "100"
		};
...
void ServerStartMenu::setStatus( const char *msg, const std::vector < std::string >  *parms )
{
	...
	const float widt = fm.getStrLength( status->getFontFace(), status->getFontSize(), status->getString());
