...
LocalPlayer::LocalPlayer( const PlayerId &_id, const char *name, const char *_email ): BaseLocalPlayer( _id, name, _email ), gettingSound( true ), server( ServerLink::getServer()), location( Dead ), firingStatus( Deceased ), flagShakingTime( 0.0f ), flagShakingWins( 0 ), antidoteFlag( NULL ), desiredSpeed( 0.0f ), desiredAngVel( 0.0f ), lastSpeed( 0.0f ), anyShotActive( false ), target( NULL ), nemesis( NULL ), recipient( NULL ), inputChanged( false ), spawning( false ), wingsFlapCount( 0 ), left( false ), right( false ), up( false ), down( false ), entryDrop( true ), wantJump( false ), jumpPressed( false ), deathPhyDrv(  - 1 )
{
...
void LocalPlayer::doUpdate( float dt )
{
	...
	if( !isPaused() && dt > 0.0f && World::getWorld()->allowShakeTimeout() && getFlag() != Flags::Null && getFlag()->endurance == FlagSticky && flagShakingTime > 0.0f )
	{
		flagShakingTime -= dt;
...
void LocalPlayer::doSlideMotion( float dt, float slideTime, float newAngVel, float *newVelocity )
{
...
float LocalPlayer::getNewAngVel( float old, float desired )
{
	...
	else
	{
		...
		// instant stop
		if(( old *desired < 0.0f ) || ( NEAR_ZERO( desired, ZERO_TOLERANCE )))
		{
			newAngVel = desired;
		}
	}
...
void LocalPlayer::doUpdateMotion( float dt )
{
	...
	if( !NEAR_ZERO( dt, ZERO_TOLERANCE ))
	{
		...
		else if(( location == OnGround ) || ( location == OnBuilding ) || ( location == InBuilding && oldPosition[2] == groundLimit ))
		{
			// full control
	...
	if( wantJump )
	{
		...
		{
			newVelocity[2] = oldVelocity[2];
			if(( lastObstacle != NULL ) && !lastObstacle->isFlatTop() && BZDB.isTrue( StateDatabase::BZDB_NOCLIMB ))
			{
				newVelocity[0] = 0.0f;
				newVelocity[1] = 0.0f;
			}
	...
	if( location != Dead && location != Exploding )
	{
		...
		obstacle = getHitBuilding( newPos, newAzimuth, newPos, newAzimuth, phased, expel );
		...
		if( obstacle && TimeKeeper::getCurrent() - stuckStartTime > 0.5 )
		{
			...
			if(( World::getWorld()->allowJumping() || ( getFlag() == Flags::Jumping ) || ( getFlag() == Flags::Wings )) && ( getFlag() != Flags::NoJumping ))
	...
	for( int numSteps = 0; numSteps < MaxSteps; numSteps++ )
	{
		...
		obstacle = getHitBuilding( tmpPos, tmpAzimuth, newPos, newAzimuth, phased, expel );
		...
		if(( oldLocation != InAir ) && obstacle->isFlatTop() && ( obstacleTop != tmpPos[2] ) && ( obstacleTop < ( tmpPos[2] + BZDB.eval( StateDatabase::BZDB_MAXBUMPHEIGHT ))))
		{
			...
			// drive over bumps
			const Obstacle *bumpObstacle = getHitBuilding( newPos, tmpAzimuth, newPos, newAzimuth, phased, expel );
			if( !bumpObstacle )
			{
		...
		for( int i = 0; searchStep > MinSearchStep && i < MaxSearchSteps; searchStep *= 0.5f, i++ )
		{
			...
			bool searchExpel;
			const Obstacle *searchObstacle = getHitBuilding( tmpPos, tmpAzimuth, newPos, newAzimuth, phased, searchExpel );
		...
		else
		{
			...
			if( !NEAR_ZERO( normal[2], ZERO_TOLERANCE ))
			{
				...
				if( newVelocity[2] < 0.0f && newVelocity[2] - ( mag + normal[2] *newVelocity[2] ) *normal[2] > 0.0f )
				{
					newVelocity[2] = 0.0f;
				}
	...
	{
		if( obstacle->isCrossing( newPos, newAzimuth, 0.5f *BZDBCache::tankLength, 0.5f *BZDBCache::tankWidth, BZDBCache::tankHeight, NULL ))
		{
	...
	else if( World::getWorld()->crossingTeleporter( newPos, newAzimuth, 0.5f *BZDBCache::tankLength, 0.5f *BZDBCache::tankWidth, BZDBCache::tankHeight, crossingPlane ))
	{
		setStatus( getStatus() | int( PlayerState::CrossingWall ));
	}
	else
	...
	if( teleporter )
	{
		...
		else
		{
			...
			int outFace;
			const Teleporter *outPort = World::getWorld()->getTeleporter( targetTele, outFace );
			...
			const Obstacle *teleObs = getHitBuilding( newPos, newAzimuth, newPos, newAzimuth, phased, expel );
			if( teleObs != NULL )
			{
				// revert
	...
	setPhysicsDriver(  - 1 );
	if(( lastObstacle != NULL ) && ( lastObstacle->getType() == MeshFace::getClassName()))
	{
	...
	const bool justLanded = ( oldLocation == InAir ) && (( location == OnGround ) || ( location == OnBuilding ));
	...
	// set falling status
	if( location == OnGround || location == OnBuilding || ( location == InBuilding && newPos[2] == 0.0f ))
	{
	...
	if((( phydrv2 != NULL ) && phydrv2->getIsSlide()) || (( getFlag() == Flags::Wings ) && ( location == InAir ) && ( BZDB.eval( StateDatabase::BZDB_WINGSSLIDETIME ) > 0.0f )))
	{
	...
	{
		float dist = (( flagAntidotePos[0] - newPos[0] )*( flagAntidotePos[0] - newPos[0] )) + (( flagAntidotePos[1] - newPos[1] )*( flagAntidotePos[1] - newPos[1] ));
	...
	if( gettingSound )
	{
		if( oldPosition[0] != newPos[0] || oldPosition[1] != newPos[1] || oldPosition[2] != newPos[2] || oldAzimuth != newAzimuth )
		{
			moveSoundReceiver( newPos[0], newPos[1], newPos[2], newAzimuth, NEAR_ZERO( dt, ZERO_TOLERANCE ) || (( teleporter != NULL ) && ( getFlag() != Flags::PhantomZone )));
		}
		if( NEAR_ZERO( dt, ZERO_TOLERANCE ))
		{
		...
		{
			speedSoundReceiver(( newPos[0] - oldPosition[0] ) / dt, ( newPos[1] - oldPosition[1] ) / dt, ( newPos[2] - oldPosition[2] ) / dt );
		}
	}
}
...
const Obstacle *LocalPlayer::getHitBuilding( const float *p, float a, bool phased, bool &expel )const
{
	const bool hasOOflag = getFlag() == Flags::OscillationOverthruster;
	const float *dims = getDimensions();
	const Obstacle *obstacle = World::getWorld()->hitBuilding( p, a, dims[0], dims[1], dims[2] );

	expel = ( obstacle != NULL );
	if( expel && phased )
		expel = ( obstacle->getType() == WallObstacle::getClassName() || obstacle->getType() == Teleporter::getClassName() || ( hasOOflag && desiredSpeed < 0.0f && NEAR_ZERO( p[2], ZERO_TOLERANCE )));
	return obstacle;
}
...
const Obstacle *LocalPlayer::getHitBuilding( const float *oldP, float oldA, const float *p, float a, bool phased, bool &expel )
{
	const bool hasOOflag = getFlag() == Flags::OscillationOverthruster;
	const float *dims = getDimensions();
	const Obstacle *obstacle = World::getWorld()->hitBuilding( oldP, oldA, p, a, dims[0], dims[1], dims[2], !hasOOflag );
...
bool LocalPlayer::getHitNormal( const Obstacle *o, const float *pos1, float azimuth1, const float *pos2, float azimuth2, float *normal )const
{
	const float *dims = getDimensions();
...
static bool notInObstacleList( const Obstacle *obs, const std::vector < const Obstacle * >  &list )
{
	...
	for( int i = 0; i < olist->count; i++ )
	{
		...
		if( obs->inBox( pos, angle, dims[0], dims[1], dims[2] ))
		{
			if( obs->getType() == MeshFace::getClassName())
			{
				...
				if( !obs->isDriveThrough() && notInObstacleList( mesh, insideBuildings ))
				{
...
void LocalPlayer::setDesiredSpeed( float fracOfMaxSpeed )
{
	...
	if( fracOfMaxSpeed < 0.0f && getLocation() == InBuilding && flag == Flags::OscillationOverthruster )
	{
		fracOfMaxSpeed = 0.0f;
	}
...
void LocalPlayer::setDesiredAngVel( float fracOfMaxAngVel )
{
	...
	if( fracOfMaxAngVel > 1.0f )
		fracOfMaxAngVel = 1.0f;
	else if( fracOfMaxAngVel <  - 1.0f )
		fracOfMaxAngVel =  - 1.0f;
...
void LocalPlayer::doJump()
{
	...
	else if(( flag != Flags::Bouncy ) && (( flag != Flags::Jumping && !World::getWorld()->allowJumping()) || ( flag == Flags::NoJumping )))
	{
		return ;
	}
...
void LocalPlayer::doMomentum( float dt, float &speed, float &angVel )
{
	...
	float linearAcc = ( getFlag() == Flags::Momentum ) ? BZDB.eval( StateDatabase::BZDB_MOMENTUMLINACC ): BZDB.eval( StateDatabase::BZDB_INERTIALINEAR );
	float angularAcc = ( getFlag() == Flags::Momentum ) ? BZDB.eval( StateDatabase::BZDB_MOMENTUMANGACC ): BZDB.eval( StateDatabase::BZDB_INERTIAANGULAR );
	...
	if( angularAcc > 0.0f )
	{
		...
		if( angAcc > angularAcc )
			angVel = oldAngVel + dt * angularAcc;
...
{
	const float friction = ( getFlag() == Flags::Momentum ) ? BZDB.eval( StateDatabase::BZDB_MOMENTUMFRICTION ): BZDB.eval( StateDatabase::BZDB_FRICTION );
	...
	if( friction > 0.0f )
	{
		...
		float delta[2] = 
		{
			newVelocity[0] - oldVelocity[0], newVelocity[1] - oldVelocity[1]
		};
...
bool LocalPlayer::checkHit( const Player *source, const ShotPath * &hit, float &minTime )const
{
	bool goodHit = false;
	...
	for( int i = 0; i < maxShots; i++ )
	{
		...
		if( source == this && (( shot->getFlag() == Flags::ShockWave ) || ( shot->getFlag() == Flags::Thief )))
			continue;
		...
		if( isPhantomZoned() && ( shotFlag != Flags::ShockWave ) && ( shotFlag != Flags::SuperBullet ) && ( shotFlag != Flags::PhantomZone ))
			continue;
		...
		if( isCrossingWall() && position[0] *crossingPlane[0] + position[1] *crossingPlane[1] + position[2] *crossingPlane[2] + crossingPlane[3] < 0.0 )
			continue;
...
void LocalPlayer::changeScore( short deltaWins, short deltaLosses, short deltaTks )
{
	Player::changeScore( deltaWins, deltaLosses, deltaTks );
	if( deltaWins > 0 && World::getWorld()->allowShakeWins() && flagShakingWins > 0 )
	{
...
void LocalPlayer::setKey( int button, bool pressed )
{
	switch( button )
	{
		case BzfKeyEvent::Left: left = pressed;
		break;
		case BzfKeyEvent::Right: right = pressed;
		break;
		case BzfKeyEvent::Up: up = pressed;
		break;
		case BzfKeyEvent::Down: down = pressed;
		break;
	}
}
