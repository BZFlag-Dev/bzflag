...
const GLfloat HUDRenderer::black[3] = 
{
	0.0f, 0.0f, 0.0f
};
...
void HUDRenderer::resize( bool firstTime )
{
	...
	scoreboard->setWindowSize( 0.01f *window.getWidth(), sby, 0.98f *window.getWidth(), sby );
}
...
void HUDRenderer::setAlert( int index, const char *string, float duration, bool warning )
{
	if( index < 0 )
		index = 0;
...
void HUDRenderer::setComposeString( const std::string &message, bool _allowEdit )const
{
...
void HUDRenderer::setComposing( const std::string &prompt, bool _allowEdit )
{
	...
	if( showCompose )
	{
		...
		{
			FontManager &fm = FontManager::instance();
			const float x = fm.getStrLength( cFontFace, cFontSize, composeTypeIn->getLabel()) + fm.getStrLength( cFontFace, cFontSize, "99" );
			const float y = 1.0f;
...
void HUDRenderer::hudColor4f( GLfloat r, GLfloat g, GLfloat b, GLfloat a )
{
	if( dim )
...
void HUDRenderer::render( SceneRenderer &renderer )
{
	...
	{
		const bool showTimes = ( fps > 0.0f ) || ( drawTime > 0.0f ) || ( triangleCount > 0 ) || ( radarTriangleCount > 0 );
...
void HUDRenderer::renderAlerts( void )
{
	...
	float y = ( float )window.getViewHeight() +  - fm.getStrHeight( majorFontFace, majorFontSize, " " ) +  - fm.getStrHeight( alertFontFace, alertFontSize, " " );
	...
	for( int i = 0; i < MaxAlerts; i++ )
	{
		if( alertClock[i].isOn())
		{
			...
			if( dim )
			{
				newAlertLabel.insert( newAlertLabel.find( ColorStrings[ResetColor], 0 ) - 1 + ColorStrings[ResetColor].size(), ColorStrings[DimColor] );
			}
			fm.drawString( centerx - 0.5f * alertLabelWidth[i], y, 0, alertFontFace, alertFontSize, newAlertLabel );
...
void HUDRenderer::renderStatus( void )
{
	...
	{
		y -= float( 1.5 *h );
		sprintf( buffer, "[%d %d %d]", ( int )myTank->getPosition()[0], ( int )myTank->getPosition()[1], ( int )myTank->getPosition()[2] );
		x = ( float )window.getWidth() - 0.25f * h - fm.getStrLength( majorFontFace, majorFontSize, buffer );
		fm.drawString( x, y, 0, majorFontFace, majorFontSize, buffer );
		y += float( 1.5 *h );
	}
	...
	static const GLfloat redColor[3] = 
	{
		1.0f, 0.0f, 0.0f
	};
	static const GLfloat yellowColor[3] = 
	{
		1.0f, 1.0f, 0.0f
	};
	static const GLfloat greenColor[3] = 
	{
		0.0f, 1.0f, 0.0f
	};
	...
	if( !roaming )
	{
		...
		{
			case LocalPlayer::Deceased: strcat( buffer, bdl->getLocalString( "Dead" ).c_str());
			break;

			case LocalPlayer::Ready: if( flag != Flags::Null && flag->endurance == FlagSticky && World::getWorld()->allowShakeTimeout())
			{
			...
			case LocalPlayer::Sealed: strcat( buffer, bdl->getLocalString( "Sealed" ).c_str());
			break;

			case LocalPlayer::Zoned: strcat( buffer, bdl->getLocalString( "Zoned" ).c_str());
			break;
		}
	}
	...
	if( roaming )
	{
		...
		if( dim )
			strcat( buffer, ColorStrings[DimColor].c_str());
...
void HUDRenderer::renderTankLabels( SceneRenderer &renderer )
{
	...
	GLint view[] = 
	{
		window.getOriginX(), window.getOriginY(), window.getWidth(), window.getHeight()
	};
	...
	for( int i = 0; i < curMaxPlayers; i++ )
	{
		...
		if( pl && pl->isAlive())
		{
			...
			gluProject( pl->getPosition()[0], pl->getPosition()[1], pl->getPosition()[2], model, proj, view, &x, &y, &z );
			if( z >= 0.0 && z <= 1.0 )
			{
				FontManager &fm = FontManager::instance();
				fm.drawString( float( x ) - fm.getStrLength( labelsFontFace, labelsFontSize, name ) / 2.0f, float( y ) + offset - fm.getStrHeight( labelsFontFace, labelsFontSize, name ), 0, labelsFontFace, labelsFontSize, name );
				...
				if( flag != Flags::Null )
				{
					...
					fm.drawString( float( x ) - fm.getStrLength( labelsFontFace, labelsFontSize, fname ) / 2.0f, float( y ) + offset - ( 2.0f *fm.getStrHeight( labelsFontFace, labelsFontSize, fname )), 0, labelsFontFace, labelsFontSize, fname );
				}
			}
		}
	}
...
void HUDRenderer::renderCracks()
{
	...
	glPushMatrix();
	glTranslatef( GLfloat( window.getWidth() >> 1 ), GLfloat( window.getViewHeight() >> 1 ), 0.0f );
	glLineWidth( 3.0 );
...
void HUDRenderer::renderTimes( void )
{
	...
	if( fps > 0.0f )
	{
		...
		hudColor3f( 1.0f, 1.0f, 1.0f );
		fm.drawString(( float )( centerx - maxMotionSize ), ( float )centery + ( float )maxMotionSize + 3.0f *fm.getStrHeight( headingFontFace, headingFontSize, "0" ), 0, headingFontFace, headingFontSize, buf );
	}
	...
	if( radarTriangleCount > 0 )
	{
		...
		hudColor3f( 1.0f, 1.0f, 1.0f );
		fm.drawString(( float )( centerx - maxMotionSize ), ( float )centery + ( float )maxMotionSize + triCountYOffset *fm.getStrHeight( headingFontFace, headingFontSize, "0" ), 0, headingFontFace, headingFontSize, buf );
	...
	if( drawTime > 0.0f )
	{
		...
		hudColor3f( 1.0f, 1.0f, 1.0f );
		fm.drawString(( float )( centerx + maxMotionSize ) - fm.getStrLength( headingFontFace, headingFontSize, buf ), ( float )centery + ( float )maxMotionSize + 3.0f *fm.getStrHeight( headingFontFace, headingFontSize, "0" ), 0, headingFontFace, headingFontSize, buf );
	}
}
...
void HUDRenderer::renderBox( SceneRenderer & )
{
	...
	{
		// first clip to area
		glScissor( ox + centerx - maxMotionSize, oy + height - viewHeight + centery + maxMotionSize - 5, 2 *maxMotionSize, 25+( int )( headingFontSize + 0.5f ));
		...
		glScissor( ox + centerx - maxMotionSize - 8, oy + height - viewHeight + centery + maxMotionSize, 2 *maxMotionSize + 16, 10 );
		glPushMatrix();
		...
		for( MarkerList::const_iterator it = markers.begin(); it != markers.end(); ++it )
		{
			...
			if( relAngle <= headingOffset || relAngle >= 360.0f - headingOffset )
			{
				...
				GLfloat mx = maxMotionSize / headingOffset * (( relAngle < 180.0f ) ? relAngle : relAngle - 360.0f );
				glBegin( GL_QUADS );
				glVertex2f( mx, 0.0f );
	...
	{
		// clip to area
		glScissor( ox + centerx + maxMotionSize - 5, oy + height - viewHeight + centery - maxMotionSize, ( int )altitudeLabelMaxWidth + 15, 2 *maxMotionSize );
		...
		if( !smooth )
			basey = floorf( basey );
		glTranslatef(( float )( centerx + maxMotionSize ), ( float )centery - basey, 0.0f );
		y = smooth ? 0.0f :  - 0.5f;
		glBegin( GL_LINES );
		for( i = minMark; i <= maxMark; i++ )
		{
...
void HUDRenderer::renderPlaying( SceneRenderer &renderer )
{
	...
	if( flagHelpClock.isOn())
	{
		...
		for( i = 0; i < flagHelpLines; i++ )
		{
			...
			fm.drawString(( float )( centerx - fm.getStrLength( minorFontFace, minorFontSize, flagHelpBase ) / 2.0 ), y, 0, minorFontFace, minorFontSize, flagHelpBase );
			while( *flagHelpBase )
				flagHelpBase++;
			flagHelpBase++;
		}
	...
	{
		float yy = 0.5f *( float )height + fm.getStrHeight( bigFontFace, bigFontSize, "0" );
		if( myTank->isAutoPilot())
		{
			hudColor3fv( messageColor );
			fm.drawString( 0.5f *(( float )width - autoPilotWidth ), yy, 0, bigFontFace, bigFontSize, autoPilotLabel );
		}
	}
...
void HUDRenderer::renderNotPlaying( SceneRenderer &renderer )
{
	...
	if( myTank && globalClock.isOn())
	{
		...
		{
			hudColor3fv( messageColor );
			fm.drawString( 0.5f *(( float )width - gameOverLabelWidth ), y, 0, bigFontFace, bigFontSize, gameOverLabel );
		}
		else if( !myTank->isAlive() && !myTank->isExploding())
		{
			hudColor3fv( messageColor );
			fm.drawString( 0.5f *(( float )width - restartLabelWidth ), y, 0, bigFontFace, bigFontSize, restartLabel );
		}
		else if( myTank->isPaused())
		{
			hudColor3fv( messageColor );
			fm.drawString( 0.5f *(( float )width - resumeLabelWidth ), y, 0, bigFontFace, bigFontSize, resumeLabel );
		}
		else if( myTank->isAutoPilot())
		{
			hudColor3fv( messageColor );
			fm.drawString( 0.5f *(( float )width - autoPilotWidth ), y, 0, bigFontFace, bigFontSize, autoPilotLabel );
		}
	}
...
void HUDRenderer::renderShots( const Player *target )
{
	...
	for( int i = 0; i < maxShots; ++i )
	{
		...
		if( factors[i] < 1.0f )
		{
			...
			glRecti( indicatorLeft + myWidth + 1, myTop, indicatorLeft + indicatorWidth, myTop + indicatorHeight );
		}
		else
		{
