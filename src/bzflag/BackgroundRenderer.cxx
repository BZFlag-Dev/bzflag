...
BackgroundRenderer::BackgroundRenderer( const SceneRenderer & ): blank( false ), invert( false ), style( 0 ), gridSpacing( 60.0f ),  // meters
gridCount( 4.0f ), mountainsAvailable( false ), numMountainTextures( 0 ), mountainsGState( NULL ), mountainsList( NULL ), cloudDriftU( 0.0f ), cloudDriftV( 0.0f )
{
...
void BackgroundRenderer::setCelestial( const SceneRenderer &renderer, const float sunDir[3], const float moonDir[3] )
{
...
void BackgroundRenderer::makeCelestialLists( const SceneRenderer &renderer )
{
	...
	{
		glPushMatrix();
		glRotatef(( GLfloat )( atan2f( sunDirection[1], ( sunDirection[0] )) *180.0 / M_PI ), 0.0f, 0.0f, 1.0f );
	...
	float coverage = ( moonDirection[0] *sunDirection[0] ) + ( moonDirection[1] *sunDirection[1] ) + ( moonDirection[2] *sunDirection[2] );
	// hack coverage to lean towards full
	coverage = ( coverage < 0.0f ) ?  - sqrtf(  - coverage ): coverage *coverage;
	float worldSize = BZDBCache::worldSize;
	const float moonRadius = 2.0f * worldSize * atanf(( float )(( 60.0 *M_PI / 180.0 ) / 60.0 ));
...
void BackgroundRenderer::addCloudDrift( GLfloat uDrift, GLfloat vDrift )
{
	...
	if( cloudDriftU > 1.0f )
		cloudDriftU -= 1.0f;
	else if( cloudDriftU < 0.0f )
		cloudDriftU += 1.0f;
	if( cloudDriftV > 1.0f )
		cloudDriftV -= 1.0f;
	else if( cloudDriftV < 0.0f )
		cloudDriftV += 1.0f;
}
...
void BackgroundRenderer::renderSky( SceneRenderer &renderer, bool fullWindow, bool mirror )
{
...
void BackgroundRenderer::renderGroundEffects( SceneRenderer &renderer, bool drawingMirror )
{
	...
	if( !blank )
	{
		...
		// kilometers away.
		if( BZDBCache::blend && BZDBCache::lighting && !drawingMirror && BZDBCache::drawGroundLights )
		{
...
void BackgroundRenderer::setupSkybox()
{
	...
	{
		"LeftSkyboxMaterial", "FrontSkyboxMaterial", "RightSkyboxMaterial", "BackSkyboxMaterial", "TopSkyboxMaterial", "BottomSkyboxMaterial"
	};
	TextureManager &tm = TextureManager::instance();
	const BzMaterial *bzmats[6] = 
	{
...
void BackgroundRenderer::drawSky( SceneRenderer &renderer, bool mirror )
{
	...
	if( !doSkybox )
	{
		...
		// toward the sun
		glRotatef(( GLfloat )(( atan2f( sunDirection[1], sunDirection[0] ) *180.0 + 135.0 ) / M_PI ), 0.0f, 0.0f, 1.0f );
...
void BackgroundRenderer::drawGround()
{
	if( BZDBCache::drawGround )
	{
		...
		{
			float color[4];
			if( BZDB.isSet( "GroundOverideColor" ) && parseColorString( BZDB.get( "GroundOverideColor" ), color ))
			{
				glColor4fv( color );
			}
			else
...
void BackgroundRenderer::drawGroundShadows( SceneRenderer &renderer )
{
	...
	shadowProjection[1] = shadowProjection[2] = shadowProjection[3] = shadowProjection[4] = shadowProjection[6] = shadowProjection[7] = shadowProjection[10] = shadowProjection[11] = shadowProjection[12] = shadowProjection[13] = shadowProjection[14] = 0.0f;
	glPushMatrix();
...
void BackgroundRenderer::drawGroundReceivers( SceneRenderer &renderer )
{
	...
	for( int k = 0; k < count; k++ )
	{
		...
		// maximum value
		const float maxVal = ( lightColor[0] > lightColor[1] ) ? (( lightColor[0] > lightColor[2] ) ? lightColor[0]: lightColor[2] ): (( lightColor[1] > lightColor[2] ) ? lightColor[1]: lightColor[2] );
		...
		glBegin( GL_TRIANGLE_FAN );
		{
			...
			{
				glVertex2f( receiverRingSize *angle[j][0], receiverRingSize *angle[j][1] );
			}
		}
		glEnd();
...
void BackgroundRenderer::drawAdvancedGroundReceivers( SceneRenderer &renderer )
{
	...
	if( BZDB.isSet( "GroundOverideColor" ) && parseColorString( BZDB.get( "GroundOverideColor" ), overrideColor ))
	{
	...
	{
		const float repeat = BZDB.eval( "groundHighResTexRepeat" );
		const float sPlane[4] = 
		{
			repeat, 0.0f, 0.0f, 0.0f
		};
		const float tPlane[4] = 
		{
			0.0f, repeat, 0.0f, 0.0f
		};
	...
	for( int k = 0; k < count; k++ )
	{
		...
		if( invert )
		{
			// beats me, should just color logic op the static nodes
		...
		// maximum value
		const float maxVal = ( baseColor[0] > baseColor[1] ) ? (( baseColor[0] > baseColor[2] ) ? baseColor[0]: baseColor[2] ): (( baseColor[1] > baseColor[2] ) ? baseColor[1]: baseColor[2] );
		...
		glBegin( GL_TRIANGLE_FAN );
		{
			...
			for( j = 0; j <= receiverSlices; j++ )
			{
				glVertex2f( outerSize *angle[j][0], outerSize *angle[j][1] );
			}
		}
		glEnd();
		triangleCount += receiverSlices;
...
void BackgroundRenderer::doFreeDisplayLists()
{
	...
	GLuint *const lists[] = 
	{
		 &simpleGroundList[0], &simpleGroundList[2],  &cloudsList, &sunList, &sunXFormList,  &moonList, &starList, &starXFormList
	};
...
void BackgroundRenderer::doInitDisplayLists()
{
	...
	bool isRiva128 = ( strncmp(( const char* )glGetString( GL_RENDERER ), "RIVA 128", 8 ) == 0 );
	...
	glNewList( sunList, GL_COMPILE );
	{
		glBegin( GL_TRIANGLE_FAN );
		{
			...
			for( i = 0; i < 20; i++ )
			{
				...
				glVertex3f( 2.0f *worldSize, sunRadius *sinf( angle ), sunRadius *cosf( angle ));
			}
		}
		glEnd();
	}
...
#define GROUND_DIVS	(4)	//FIXME -- seems to be enough
	...
	if( cloudsAvailable )
	{
		...
		glNewList( cloudsList, GL_COMPILE );
		{
			...
			glTexCoord2f( uvScale *cloudRepeats * squareShape[3][0], uvScale *cloudRepeats * squareShape[3][1] );
			glVertex3fv( cloudsInner[3] );
			glTexCoord2f( uvScale *cloudRepeats * squareShape[2][0], uvScale *cloudRepeats * squareShape[2][1] );
			glVertex3fv( cloudsInner[2] );
			glTexCoord2f( uvScale *cloudRepeats * squareShape[1][0], uvScale *cloudRepeats * squareShape[1][1] );
			glVertex3fv( cloudsInner[1] );
			glTexCoord2f( uvScale *cloudRepeats * squareShape[0][0], uvScale *cloudRepeats * squareShape[0][1] );
			...
			glTexCoord2f( cloudRepeats *squareShape[1][0], cloudRepeats *squareShape[1][1] );
			...
			glColor4f( 1.0f, 1.0f, 1.0f, minAlpha );
			glTexCoord2f( cloudRepeats *squareShape[2][0], cloudRepeats *squareShape[2][1] );
			...
			glColor4f( 1.0f, 1.0f, 1.0f, minAlpha );
			glTexCoord2f( cloudRepeats *squareShape[3][0], cloudRepeats *squareShape[3][1] );
			...
			glColor4f( 1.0f, 1.0f, 1.0f, minAlpha );
			glTexCoord2f( cloudRepeats *squareShape[0][0], cloudRepeats *squareShape[0][1] );
