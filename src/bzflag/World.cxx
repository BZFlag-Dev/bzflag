...
World::World(): gameStyle( TeamFFAGameStyle ), maxPlayers( 0 ), curMaxPlayers( 0 ), maxShots( 1 ), maxFlags( 0 ), players( NULL ), flags( NULL ), flagNodes( NULL ), flagWarpNodes( NULL )
{
...
TeamColor World::whoseBase( const float *pos )const
{
	...
	for( int i = 1; i < NumTeams; i++ )
	{
		for( TeamBases::const_iterator it = bases[i].begin(); it != bases[i].end(); ++it )
		{
			...
			if( fabsf( rx ) < it->p[4] && fabsf( ry ) < it->p[5] )
			{
...
const Obstacle *World::inBuilding( const float *pos, float radius, float height )const
{
...
const Obstacle *World::inBuilding( const float *pos, float angle, float dx, float dy, float dz )const
{
...
const Obstacle *World::hitBuilding( const float *pos, float angle, float dx, float dy, float dz )const
{
	// check walls
...
const Obstacle *World::hitBuilding( const float *oldPos, float oldAngle, const float *pos, float angle, float dx, float dy, float dz, bool directional )const
{
	// check walls
	const ObstacleList &walls = OBSTACLEMGR.getWalls();
	for( unsigned int w = 0; w < walls.size(); w++ )
	...
	for( i = 0; i < olist->count; i++ )
	{
		...
		if( !obs->isDriveThrough() && obs->inMovingBox( oldPos, oldAngle, pos, angle, dx, dy, dz ))
		{
			return obs;
		}
	}
...
bool World::crossingTeleporter( const float *pos, float angle, float dx, float dy, float dz, float *plane )const
{
	const ObstacleList &teleporters = OBSTACLEMGR.getTeles();
...
const Teleporter *World::crossesTeleporter( const float *oldPos, const float *newPos, int &face )const
{
	// check teleporters
	const ObstacleList &teleporters = OBSTACLEMGR.getTeles();
	for( unsigned int i = 0; i < teleporters.size(); i++ )
...
void World::initFlag( int index )
{
	...
	if( flag.status == FlagComing || flag.status == FlagGoing )
	{
		...
		pos[2] = 0.5f * flag.flightEnd *( flag.initialVelocity + 0.25f *BZDBCache::gravity *flag.flightEnd ) + flag.position[2];
...
void World::updateWind( float /*dt*/ )
{
	...
	const float windSpeed = (( 1.0f - wsf ) *minWindSpeed ) + ( wsf *maxWindSpeed );

	const float windAngle = ( float )(( M_PI *2.0 ) *( cos( gt *3.0f * oneMinuteFactor ) + cos( gt *10.0f * oneMinuteFactor )));
	...
	switch( flag.status )
	{
		...
		case FlagInAir:
			...
			else
			{
				...
				flag.position[0] = ( 1.0f - t ) *flag.launchPosition[0] + t *flag.landingPosition[0];
				flag.position[1] = ( 1.0f - t ) *flag.launchPosition[1] + t *flag.landingPosition[1];
				flag.position[2] = ( 1.0f - t ) *flag.launchPosition[2] + t *flag.landingPosition[2] + flag.flightTime *( flag.initialVelocity + 0.5f *BZDBCache::gravity *flag.flightTime );
			}
			break;
		...
		case FlagComing:
			...
			{
				// falling
				flag.position[2] = flag.flightTime *( flag.initialVelocity + 0.5f *BZDBCache::gravity *flag.flightTime ) + flag.landingPosition[2];
				alpha = 1.0f;
			}
			else
			{
				// hovering
				flag.position[2] = 0.5f * flag.flightEnd *( flag.initialVelocity + 0.25f *BZDBCache::gravity *flag.flightEnd ) + flag.landingPosition[2];
				...
				{
					// second half
					float t = ( flag.flightTime - 0.25f * flag.flightEnd ) / ( 0.25f *flag.flightEnd );
					alpha = 1.0f;
		...
		case FlagGoing:
			...
			else
			{
				...
				{
					// first half
					float t = ( 0.75f *flag.flightEnd - flag.flightTime ) / ( 0.25f *flag.flightEnd );
					alpha = 1.0f;
					flagWarpNodes[index]->setSizeFraction( 1.0f - t );
				}
				else
				{
					// second half
					float t = ( flag.flightEnd - flag.flightTime ) / ( 0.25f *flag.flightEnd );
					alpha = t;
					flagWarpNodes[index]->setSizeFraction( t );
				}
			}
...
void World::addFlags( SceneDatabase *scene, bool seerView )
{
	...
	for( int i = 0; i < maxFlags; i++ )
	{
		...
		if(( flags[i].status == FlagComing && flags[i].flightTime < 0.5 *flags[i].flightEnd ) || ( flags[i].status == FlagGoing && flags[i].flightTime >= 0.5 *flags[i].flightEnd ))
...
static void writeDefaultOBJMaterials( std::ostream &out )
{
	...
	const MatProps defaultMats[] = 
	{
		...
		, 
		{
			"basewall_team1", "red\basewall.png", 
			{
				1.0f, 0.8f, 0.8f, 1.0f
			}
		}
		...
		, 
		{
			"basetop_team2", "green\basetop.png", 
			{
				0.8f, 1.0f, 0.8f, 1.0f
			}
		}
		...
		, 
		{
			"basewall_team2", "green\basewall.png", 
			{
				0.8f, 1.0f, 0.8f, 1.0f
			}
		}
		...
		, 
		{
			"basetop_team3", "blue\basetop.png", 
			{
				0.8f, 0.8f, 1.0f, 1.0f
			}
		}
		...
		, 
		{
			"basewall_team3", "blue\basewall.png", 
			{
				0.8f, 0.8f, 1.0f, 1.0f
			}
		}
		...
		, 
		{
			"basetop_team4", "purple\basetop.png", 
			{
				1.0f, 0.8f, 1.0f, 1.0f
			}
		}
		...
		, 
		{
			"basewall_team4", "purple\basewall.png", 
			{
				1.0f, 0.8f, 1.0f, 1.0f
			}
		}
...
static void writeBZDBvar( const std::string &name, void *userData )
{
	...
	if(( BZDB.getPermission( name ) == StateDatabase::Server ) && ( BZDB.get( name ) != BZDB.getDefault( name )) && ( name != "poll" ))
	{
		std::string qmark = "";
		...
		out << indent << "  -set " << name << " " << qmark << BZDB.get( name ) << qmark << std::endl;
	}
	return ;
}
...
bool World::writeWorld( const std::string &filename, std::string &fullname )
{
	...
	// Write weapons
	{
		for( std::vector < Weapon > ::iterator it = weapons.begin(); it != weapons.end(); ++it )
		{
			...
			out << indent << "  position " << weapon.pos[0] << " " << weapon.pos[1] << " " << weapon.pos[2] << std::endl;
	...
	// Write entry zones
	{
		for( std::vector < EntryZone > ::iterator it = entryZones.begin(); it != entryZones.end(); ++it )
		{
			...
			out << indent << "  position " << zone.pos[0] << " " << zone.pos[1] << " " << zone.pos[2] << std::endl;
			out << indent << "  size " << zone.size[0] << " " << zone.size[1] << " " << zone.size[2] << std::endl;
...
static void drawLines( int count, float( *vertices )[3], int color )
{
	const float colors[][4] = 
	{
		...
		,  // gray    (branch node)
		{
			0.25f, 0.25f, 0.0f, 0.8f
		}

		,  // yellow  (regular)
		{
			0.0f, 0.25f, 0.25f, 0.8f
		}

		,  // cyan    (meshed)
		{
			0.25f, 0.0f, 0.25f, 0.8f
		}

		,  // purple  (meshed + regular)
	};
