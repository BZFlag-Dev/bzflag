...
void					World::initFlag(int index)
{
	...
	if (flag.status == FlagComing || flag.status == FlagGoing) {
		...
		pos[2] = 0.5f * flag.flightEnd * (flag.initialVelocity +
					0.25f * BZDB->eval("_gravity") * flag.flightEnd) + flag.launchPosition[2];
	...
	switch (flag.status) {
		...
		case FlagInAir:
			...
			if (flag.flightTime >= flag.flightEnd) {
				...
				flag.position[2] = flag.flightTime * (flag.initialVelocity +
										0.5f * BZDB->eval("_gravity") * flag.flightTime) +
		...
		case FlagComing:
			...
			if (flag.flightTime >= flag.flightEnd) {
				...
				// falling
				flag.position[2] = flag.flightTime * (flag.initialVelocity +
		    0.5f * BZDB->eval("_gravity") * flag.flightTime) + flag.landingPosition[2];
				alpha = 1.0f;
			}
			else {
				// hovering
				flag.position[2] = 0.5f * flag.flightEnd * (flag.initialVelocity +
		    0.25f * BZDB->eval("_gravity") * flag.flightEnd) + flag.landingPosition[2];
