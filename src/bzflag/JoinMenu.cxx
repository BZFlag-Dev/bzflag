...
void JoinMenu::execute()
{
	...
	if( _focus == startServer )
	{
		...
		if( !serverStartMenu )
			serverStartMenu = new ServerStartMenu;
...
void JoinMenu::setFailedMessage( const char *msg )
{
	...
	const float _width = fm.getStrLength( MainMenu::getFontFace(), failedMessage->getFontSize(), failedMessage->getString());
...
void JoinMenu::setStatus( const char *msg, const std::vector < std::string > * )
{
	...
	FontManager &fm = FontManager::instance();
	const float _width = fm.getStrLength( status->getFontFace(), status->getFontSize(), status->getString());
...
void JoinMenu::updateTeamTexture()
{
	...
	const float x = team->getX() + fm.getStrLength( team->getFontFace(), team->getFontSize(), bdl->getLocalString( team->getList()[team->getIndex()] ) + "x" );
