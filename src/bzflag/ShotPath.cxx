...
ShotPath::ShotPath( const FiringInfo &info ): firingInfo( info ), reloadTime( BZDB.eval( StateDatabase::BZDB_RELOADTIME )), startTime( TimeKeeper::getTick()), currentTime( TimeKeeper::getTick()), expiring( false ), expired( false )
{
...
float ShotPath::checkHit( const BaseLocalPlayer *player, float position[3] )const
{
...
LocalShotPath::LocalShotPath( const FiringInfo &info ): ShotPath( info )
{
	// do nothing
}
...
RemoteShotPath::RemoteShotPath( const FiringInfo &info ): ShotPath( info )
{
	// do nothing
}
...
void RemoteShotPath::update( const ShotUpdate &shot, uint16_t code, void *msg )
{
	// update shot info
