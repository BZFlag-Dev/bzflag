...
FiringInfo::FiringInfo(const BaseLocalPlayer& tank, int id)
{
	...
	shot.vel[0] = tankVel[0] + BZDB->eval("_shotSpeed") * dir[0];
	shot.vel[1] = tankVel[1] + BZDB->eval("_shotSpeed") * dir[1];
	shot.vel[2] = tankVel[2] + BZDB->eval("_shotSpeed") * dir[2];
	shot.dt = 0.0f;
	flag = tank.getFlag();
	lifetime = BZDB->eval("_reloadTime");
}
...
ShotPath::ShotPath(const FiringInfo& info) :
								firingInfo(info),
								reloadTime(BZDB->eval("_reloadTime")),
