...
ShotStrategy::ShotStrategy( ShotPath *_path ): path( _path )
{
	// do nothing
}
...
const Obstacle *ShotStrategy::getFirstBuilding( const Ray &ray, float min, float &t )
{
	...
	for( i = 0; i < ( unsigned int )olist->count; i++ )
	{
		...
		if( !obs->isShootThrough())
		{
			...
			if( obs->getType() == Teleporter::getClassName())
			{
				...
				int face;
				if(( timet > min ) && ( timet < t ) && ( tele->isTeleported( ray, face ) < 0.0f ))
				{
					t = timet;
					closestObstacle = obs;
				}
...
const Teleporter *ShotStrategy::getFirstTeleporter( const Ray &ray, float min, float &t, int &f )const
{
