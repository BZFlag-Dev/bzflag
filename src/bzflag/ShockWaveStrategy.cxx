...
ShockWaveStrategy::ShockWaveStrategy( ShotPath *_path ): ShotStrategy( _path ), radius( BZDB.eval( StateDatabase::BZDB_SHOCKINRADIUS )), radius2( radius *radius )
{
	// setup shot
...
void ShockWaveStrategy::update( float dt )
{
	...
	if(( myTank->getFlag() == Flags::Colorblindness ) && ( getPath().getPlayer() != ServerPlayer ))
	{
...
void ShockWaveStrategy::radarRender()const
{
	...
	for( int i = 0; i < sides; i++ )
	{
		...
		glVertex2f( shotPos[0] + radius * cosf( angle ), shotPos[1] + radius * sinf( angle ));
	}
	glEnd();
}
