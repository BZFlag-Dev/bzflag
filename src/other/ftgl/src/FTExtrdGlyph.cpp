    ...
    glList(0)
{
    ...
    for( tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex)
    {
        ...
        glBegin( polyonType);
            for( unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex)
            {
                ...
                glTexCoord2f( (float)point.X() / horizontalTextureScale,
                              (float)point.Y() / verticalTextureScale);
                
                glVertex3f( (float)point.X() / 64.0f,
                            (float)point.Y() / 64.0f,
                            0.0f);
            }
        glEnd();
    ...
    for( tesselationIndex = 0; tesselationIndex < mesh->TesselationCount(); ++tesselationIndex)
    {
        ...
        glBegin( polyonType);
            for( unsigned int pointIndex = 0; pointIndex < subMesh->PointCount(); ++pointIndex)
            {
                ...
                glTexCoord2f( (float)(subMesh->Point(pointIndex).X()) / horizontalTextureScale,
                              (float)(subMesh->Point(pointIndex).Y()) / verticalTextureScale);
                
                glVertex3f( (float)subMesh->Point( pointIndex).X() / 64.0f,
                            (float)subMesh->Point( pointIndex).Y() / 64.0f,
                            -depth);
            }
        glEnd();
    }
    ...
    {
        const FTContour* contour = vectoriser.Contour((unsigned int)c);
        unsigned int numberOfPoints = (unsigned int)contour->PointCount();
        ...
        glBegin( GL_QUAD_STRIP);
            for( unsigned int j = 0; j <= numberOfPoints; ++j)
            {
                ...
                {
                    glTexCoord2f( (float)point.X() / horizontalTextureScale,
                                  (float)point.X() / verticalTextureScale);
                
                    glVertex3f( (float)point.X() / 64.0f, (float)point.Y() / 64.0f, 0.0f);
                    glVertex3f( (float)point.X() / 64.0f, (float)point.Y() / 64.0f, -depth);
                }
                else
                {
                    glTexCoord2f( (float)point.X() / horizontalTextureScale,
...
{
    float vectorX = (float)a.X() - (float)b.X();
    float vectorY = (float)a.Y() - (float)b.Y();
