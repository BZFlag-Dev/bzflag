...
FTContour::FTContour( FT_Vector* contour, char* pointTags, unsigned int numberOfPoints)
{
    for( unsigned int pointIndex = 0; pointIndex < numberOfPoints; ++ pointIndex)
    {
        ...
        {
            AddPoint( (float)contour[pointIndex].x, (float)contour[pointIndex].y);
            continue;
        }
        ...
        if( pointTag == FT_Curve_Tag_Conic)
        {
            ...
            while( nextPointTag == FT_Curve_Tag_Conic)
            {
                ...
                controlPoints[0][0] = (float)previousPoint.X(); controlPoints[0][1] = (float)previousPoint.Y();
                controlPoints[1][0] = (float)controlPoint.X();  controlPoints[1][1] = (float)controlPoint.Y();
                controlPoints[2][0] = (float)nextPoint.X();     controlPoints[2][1] = (float)nextPoint.Y();
        ...
        if( pointTag == FT_Curve_Tag_Cubic)
        {
            ...
            controlPoints[0][0] = (float)previousPoint.X(); controlPoints[0][1] = (float)previousPoint.Y();
            controlPoints[1][0] = (float)controlPoint.X();  controlPoints[1][1] = (float)controlPoint.Y();
            controlPoints[2][0] = (float)controlPoint2.X(); controlPoints[2][1] = (float)controlPoint2.Y();
            controlPoints[3][0] = (float)nextPoint.X();     controlPoints[3][1] = (float)nextPoint.Y();
