...
void dispScores( int who )
{
	...
	{
		x = sortList[i];
		bz_sendTextMessagef( BZ_SERVER, who, "%20.20s :%3d %c", Players[x].callsign, Players[x].score, x == playerLastCapped ? '*' : ' ' );
	}
	Leader = sortList[0];
}
...
void HTFscore::process( bz_EventData *eventData )
{
	...
	if( eventData->eventType == bz_ePlayerJoinEvent )
	{
		...
		bz_debugMessagef( 3, "++++++ HTFscore: Player JOINED (ID:%d, TEAM:%d, CALLSIGN:%s)", joinData->playerID, joinData->team, joinData->callsign.c_str());
		fflush( stdout );
	...
	{
		bz_PlayerJoinPartEventData_V1 *joinData = ( bz_PlayerJoinPartEventData_V1* )eventData;
		bz_debugMessagef( 3, "++++++ HTFscore: Player PARTED (ID:%d, TEAM:%d, CALLSIGN:%s)", joinData->playerID, joinData->team, joinData->callsign.c_str());
		fflush( stdout );
	...
	else if( eventData->eventType == bz_eGameStartEvent )
	{
		...
		bz_debugMessagef( 2, "++++++ HTFscore: Game START (%f, %f)", msgData->time, msgData->duration );
		fflush( stdout );
		htfStartGame();
	...
	{
		bz_GameStartEndEventData_V1 *msgData = ( bz_GameStartEndEventData_V1* )eventData;
		bz_debugMessagef( 2, "++++++ HTFscore: Game END (%f, %f)", msgData->time, msgData->duration );
		fflush( stdout );
		htfEndGame();
	}
}
...
bool commandLineHelp( void )
{
	const char *help[] = 
	{
		"Command line args:  PLUGINNAME,[TEAM=color]", NULL
	};
