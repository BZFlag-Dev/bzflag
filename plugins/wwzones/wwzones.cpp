...
bool WWZHandler::handle(bz_ApiString object, bz_CustomMapObjectInfo * data)
{
  ...
  for (unsigned int h = 0; h < playerList->size(); h++) {
	...
	if (zoneList[i].pointIn(player->lastKnownState.pos) && player->spawned) {
	  if (wasHere(i, player->playerID) && OKToFire(i, player->playerID) && !zoneList[i].zoneWeaponFired) {
	    bz_fireWorldWep(zoneList[i].zoneWeapon.c_str(), zoneList[i].zoneWeaponLifetime, zoneList[i].zoneWeaponPosition,
			    zoneList[i].zoneWeaponTilt, zoneList[i].zoneWeaponDirection, zoneList[i].zoneWeaponShotID,
			    zoneList[i].zoneWeaponDT);
