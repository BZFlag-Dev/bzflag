...
void updatePlayerNextEvent( int playerID, double now )
{
	...
	for( idx = 0; idx < Config.nagMsgs.size(); idx++ )
	{
		...
		{
			if( idx > 0 && ( repeat = nextRepeat( playerTime, Config.nagMsgs[idx - 1] )) > 0 && repeat < Config.nagMsgs[idx]->time )
			{
	...
	if( Players[playerID].nextEventTime < 0 && ( repeat = nextRepeat( playerTime, Config.nagMsgs[Config.nagMsgs.size() - 1] )) > 0 )
	{
		Players[playerID].nextEventTime = Players[playerID].joinTime + repeat;
...
void tickEvent( float time )
{
	...
	if( Config.kickMsg->time > 0 && NumPlayers >= Config.minPlayers )
	{
		// kick someone !
...
void dispNagMsg( int who, char *label, MsgEnt *m )
{
	...
	if( strlen( msg ) > 124 )
	// max line len is currently 125 (not 128!)
		strcpy( &msg[122], "..." );
...
void Nagware::process( bz_EventData *eventData )
{
	...
	if( eventData->eventType == bz_ePlayerJoinEvent )
	{
		...
		bz_debugMessagef( 4, "+++ nagware: Player JOINED (ID:%d, TEAM:%d, CALLSIGN:%s)", joinData->playerID, joinData->team, joinData->callsign.c_str());
		fflush( stdout );
	...
	{
		bz_PlayerJoinPartEventData_V1 *joinData = ( bz_PlayerJoinPartEventData_V1* )eventData;
		bz_debugMessagef( 4, "+++ nagware: Player PARTED (ID:%d, TEAM:%d, CALLSIGN:%s)", joinData->playerID, joinData->team, joinData->callsign.c_str());
		fflush( stdout );
	...
	else if( eventData->eventType == bz_eGameStartEvent )
	{
		...
		bz_debugMessagef( 4, "+++ nagware: Game START (%f, %f)", msgData->time, msgData->duration );
		fflush( stdout );
	...
	{
		bz_GameStartEndEventData_V1 *msgData = ( bz_GameStartEndEventData_V1* )eventData;
		bz_debugMessagef( 4, "+++ nagware: Game END (%f, %f)", msgData->time, msgData->duration );
		fflush( stdout );
		MatchStartTime = 0.0f;
...
bool commandLineHelp( void )
{
	const char *help[] = 
	{
		"Command line args:  PLUGINNAME,configname", "nagware plugin NOT loaded!", NULL
	};
