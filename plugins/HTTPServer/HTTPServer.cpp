...
class HTTPServer : public bz_Plugin, bz_NonPlayerConnectionHandler
{
public:
...
BZ_PLUGIN(HTTPServer)

std::string HTTPServer::serverVersion;
std::string HTTPServer::serverHostname;

void HTTPServer::Init ( const char * /*commandLine*/ )
{
  Unloadable = false;
...
void HTTPConnection::HTTPTask::generateBody (HTTPReply& r, bool noBody)
{
  ...
  case HTTPReply::e301Redirect:
    ...
    pageBuffer += "Host: " + HTTPServer::serverHostname + "\n";
  ...
  case HTTPReply::e401Unauthorized:
    ...
    if (r.authRealm.size())
      ...
      pageBuffer += HTTPServer::serverHostname;
    pageBuffer += "\"\n";
    forceClose = false;
    break;
  ...
  pageBuffer += "Server: " + HTTPServer::serverVersion + "\n";
