...
void RabidRabbitEventHandler::process ( bz_EventData *eventData )
{
	...
	for ( unsigned int i = 0; i < zoneList.size(); i++ )
	{
		...
		{
			bz_fireWorldWep (zoneList[i].WW.c_str(),zoneList[i].WWLifetime,BZ_SERVER,zoneList[i].WWPosition,zoneList[i].WWTilt,zoneList[i].WWDirection,zoneList[i].WWShotID,zoneList[i].WWDT);
	...
	bz_PlayerRecord *player = bz_getPlayerByIndex(playerList->operator[](h));
			...
			if (player){
				...
				for ( unsigned int i = 0; i < zoneList.size(); i++ )
				{
					if (zoneList[i].pointIn(player->pos) && player->spawned && player->team == eRabbitTeam && rrzoneinfo.currentKillZone != i && !rrzoneinfo.rabbitNotifiedWrongZone)
					{
					...
					if (!zoneList[i].pointIn(player->pos) && player->spawned && player->team == eRabbitTeam && rrzoneinfo.rabbitNotifiedWrongZone && rrzoneinfo.rabbitNotifiedWrongZoneNum == i)
						rrzoneinfo.rabbitNotifiedWrongZone = false;

					if (zoneList[i].pointIn(player->pos) && player->spawned && player->team == eRabbitTeam && rrzoneinfo.currentKillZone == i && bz_getTeamCount(eHunterTeam) > 0)
					{
					...
					if (zoneList[i].pointIn(player->pos) && player->spawned && player->team != eRabbitTeam && zoneList[i].zonekillhunter)
					{
						bz_killPlayer(player->playerID, true, BZ_SERVER);
